...
!.. Thompson et al (2008). Coefficient fv is zero for graupel/ice.
      ...
      REAL, PRIVATE:: av_s = 40.0
      REAL, PRIVATE:: bv_s = 0.55
      REAL, PRIVATE:: fv_s = 100.0
      REAL, PARAMETER, PRIVATE:: av_g = 442.0
      REAL, PARAMETER, PRIVATE:: bv_g = 0.89
      REAL, PRIVATE:: av_i = 1847.5
      REAL, PRIVATE:: bv_i = 1.0
...
!ctrlL
      ...
      av_s = snow_fall_a*snow_fall_a_fudge
      bv_s = snow_fall_b
      fv_s = snow_fall_fv

      av_i = clice_fall_a*clice_fall_a_fudge
      bv_i = clice_fall_b
...
!..Initialize various constants for computing radar reflectivity.
      ...
      if (.not. iiwarm) then
        !bloss: Only check for lookup table and read them on master process
        !   If they are read, then they will be broadcast to the rest of the nodes.
        if(masterproc) then
          ...
          if(read_lookup_tables_from_file) then
            ! lookup table file exists.  Open and check to see if it matches current setup
            if(masterproc) CALL wrf_debug(200, '  TRYING TO GET LOOKUP TABLES FROM ' // TRIM(lookup_table_filename) )
            ...
            if(ioerror.ne.0) then
              ! error reading file
              if(masterproc) write(*,*) 'Failed to read integers from thompson_lookup_tables_r1.bin'
            ...
            if(ioerror.ne.0) then
              ! error reading file
              if(masterproc) write(*,*) 'Failed to read parameters from thompson_lookup_tables_r1.bin'
              read_lookup_tables_from_file = .false.
            else
          ...
          if(read_lookup_tables_from_file) then
            ! The lookup table matches the setup here.  Try to read the tables.

            ! get tables computed in qr_acr_qg
            READ(92,IOSTAT=ioerror) tcg_racg, tmr_racg, tcr_gacr, tmg_gacr, &
                 tnr_racg, tnr_gacr
            if(ioerror.ne.0) then
              if(masterproc) write(*,*) 'Failed to read first set of tables from thompson_lookup_tables_r1.bin'
              read_lookup_tables_from_file = .false.
            end if
            ...
            READ(92,IOSTAT=ioerror) tcs_racs1, tmr_racs1, tcs_racs2, tmr_racs2, &
                 ...
                 tnr_racs1, tnr_racs2, tnr_sacr1, tnr_sacr2
            if(ioerror.ne.0) then
              if(masterproc) write(*,*) 'Failed to read second set of tables from thompson_lookup_tables_r1.bin'
              read_lookup_tables_from_file = .false.
            end if
            CLOSE(92)
            if(read_lookup_tables_from_file) then
        ...
        if(dompi) then
          ! broadcast status of lookup tables to other processors
          itmp(1) = 0
          if(masterproc.AND.read_lookup_tables_from_file) itmp(1) = 1
          call task_bcast_integer(0,itmp,1)

          if(itmp(1).eq.1) then
            ! broadcast lookup tables to rest of processors.
            call task_bcast_fourdim_array_real8(0,tcg_racg,ntb_g1,ntb_g,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tmr_racg,ntb_g1,ntb_g,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tcr_gacr,ntb_g1,ntb_g,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tmg_gacr,ntb_g1,ntb_g,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tnr_racg,ntb_g1,ntb_g,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tnr_gacr,ntb_g1,ntb_g,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tcs_racs1,ntb_s,ntb_t,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tmr_racs1,ntb_s,ntb_t,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tcs_racs2,ntb_s,ntb_t,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tmr_racs2,ntb_s,ntb_t,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tcr_sacr1,ntb_s,ntb_t,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tms_sacr1,ntb_s,ntb_t,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tcr_sacr2,ntb_s,ntb_t,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tms_sacr2,ntb_s,ntb_t,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tnr_racs1,ntb_s,ntb_t,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tnr_racs2,ntb_s,ntb_t,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tnr_sacr1,ntb_s,ntb_t,ntb_r1,ntb_r)
            call task_bcast_fourdim_array_real8(0,tnr_sacr2,ntb_s,ntb_t,ntb_r1,ntb_r)

            read_lookup_tables_from_file = .true.
          else
            read_lookup_tables_from_file = .false.
          end if ! if tables were read successfully on masterproc
        end if !if(dompi)

        if(.NOT.read_lookup_tables_from_file) then
          ! compute lookup tables on each processor.
          ...
          if(masterproc) CALL wrf_debug(200, '  creating rain collecting snow table')
          if(masterproc) CALL wrf_debug(200, '  This may take a few minutes on a sinlge processor...')
          call qr_acr_qs
...
!  rho when they are put together into q*ten.
      ...
      !bloss(2025-03)
      REAL:: max_prs_iau, max_pni_iau, iau_scale, tmp_xx
!+---+
      debug_flag = .false.
...
!..Deposition/sublimation of cloud ice (Srivastava & Coen 1992).
          if (L_qi(k)) then
           ...
           if (pri_ide(k) .lt. 0.0) then
            ...
            if(.NOT.do_preserve_cloud_ice) then !bloss(2025-03)
            prs_ide(k) = (1.0D0-tpi_ide(idx_i,idx_i1))*pri_ide(k)
            pri_ide(k) = tpi_ide(idx_i,idx_i1)*pri_ide(k)
            end if !if(.NOT.do_preserve_cloud_ice) bloss(2025-03)
           endif
...
!.. table that resulted from explicit bin representation of distrib.
           if(do_preserve_cloud_ice) then
              if(ssati(k).gt.0.) then !bloss(2025-03)
                 ! Only allow autoconversion when Sice > 1
                 max_prs_iau = MIN(ri(k)*0.875*odts, ri(k)*0.875/30. ) ! qi e-folding time ~ 34 sec.
                 max_pni_iau = MIN(ni(k)*0.12*odts, ni(k)*0.12/30. ) ! Dice halves in ~ 30 sec.
                 if ( (idx_i.eq. ntb_i) .or. (xDi.gt. 5.0*D0s) ) then
                    prs_iau(k) = max_prs_iau
                    pni_iau(k) = max_pni_iau
                 elseif (xDi.lt. 0.1*D0s) then
                    prs_iau(k) = 0.
                    pni_iau(k) = 0.
                 else ! 0.1*D0s < xDi < 5*D0s
                    prs_iau(k) = tps_iaus(idx_i,idx_i1)*odts
                    pni_iau(k) = tni_iaus(idx_i,idx_i1)*odts
                    if(D0i_iau.gt.0.1*D0s) then !bloss: restrain iau
                       ! no autoconv for xDi<D0i_iau, weakens autoconv for xDi<2*D0i_iau
                       tmp_xx = MAX(0., MIN(1., (xDi-D0i_iau) / (2.*D0i_iau-D0i_iau) ) )
                       iau_scale = 0.5*( 1. - cos(pi*tmp_xx) )
                       prs_iau(k) = iau_scale*prs_iau(k)
                       pni_iau(k) = iau_scale*pni_iau(k)
                    end if !bloss(2025-03): restrain iau
                    if(prs_iau(k).gt.max_prs_iau) then
                       ! scale down autoconversion using same factor for q,N
                       iau_scale = MAX(R1, max_prs_iau) / MAX(R1, prs_iau(k)) ! R1 ~ 1e-18 kg/kg
                       prs_iau(k) = prs_iau(k)*iau_scale
                       pni_iau(k) = pni_iau(k)*iau_scale
                    end if
                 end if
              end if ! if(ssati(k).gt.0.) then !bloss(2025-03)
           else !bloss(2025-03)
           ! This is the default Thompson ice autoconversion
           if ( (idx_i.eq. ntb_i) .or. (xDi.gt. 5.0*D0s) ) then
        ...
        endif!bloss: if(do_preserve_cloud_ice)
     endif
...
!..Local variables
     ...
     &                504,720,990,1320,1716,2184,2730,3360,4080,4896/)
      ...
      !bloss(2025-03): Set default values
      re_qc1d(:) = 5.e-6
      re_qi1d(:) = 25.e-6
      re_qs1d(:) = 25.e-6
      dge_qi1d(:) = 25.e-6

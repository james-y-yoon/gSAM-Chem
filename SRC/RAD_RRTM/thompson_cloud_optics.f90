...
!DEBUG ONLY    use params, only: dothirtypercentcloud
   ...
   use vars, only: qcl, qci
    ...
    if(allocated(CloudLiquidMassMixingRatio)) then
       do ilon = 1,nx
          m = k_terra(ilon,ilat)-1 ! account for changing index of lowest atmospheric layer
          lwp(ilon, 1:nz-m) = CloudLiquidMassMixingRatio(ilon, ilat, m+1:nz) * 1.e3 * layerMass(ilon, 1:nz-m) ! scale from kg to g 
       end do
    else
       ! use qcl(:,:,:) instead
       do ilon = 1,nx
          m = k_terra(ilon,ilat)-1
          lwp(ilon, 1:nz-m) = qcl(ilon, ilat, m+1:nz) * 1.e3 * layerMass(ilon, 1:nz-m) ! scale from kg to g 
       end do
    end if
    ...
    if(ALLOCATED(CloudIceMassMixingRatio)) then
       do ilon = 1,nx
          m = k_terra(ilon,ilat)-1 ! account for changing index of lowest atmospheric layer
          iwp(ilon, 1:nz-m) = CloudIceMassMixingRatio(ilon, ilat, m+1:nz) * 1.e3 * layerMass(ilon, 1:nz-m) ! scale from kg to g
       end do
    else
       ! use qci(:,:,:) instead
       do ilon = 1,nx
          m = k_terra(ilon,ilat)-1
          iwp(ilon, 1:nz-m) = qci(ilon, ilat, m+1:nz) * 1.e3 * layerMass(ilon, 1:nz-m) ! scale from kg to g
       end do
    end if

    do ilon = 1,nx
       m = k_terra(ilon,ilat)-1 ! account for changing index of lowest atmospheric layer
       rel(ilon, 1:nz-m) = reffc(ilon,ilat,m+1:nz)
       dgi(ilon, 1:nz-m) = reffi(ilon,ilat,m+1:nz)
    end do

    if(.NOT.reff_ice_holds_Dge) then
       ! we DO need to translate reff into Dge
       ...
       dgi(:,:) = dgi(:,:)*rho_cloud_ice/917._r8*2._r8
    end if
    !
    ...
    if(dosnow_radiatively_active) then
       ...
       do ilon = 1,nx
          ...
          swp(ilon, 1:nz-m) = SnowMassMixingRatio(ilon, ilat, m+1:nz) * 1.e3 * layerMass(ilon, 1:nz-m) ! scale from kg to g 
          dgs(ilon, 1:nz-m) = reffs(ilon,ilat,m+1:nz)
       end do

      if(.NOT.reff_ice_holds_Dge) then
        ! we DO need to translate reff into Dge
        dgs(:,:) = dgs(:,:)*rho_snow/917._r8*2._r8
      end if
...
!bloss    999 format('Max tauLW = ',F10.4,' rel at max tauLW= ',F10.4,' lwp at max tauLW= ',F10.4)
  ...
  subroutine compute_liquid_cloud_optics(nx, nz, lwp, rel, tauLW, tauSW, taussaSW, taussagSW, taussafSW)
    ...
    do i = 1, nUse
      ...
      radliq(i)  = MAX(2.5, MIN(60., rel( iUse(i), jUse(i) ) ) )

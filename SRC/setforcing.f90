...
!	Read sounding file (snd):
 ...
 allocate(usnd(nzsnd,nsnd),vsnd(nzsnd,nsnd), &
          ...
          zsnd(nzsnd,nsnd),psnd(0:nzsnd,nsnd),daysnd(nsnd))
 do n=1,nsnd-1
   read(77,*) daysnd(n),i,psnd(0,n)
   do i=1,nzsnd
       read(77,*) zsnd(i,n),psnd(i,n),tsnd(i,n),qsnd(i,n),usnd(i,n),vsnd(i,n)
       if(docap_snd_cu) call snd_cap(usnd(i,n),vsnd(i,n))
   end do
 end do
 close(77)
 if(nsnd.gt.2) then
 ...
 psnd(0:nzsnd,nsnd) = psnd(0:nzsnd,nsnd-1)
 ...
 daysnd(2) = 10000.
 do n=1,2
   zsnd(1:nzsnd,n) = z(1:nzsnd)
   psnd(0:nzsnd,n) = presi(1:nzsnd+1)
   tsnd(1:nzsnd,n) = t0(1:nzsnd)
   qsnd(1:nzsnd,n) = q0(1:nzsnd)
   usnd(1:nzsnd,n) = u0(1:nzsnd)+ug
   vsnd(1:nzsnd,n) = v0(1:nzsnd)+vg
 end do

end if ! .not.dosimplesnd
...
open(77,file='./CASES/'//trim(case)//'/lsf',status='old',form='formatted') 
read(77,*)
nlsf=0
do while(.true.)
...
open(77,file='./CASES/'//trim(case)//'/rad',status='old',form='formatted') 
...
if(dosfcforcing) then
    open(77,file='./CASES/'//trim(case)//'/sfc',status='old',form='formatted') 
    read(77,*)
    nsfc=0
    do while(.true.)
    ...
    rewind(77)
    read(77,*)
    ! determine if there are more data in line than in classic SAM sfc file:
    extended_data = .false.
    read(77,err=78,end=78,fmt=*) tmp,tmp,tmp,tmp,tmp,tmp,tmp,tmp
    ! extended_data = .true.
    78 continue
    rewind(77)
    read(77,*)
    ...
    allocate(t2msfc(nsfc),td2msfc(nsfc),wind10msfc(nsfc))
    if(extended_data) then
     do n=1,nsfc
       read(77,*) daysfc(n),sstsfc(n),shsfc(n),lhsfc(n),tausfc(n),t2msfc(n),td2msfc(n),wind10msfc(n)
     end do
    else
     do n=1,nsfc

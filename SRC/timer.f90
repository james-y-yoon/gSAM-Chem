...
character(50):: name(NTIMERS_MAX) = ' ' ! unique name of the given timer
logical:: active(NTIMERS_MAX) =.false. ! to tell if it is active now
integer:: parent(NTIMERS_MAX) = 0      ! parrent (called from) timer
real(8):: time_start(NTIMERS_MAX)  ! time when t_startf is called 
real(8):: time(NTIMERS_MAX) = 0. ! accumulated time
...
real(8):: time0 ! time of initialization of timers

contains

recursive subroutine print_children(iparent,fmt)
use grid, only: nstep_run
implicit none
...
do i=1,ntimers
  if(parent(i).eq.iparent) then
   write(*,'('//fmt//')') trim(name(i)),ncalls(i),time(i),time(i)/real(nstep_run,8)*1000., &
                          time(i)/time(1)*100.

...
integer i,j,k
real dtabs, tabs1, tabs2, an, bn, ap, bp, om, ag, omp
real fac1,fac2  
real fff,dfff,dqsat
...
do k = 1, nzm
 ...
 ! Marat 2023
    ...
    if(tabs(i,j,k).lt.235.and.qn(i,j,k).lt.1.e-8) then
    ...
    if(q(i,j,k)-qn(i,j,k).lt.0.) then
      print*,'cloud.f90: q(i,j,k)-qn(i,j,k).lt.0.! rank,i,j,k,landmask,pres,gamaz,q,qn,qp,qsat, &
         t,dtabs,tabs2,tabs,sst,qrad,niter:',&
           rank,i,j,k,landmask(i,j),pres(k),gamaz(k),q(i,j,k),qn(i,j,k),qp(i,j,k),qsatt(i,j,k),t(i,j,k),dtabs,tabs2,tabs(i,j,k),sst_xy(i,j),qrad(i,j,k),niter 
      call task_abort()
    end if

...
use slm_vars, only: seaicemask, slm_forcing
use tracers, only: tracers_source

implicit none


integer i,j,k,n,nn,m,iz,iday0,iday,it,jt
real coef, radtend, dayy, tmp
real tt(nzm,2),qq(nzm,2),uu(nzm,2),vv(nzm,2),ww(nzm,2)
real ratio1,ratio2,ratio_t1,ratio_t2
logical zgrid, tabssnd
real tauv,taut,trce,phi
real, external :: qsatw
...
!      qq(iz,n)=max(0.,2.*qq(iz-1,n)-qq(iz-2,n))
  ...
  do k=1,nzm
    ...
    if(qg0(k).lt.0.) then ! convert from RH to mixing ratio
      qg0(k)=-qg0(k)/100.*qsatw(tg0(k),pres(k)) 
    else
      qg0(k)=qg0(k)*1.e-3
    end if
...
! recompute reference pressure profiles:
    ...
    do iz = 1,nzm
     if(zgrid) then
      ...
      !   coef = (pres(iz)-pls(i-1,m))/(pls(i,m)-pls(i-1,m))
         coef = pls(i,m)*(pls(i-1,m)-pres(iz))/(pres(iz)*(pls(i-1,m)-pls(i,m)))
         tt(iz,n)=(1.-coef)*dtls(i-1,m)+coef*dtls(i,m)
         qq(iz,n)=(1.-coef)*dqls(i-1,m)+coef*dqls(i,m)
         uu(iz,n)=(1.-coef)*ugls(i-1,m)+coef*ugls(i,m)
         vv(iz,n)=(1.-coef)*vgls(i-1,m)+coef*vgls(i,m)
         ww(iz,n)=(1.-coef)*wgls(i-1,m)+coef*wgls(i,m)
         goto 12
       endif
      end do
     end if
  ...
  end if !dolargescale.and.time.gt.timelargescale

end if ! .not.readinit

!--------------------------------------------------------------------
if(dohs94) then
! Held-Suarez 1994 test:
 bet(:) = ggr/tabs0(:)
 do k=1,nzm
  tauv = 1./86400.*max(0.,(pres(k)/pres(1)-0.7)/(1.-0.7))
  do j=1,ny
   phi = latitude(1,j)*pi/180.
   trce = max(200.,(315.-60.*sin(phi)**2-10.*log(pres(k)/1000.)*cos(phi)**2) &
                               /prespot(k)) + gamaz(k)
   taut = (1./40.+(1./4-1./40.)*max(0.,(pres(k)/pres(1)-0.7)/(1.-0.7))*cos(phi)**4)/86400.*dtn
   do i=1,nx
     dudt(i,j,k,na)= dudt(i,j,k,na)-tauv*u(i,j,k)
     dvdt(i,j,k,na)= dvdt(i,j,k,na)-tauv*v(i,j,k)
     dwdt(i,j,k,na)= dwdt(i,j,k,na)-tauv*w(i,j,k)
     t(i,j,k) = t(i,j,k)-taut*(t(i,j,k)-trce)
   end do
  end do
 end do
end if
...
if(dosfcforcing) then
    ...
    if(dostatis) then
      sst1obs = tabs_s  ! sst is not averaged over the sampling period
      lhobs = lhobs + fluxq0 * rhow(1)*lcond
      shobs = shobs + fluxt0 * rhow(1)*cp
      tauobs = tauobs + tau0 
      t2mobs = t2mobs + t2msfc(nn)+(t2msfc(nn+1)-t2msfc(nn))*coef
      q2mobs = q2mobs + qsatw(td2msfc(nn)+(td2msfc(nn+1)-td2msfc(nn))*coef,pres0)
      wind10mobs = wind10mobs + wind10msfc(nn)+(wind10msfc(nn+1)-wind10msfc(nn))*coef
    end if

else

    if(readsst.and..not.dofcast.and.daysstobs(2).gt.0) then
      if(day.gt.daysstobs(2)) call readsurface(sstfile,sstobs,daysstobs)
      coef=(day-daysstobs(1))/(daysstobs(2)-daysstobs(1))
      do j=1,ny
       do i=1,nx
         sstxy0(i,j) = sstobs(i,j,1)+(sstobs(i,j,2)-sstobs(i,j,1))*coef
    ...
    if(SLM) call slm_forcing()

endif

!-----------------------------------------------------------------------------------
if(dotracers.and.dopointsource.and.nstep.ge.pointsource_start_step) then

...
real grd,betdz,Ck,Ce,Ces,Ce1,Ce2,smix,Pr,Cee,smixmo
real buoy_sgs,ratio,a_prod_sh,a_prod_bu,a_diss
real lstarn, lstarp, bbb, omn, omp, sumxy
real coef(ny),qsatt,dqsat,icp
integer i,j,k,kc,kb

real tabs_interface, qp_interface, qtot_interface, qsat_check, qctot
real, external :: qsatw,qsati,dtqsatw,dtqsati
real, allocatable :: dtdiss(:,:,:) ! dissipative heating if doaddTKEdiss=.true. - MK 2025
...
if(doaddTKEdiss) then
   allocate(dtdiss(nx,ny,nzm))
   dtdiss(:,:,:) = 0.
   tdiss(:) = 0.
end if
...
if((nstep.eq.1).AND.(icycle.eq.1)) then
  ...
  do j = 1,ny
    do i = 1,nx
      ...
      grd=(dz*adz(k)*coef(j))**0.33333
   !   grd=dz*adz(k)  ! standard SAM
      Ce1=Ces/0.7*0.19
      Ce2=Ces/0.7*0.51
      Cee=Ce1+Ce2
...
!    Try to estimate  buoy_sgs_below without using tk as it can be non exiting creating
!    oscillations and unphysically large values. Instead, assume that shear priduction is zero
!    (otherwise a complex cubic equation would result),
!    and use smagorinski formula for eddy diffusivity to estimete ibuoy_sgs_below.
!    Only works for positive surface buoyancy flux. 
!    For negative fluxes just assume zero production. -MK, 2022
    if(a_prod_bu_below(i,j).gt.0.) then
       grd=(dz*adz(k)*coef(j))**0.33333
     !  grd=dz*adz(k) ! standard SAM
       buoy_sgs_below(i,j) = -(a_prod_bu_below(i,j)**2*Ce/(Ck**3*Pr*grd**4))**0.3333
    else
       buoy_sgs_below(i,j) = 0.
    end if
  end do
end do
  ...
  do j=1,ny
   ...
   ! grd=dz*adz(k)  ! standard SAM
    do i=1,nx
     ...
     ! make buoyancy production at the very first level above the surface only controlled 
     ! by surface fluxes to avoid some weird cases when cold air over land is advected over 
     ! warm water - a fix that should be addressed in the future - MK 2026
       if(k.eq.k_terra(i,j)) then
         buoy_sgs = buoy_sgs_below(i,j)
       else
...
!       smixmo = 0.4*(z(k)-zi(k_terra(i,j)))  ! added  behaviour near surface as mixing l = kappa*z
!       smix = sqrt(sqrt(1./(1./smix**4+1./smixmo**4)))! and then going seemlessly to smix MK 2026
       ...
       if(dosmagor) then
         ...
         a_diss=min(0.1,a_prod_sh+a_prod_bu) ! cap dissipataon rate at 0.1 m2/s3
         ...
         a_prod_sh=(tk(i,j,k)+0.001)*def2(i,j,k)
         a_prod_bu = 0.5*( a_prod_bu_below(i,j) + a_prod_bu_above(i,j) )
         a_diss=min(0.1,tke(i,j,k)/(4.*dt),Cee/smix*tke(i,j,k)**1.5) ! cap the diss rate (useful for large time steps
       ...
       if(doaddTKEdiss) then
          dtdiss(i,j,k) = dtn*icp*a_diss
          tdiss(k) = tdiss(k) + dtdiss(i,j,k)
       end if
       ...
       buoy_sgs_below(i,j) = buoy_sgs_above(i,j)
       a_prod_bu_below(i,j) = a_prod_bu_above(i,j)
  ...
  tkelediss(k) = tkelediss(k)/(sumxy+1.e-5)
 
end do ! k

if(doaddTKEdiss) t(1:nx,1:ny,1:nzm) = t(1:nx,1:ny,1:nzm) + dtdiss(1:nx,1:ny,1:nzm)

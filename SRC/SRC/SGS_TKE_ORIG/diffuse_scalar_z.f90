...
do j=1,ny
 do i=1,nx
  ...
  c = -tkz*iadzw
  b = 1. - c
  d = (1.+c)*field(i,j,k) - c*field(i,j,kc) + dtn*rhow(k)/(rho(k)*dz*adz(k))*fluxb(i,j)
  alpha(i,j,k) = -c/b
  beta(i,j,k) = d/b
 end do
end do
 ...
 do j=1,ny
   do i=1,nx
    if(k.gt.k_terra(i,j)) then
      ...
      c = -tkz*iadzw
      b = 1. - a - c
      d = (1.+a+c)*field(i,j,k) - a*field(i,j,kb) - c*field(i,j,kc)
      e = b + a*alpha(i,j,k-1)
      alpha(i,j,k) = -c/e
...
do j=1,ny
 do i=1,nx
  ...
  a = -tkz*iadz
  b = 1. - a
  d = (1.+a)*field(i,j,k) - a*field(i,j,kb) - dtn*rhow(kc)/(rho(k)*dz*adz(k))*fluxt(i,j)
  e = b + a*alpha(i,j,k-1)

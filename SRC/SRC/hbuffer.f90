...
implicit none
	...
	character(16) namelist(HBUF_MAX_LENGTH)	
...
hbuf_length = hbuf_length+trcount
trcount = 0
if (dochem) call hbuf_chem_init(namelist,deflist,unitlist,status,average_type,count,trcount)
hbuf_length = hbuf_length+trcount
...
if(masterproc) then
  ...
  open (ntape,file='./OUT_STAT/'// &
                  case(1:lenstr(case))//'_'// &
                  caseid(1:lenstr(caseid))//'.stat', &
                  status='unknown',form='unformatted' )!,BUFFEREDYES ACTION='READWRITE')
  if(nstep.ne.nstat) then
    do while(.true.)
      if(nrestart.eq.2) then
        read(ntape, end=223) 
      else
        read(ntape, end=222) 
      end if
...
223 continue
  ...
  write(ntape)   &
        ...
        real(sst1obs,4),real(lhobs*aver,4),real(shobs*aver,4), &
        ...
        real(s_fldsc*aver*factor,4),real(s_fsdsc*aver*factor,4),&
        real(s_t2m*aver*factor,4),real(s_q2m*aver*factor,4),&
        real(s_u10m*aver*factor,4),real(s_v10m*aver*factor,4),&
        real(s_snowd*aver*factor,4),real(s_snowt*aver*factor,4), &
        real(s_lhfc*aver*factor,4), real(s_shfc*aver*factor,4), &
        real(s_lhfs*aver*factor,4), real(s_shfs*aver*factor,4), &
        real(s_soilflux*aver*factor,4), &
        real(s_tsoil*aver*factor,4), real(s_tcanopy*aver*factor,4), &
        real(n_face_sunny*aver,4),real(n_face_shady*aver,4),real(t_face_sunny*aver,4), &
        real(t_face_shady*aver,4),real(lw_face*aver,4), &
        real(sw_face*aver,4),real(shf_face*aver,4),&
        real(lw_face_sfc*aver,4),real(lw_face_face*aver,4), &
        real(tair_str*aver,4),real(skt_str*aver,4), &
        real(tair_out*aver,4),real(skt_out*aver,4), &
        real(tair_str_min*aver,4),real(tair_out_min*aver,4), &
        real(tair_str_max*aver,4),real(tair_out_max*aver,4), &
        real(snowmean*aver*factor,4),real(prectot*factor*86400.,4),real(wvp*aver*factor,4), &
        real(cwp*aver*factor*1000.,4), real(iwp*aver*factor*1000.,4), &
        real(rwp*aver*factor*1000.,4), &
        real(swp*aver*factor*1000.,4), real(gwp*aver*factor*1000.,4), &
        real(s_lhf*aver*factor,4), real(s_shf*aver*factor,4), real(wvpobs*aver*factor,4), &
        real(tauobs*aver,4), real(t2mobs*aver,4), real(q2mobs*aver,4), real(wind10mobs*aver,4) 
  write(ntape) length
  ...
  do l = 1,hbuf_length
   ...
   !      print*,namelist(l),minval(hbuf_real4((l-1)*nzm+1:(l-1)*nzm+nzm)), &
   !                         maxval(hbuf_real4((l-1)*nzm+1:(l-1)*nzm+nzm))
     end if
  end do
  close (ntape)
  print *,'done with stat file... '

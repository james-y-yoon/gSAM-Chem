...
subroutine sgs_setparm()
  ...
  !======================================================================
  ! not these parameters, except dosmagor are not active in standard-SAM's SGS
  NAMELIST /SGS_TKE/ &
       dosmagor, tk_factor, dodns, PR_DNS, DIFF_DNS, RHO_DNS, PRES_DNS, &
                     dochamber, t_top, t_bot, t_wall, dosidewalls 
  ...
  if(dochamber) then
      if(masterproc) print*,'dochamber cannot be set in standard-SAM SGS. exit...'
       call task_abort()
  end if
  if(dodns) then
      if(masterproc) print*,'dodns cannot be set in standard-SAM SGS'
       call task_abort()
  end if
  if(doimplicitdiff) then
      if(masterproc) print*,'doimplicitdiff cannot be set in standard-SAM'
       call task_abort()
  end if

...
contains
  ...
  subroutine compute_m2005_cloud_optics(nx, nz, ilat, layerMass, cloudFrac, &
    ...
    !
!DEBUG ONLY    use params, only: dothirtypercentcloud
   ...
   use terrain, only: k_terra
    ...
    do ilon = 1,nx
       ! account for changing index of lowest atmospheric layer
       m = k_terra(ilon,ilat)-1
       lwp(ilon, 1:nz-m) = CloudLiquidMassMixingRatio(ilon, ilat, m+1:nz)  * layerMass(ilon, 1:nz-m)
       iwp(ilon, 1:nz-m) = CloudIceMassMixingRatio(ilon, ilat, m+1:nz)  * layerMass(ilon, 1:nz-m)

       pgam(ilon, 1:nz-m) = CloudLiquidGammaExponent(ilon,ilat,m+1:nz)
       lamc(ilon, 1:nz-m) = CloudLiquidLambda(ilon,ilat,m+1:nz)
       ...
       dgi(ilon, 1:nz-m) = reffi(ilon,ilat,m+1:nz)*rho_cloud_ice/917._r8*2._r8
    end do
    ! 
    ...
    if(dosnow_radiatively_active) then
       ...
       do ilon = 1,nx
          ...
          swp(ilon, 1:nz-m) = SnowMassMixingRatio(ilon, ilat, m+1:nz) * layerMass(ilon, 1:nz-m)
          dgs(ilon, 1:nz-m) = reffs(ilon,ilat,m+1:nz)*rho_snow/917._r8*2._r8
       end do
    ...
    !bloss: Re-define cloud fraction here, setting cloud fraction to one
    !  in grid cells with snow but not cloud liquid/ice.
    if(dosnow_radiatively_active) then
       do k = 1,nz
          do ilon = 1,nx
             if((cloudFrac(ilon,k).eq.0.).AND.(swp(ilon,k).gt.0.)) then
                cloudFrac(ilon,k) = 1.
             end if
          end do
       end do
...
!DEBUG ONLY    if(dothirtypercentcloud) cloudFrac(:,:) = 0.3*cloudFrac(:,:)
    ...
    lwp(1:nx,1:nz) = lwp(1:nx,1:nz) / (1.e-6 + CloudFrac(1:nx,1:nz))
    iwp(1:nx,1:nz) = iwp(1:nx,1:nz) / (1.e-6 + CloudFrac(1:nx,1:nz))
    if(dosnow_radiatively_active) then
    ...
    do k = 1,nz
       do ilon = 1,nx
          ! convert grid-mean optical depths to in-cloud values
          tauLW(:,ilon,k) = tauLW(:,ilon,k) / (1.e-6 + CloudFrac(ilon,k))
          tauSW(:,ilon,k) = tauSW(:,ilon,k) / (1.e-6 + CloudFrac(ilon,k))
          tauSW_cldliq(:,ilon,k) = tauSW_cldliq(:,ilon,k) / (1.e-6 + CloudFrac(ilon,k))
          tauSW_cldice(:,ilon,k) = tauSW_cldice(:,ilon,k) / (1.e-6 + CloudFrac(ilon,k))
          tauSW_snow(:,ilon,k) = tauSW_snow(:,ilon,k) / (1.e-6 + CloudFrac(ilon,k))
       end do
    end do

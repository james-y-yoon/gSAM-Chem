...
use stat_coars
use micro_params, only: doprecipdiss
...
integer nprec, iprec
real  flagstat, coef, coefdt, coefdiss(nzm)
...
do k = 1,nzm
    ...
    iwmax(k)   = 1./wmax
    if(doprecipdiss) then
      coefdiss(k) = -ggr*coef/(rho(k)*cp) ! coefficient to compute dissipative heating due to falling precip - MK 2025
    else
      coefdiss(k) = 0.
    end if
end do
...
do j=1,ny
   do i=1,nx
      ...
      do k=k_terra(i,j),nzm
         ...
         wp(k)=rhofac(k)*term_vel(i,j,k,ind)/gamma_RAVE
      ...
      do iprec = 1,nprec
         ...
         if(nonos) then
            ...
            do k=k_terra(i,j),nzm
               kc=min(nzm,k+1)
               mx(k)=rho(k)*adz(k)*(mx(k)-tmp_qp(k))/(pneg(www(kc)) + ppos(www(k))+eps)
               mn(k)=rho(k)*adz(k)*(tmp_qp(k)-mn(k))/(ppos(www(kc)) + pneg(www(k))+eps)
            end do
            ...
            do k=k_terra(i,j),nzm
               ...
               fz(k) = fz(k) &                        ! Upwind flux
                    + ppos(www(k))*min(1.,mx(k), mn(kb)) &
                    - pneg(www(k))*min(1.,mx(kb),mn(k)) ! Anti-diffusive flux
            end do
         ...
         do k=k_terra(i,j),nzm
            ...
            qp(i,j,k)=max(0.,qp(i,j,k)-(fz(kc)-fz(k))*irhoadz(k))
            qpfall(k)=qpfall(k)-(fz(kc)-fz(k))*irhoadz(k)*flagstat  ! For qp budget
            lat_heat = -(lfac(kc)*fz(kc)-lfac(k)*fz(k))*irhoadz(k)
            t(i,j,k)=t(i,j,k)-lat_heat + coefdiss(k)*fz(k)
            tlat(k)=tlat(k)-lat_heat*wgt(j,k)            ! For energy budget
            tpdiss(k)=tpdiss(k) + coefdiss(k)*fz(k)*wgt(j,k)            ! For energy budget
            precflux(k) = precflux(k) - fz(k)*flagstat*wgt(j,k)   ! For statistics
         end do
         prate = -coef * fz(k_terra(i,j)) * flagstat ! precip rate in mm/s
         precinst(i,j) = precinst(i,j) + prate
         prectot = prectot + wgty(j)*prate*dtn ! For statistics
         if(dostatis) then
            precsfc(i,j) = precsfc(i,j) + prate*dtn ! For statistics
            precsfcsnow(i,j) = precsfcsnow(i,j) + (1.-omega(i,j,k_terra(i,j)))*prate*dtn ! For statistics
         end if
         prec_xy(i,j) = prec_xy(i,j) + prate*dtn ! For 2D output
         preca_xy(i,j) = preca_xy(i,j) + prate*dtn ! For 2D output
         if(snow2Dout) then
           precs_xy(i,j) = precs_xy(i,j) + prate*(1.-omega(i,j,k_terra(i,j)))*dtn ! For 2D output
           precsa_xy(i,j) = precsa_xy(i,j) + prate*(1.-omega(i,j,k_terra(i,j)))*dtn ! For 2D output
         end if

         if(collect_coars.and.flagstat.eq.1.) then
            misc(i,j,1:nzm) = misc(i,j,1:nzm) - fz(1:nzm)*coef
            misc1(i,j,1:nzm) = misc1(i,j,1:nzm) - fz(1:nzm)*lfac(1:nzm)*coef
         end if

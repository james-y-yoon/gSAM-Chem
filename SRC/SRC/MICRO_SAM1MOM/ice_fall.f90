...
use vars
use microphysics, only: micro_field, index_cloud_ice, total_water, icefall_fudge, donomicro, dovticeifs
!use micro_params
use params
   ...
   do j = 1,ny
      do i = 1,nx
        if(k.ge.k_terra(i,j)) then
         ...
         if(dovticeifs) then
!          ice sedimentation similar to IFS Cy47r3
           vt_ice = fudge(j)*0.1 *rhofac  ! From IFS Cy47r3
         else 

...
subroutine micro_setparm()
  ...
  integer ierr, ios, ios_missing_namelist, place_holder
   ...
   NAMELIST /MICRO_M2005/ &
      ...
      do_output_micro_process_rates, doeiscld, &
      snow_fall_a, snow_fall_b, snow_fall_a_fudge, & ! option to change snow fall speed relationship
      clice_fall_a, clice_fall_b, clice_fall_a_fudge, & ! option to change cloud ice fall speed relationship
      clice_snow_Dauto ! option to change autoconversion lengthscale
  ...
  !------------
  if(masterproc) print*,'Microphysics: M2005'
  ...
  if(masterproc) write (*,nml=MICRO_M2005,IOSTAT=ios)
  close(55)
...
!bloss/qt: cloud liquid water no longer prognosed
  ...
  if(.not.isallocatedMICRO) then
     ...
     allocate(micro_field(dimx1_s:dimx2_s,dimy1_s:dimy2_s,nzm,nmicro_fields), &
          ...
          cloudliq(nx,ny,nzm), &
          tmtend3d(nx,ny,nzm), flag_micro3Dout(nmicro_fields), flag_micro2DZout(nmicro_fields),&
      ...
      do k=1,nzm
       do j=1,ny
        do i=1,nx
          ...
          qpi(i,j,k) = max(0.,qpi(i,j,k))
          qpg(i,j,k) = max(0.,qpg(i,j,k))
...
subroutine micro_init()
  ...
  if(docloud.AND.(doprecip.OR.dopredictNc)) then
     flag_micro3Dout = 1
     flag_micro2DZout = 1
     total_micro_3Dout = sum(flag_micro3Dout) + 1
  end if
...
use params, only: fac_cond, fac_sub, rgas, gamma_RAVE
use grid, only: z, zi, wgty, dx, dy, mu, ady
use vars, only: t,  gamaz, prectot, precsfc, precsfcsnow, precflux, qpfall, tlat, prec_xy, preca_xy, &
     precs_xy, precsa_xy, nstep, nstatis, icycle, total_water_prec, precinst
...
integer :: i1, i2, j1, j2, i, j, k, m, n, kk
...
!bloss/qt      tmpqcl(:) = micro_field(i,j,:,iqcl)
      ...
      tmptabs(kk:) = t(i,j,kk:)  &           ! liquid water-ice static energy over Cp
           - gamaz(kk:) &                                   ! potential energy
           + fac_cond * (tmpqr(kk:)) &    ! bloss/qt: liquid latent energy due to rain only
           + fac_sub  * (tmpqci(kk:) + tmpqs(kk:) + tmpqg(kk:)) ! ice latent energy
...
!      tmpw = 0.5*(w(i,j,1:nzm) + w(i,j,2:nz))  ! MK: changed for stretched grids 
      ...
      call satadj_liquid(nzm-kk+1,tmptabs(kk:),tmpqv(kk:),tmpqcl(kk:),tmppres(kk:))
      ...
      ! send above-terrain profiles only. (MK)

      call m2005micro_graupel(&
           mtendqcl(kk:),mtendqci(kk:),mtendqs(kk:),mtendqr(kk:), &
           mtendncl(kk:),mtendnci(kk:),mtendns(kk:),mtendnr(kk:), &
           tmpqcl(kk:),tmpqci(kk:),tmpqs(kk:),tmpqr(kk:), &
           tmpncl(kk:),tmpnci(kk:),tmpns(kk:),tmpnr(kk:), &
           tmtend1d(kk:),mtendqv(kk:), &
           tmptabs(kk:),tmpqv(kk:),tmppres(kk:),rho(kk:),tmpdz(kk:),tmpw(kk:),tmpwsub(kk:), &
           sfcpcp, sfcicepcp, &
           effc1d(kk:),effi1d(kk:),effs1d(kk:),effr1d(kk:), &
           dtn, &
           i1,i2, j1,j2, 1,nzm-kk+1, i1,i2, j1,j2, 1,nzm-kk+1, &
           mtendqg(kk:),mtendng(kk:),tmpqg(kk:),tmpng(kk:),effg1d(kk:),stendqg(kk:), &
           stendqr(kk:),stendqci(kk:),stendqs(kk:),stendqcl(kk:), &
           tmp_cl_pgam(kk:), tmp_cl_lambda(kk:), &
           micro_proc_rates(kk:,:),do_accumulate_process_rates)
...
!         total_water_prec = total_water_prec + sfcpcp
         ...
         precsa_xy(i,j) = precsa_xy(i,j) + sfcicepcp

         ! update rain
         micro_field(i,j,kk:,iqr) = tmpqr(kk:)
         micro_field(i,j,kk:,inr) = tmpnr(kk:)
      else
         ! add rain to cloud
         tmpqcl(kk:) = tmpqcl(kk:) + tmpqr(kk:) ! add rain mass back to cloud water
         tmpncl(kk:) = tmpncl(kk:) + tmpnr(kk:) ! add rain number back to cloud water
         ...
         stendqcl(kk:) = stendqcl(kk:) + stendqr(kk:)
         mtendqcl(kk:) = mtendqcl(kk:) + mtendqr(kk:)
         mtendncl(kk:) = mtendncl(kk:) + mtendnr(kk:)
      ...
      micro_field(i,j,kk:,iqv) = tmpqv(kk:) + tmpqcl(kk:) !bloss/qt: total water
      cloudliq(i,j,kk:) = tmpqcl(kk:) !bloss/qt: auxilliary cloud liquid water variable
      if(dopredictNc) micro_field(i,j,kk:,incl) = tmpncl(kk:)

      reffc(i,j,kk:) = effc1d(kk:)
      CloudLiquidMassMixingRatio(i,j,kk:) = tmpqcl(kk:)
      CloudLiquidGammaExponent(i,j,kk:) = tmp_cl_pgam(kk:)
      CloudLiquidLambda(i,j,kk:) = tmp_cl_lambda(kk:)

      if(doicemicro) then
         micro_field(i,j,kk:,iqci) = tmpqci(kk:)
         micro_field(i,j,kk:,inci) = tmpnci(kk:)
         micro_field(i,j,kk:,iqs) = tmpqs(kk:)
         micro_field(i,j,kk:,ins) = tmpns(kk:)
         if(dograupel) then
            micro_field(i,j,kk:,iqg) = tmpqg(kk:)
            micro_field(i,j,kk:,ing) = tmpng(kk:)
         end if
         reffi(i,j,kk:) = effi1d(kk:)  
         reffs(i,j,kk:) = effs1d(kk:)  
         CloudIceMassMixingRatio(i,j,kk:) = tmpqci(kk:)
         SnowMassMixingRatio(i,j,kk:) = tmpqs(kk:)
      end if

      !=====================================================
      ! update liquid-ice static energy due to precipitation
      t(i,j,kk:) = t(i,j,kk:) &
           - dtn*fac_cond*(stendqcl(kk:)+stendqr(kk:)) &
           - dtn*fac_sub*(stendqci(kk:)+stendqs(kk:)+stendqg(kk:))
      !=====================================================

      if(dostatis) then
!bloss/qt: total water microphysical tendency includes qv and qcl
         mtend(kk:,iqv) = mtend(kk:,iqv) + mtendqv(kk:) + mtendqcl(kk:)
!bloss/qt         mtend(kk:,iqcl) = mtend(kk:,iqcl) + mtendqcl
         if(dopredictNc) mtend(kk:,incl) = mtend(kk:,incl) + mtendncl(kk:)
         if(doprecip) then
            mtend(kk:,iqr) = mtend(kk:,iqr) + mtendqr(kk:)
            mtend(kk:,inr) = mtend(kk:,inr) + mtendnr(kk:)
         end if

         if(doicemicro) then
            mtend(kk:,iqci) = mtend(kk:,iqci) + mtendqci(kk:)
            mtend(kk:,inci) = mtend(kk:,inci) + mtendnci(kk:)
            !bloss            stend(kk:,inci) = stend(kk:,inci) + stendnci(kk:)

            mtend(kk:,iqs) = mtend(kk:,iqs) + mtendqs(kk:)
            mtend(kk:,ins) = mtend(kk:,ins) + mtendns(kk:)
            !bloss            stend(kk:,ins) = stend(kk:,ins) + stendns(kk:)

            if(dograupel) then
               mtend(kk:,iqg) = mtend(kk:,iqg) + mtendqg(kk:)
               mtend(kk:,ing) = mtend(kk:,ing) + mtendng(kk:)
               !bloss            stend(kk:,ing) = stend(kk:,ing) + stendng(kk:)
            end if
         end if
         ...
         tlat(kk:nzm) = tlat(kk:nzm) &
              - dtn*fac_cond*(stendqcl(kk:)+stendqr(kk:)) &
              - dtn*fac_sub*(stendqci(kk:)+stendqs(kk:)+stendqg(kk:))
         qpfall(kk:nzm) = qpfall(kk:nzm) + dtn*(stendqr(kk:)+stendqs(kk:)+stendqg(kk:))

         !bloss: temperature tendency (sensible heating) due to phase changes
         tmtend3d(i,j,kk:) = tmtend1d(kk:)

      end if ! dostatis

      stend(kk:,iqv) = stend(kk:,iqv) + stendqcl(kk:) !bloss/qt: iqcl --> iqv
      if(doprecip) then
         stend(kk:,iqr) = stend(kk:,iqr) + stendqr(kk:)
      end if

      if(doicemicro) then
         stend(kk:,iqci) = stend(kk:,iqci) + stendqci(kk:)
         stend(kk:,iqs) = stend(kk:,iqs) + stendqs(kk:)
         if(dograupel) stend(kk:,iqg) = stend(kk:,iqg) + stendqg(kk:)
      end if
...
precflux(kk:nzm) = precflux(kk:nzm) - mksed(kk:,iqv)*dtn/dz
...
if(doprecip) then
   ...
   precflux(kk:nzm) = precflux(kk:nzm) - mksed(kk:,iqr)*dtn/dz
end if
...
if(doicemicro) then
   ...
   precflux(kk:nzm) = precflux(kk:nzm) &
        - (mksed(kk:,iqci) + mksed(kk:,iqs) + mksed(kk:,iqg))*dtn/dz
end if
...
if(doicemicro) then
   ...
   if(dograupel) then
      qpg(1:nx,1:ny,1:nzm) = micro_field(1:nx,1:ny,1:nzm,iqg) 
...
if(dosnow_radiatively_active) then
   where(qcl(1:nx,1:ny,1:nzm)+qci(1:nx,1:ny,1:nzm) &
        + SnowMassMixingRatio(1:nx,1:ny,1:nzm).gt.0)
...
!-----------------------------------------
subroutine satadj_liquid(nnn,tabs,qt,qc,pres)
  ...
  real, intent(inout) :: qt(:)  ! on input: qt; on output: qv
  real, intent(out)   :: qc(:) ! cloud liquid water, kg/kg
  ...
  do k = 1,nnn
    ...
    qsat1 = 0.622*esat1/ max(esat1,pres(k)-esat1) ! added max to avoid unphysical values
    ...
    if (qt(k).gt.qsat1) then
      ...
      do while((ABS(dtabs).GT.thresh) .AND. (niter.lt.maxiter))
        ...
        esat1 = polysvp(tabs1,0)
        qsat1 = 0.622*esat1/ max(esat1,pres(k) - esat1) ! saturation mixing ratio
      ...
      if(niter.gt.maxiter-1) then
          write(*,*) 'Reached iteration limit in satadj_liquid', &
                                      tabs(k),tabs1,qt(k),esat1,qsat1,qc(k),pres(k) 
                                             
       !  don't stop, just ignore - MK 2025
           cycle
 !         stop
      end if

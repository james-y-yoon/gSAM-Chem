module micro_params
   ...
   logical, public :: &
        ...
        doeiscld = .false. ! set shallow cloud fraction for radiation based on EIS

! Marat's scaling of autoconversion/activation based on grid spacing (dx).
   logical, public :: do_scale_dependence_of_autoconv = .false. 
   logical, public :: do_scale_dependence_of_activation = .false. 
...
!bloss   real, public :: ccnconst = 1000., ccnexpnt = 0.5 ! continental suggestion from module_mp_graupel
  ...
  ! NOTE v ~ (rho0/rho)^0.54*MIN(1.2, v), so large predicted fallspeeds will not be applied.
  real :: snow_fall_a_fudge = 1. ! scaling factor
  real :: snow_fall_a = 11.72 ! m/s
  real :: snow_fall_b = 0.41 ! exponent
  ...
  !    v = a*D^b, with units of v (m/s), D (m)
  ! NOTE v ~ (rho0/rho)^0.35*MIN(1.2, v), so large predicted fallspeeds will not be applied.
  real :: clice_fall_a_fudge = 1. ! scaling factor
  real :: clice_fall_a = 700. ! m/s
  real :: clice_fall_b = 0.865 ! exponent
  ! Suggested options: (with v for D=10, 100, 1000um)
  ...
  ! gSAM MK tune: clice_fall_a = 700., clice_fall_b = 0.865, v = 0.03, 0.24, 1.78 m/s
  ...
  !  - cloud-ice-to-snow autoconversion lengthscale, units m
  real :: clice_snow_Dauto = 250.e-6 ! gSAM default
  ! real :: clice_snow_Dauto = 125.e-6 ! M2005 default
  !bloss(2025-03) end ==============
...
!   in the statistics file.  Only 11 for warm cloud microphysics.
   ...
   real gridarea   ! needed for some fudge factors

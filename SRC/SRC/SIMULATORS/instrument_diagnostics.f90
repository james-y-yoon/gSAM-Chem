module instrument_diagnostics 
  ...
  !
  use vars,   only: nx, ny, nz, nzm, sstxy, t00, qv, tabs, qcl, qci, tabs0, qv0
  use grid,   only: z, pres, pres0,presi, doisccp, domodis, domisr, dosimfilesout 
      ...
      if(domodis) then
        ...
        if(dosnow_radiatively_active) then
          ...
          re_i(:,:) = reshape(tau_067_cldice(1:nx,1:ny,nzm:1:-1)*rad_reffi(1:nx,1:ny,nzm:1:-1) &
                        , shape = (/ nx*ny, nzm /) )
          re_i(:,:) = re_i(:,:) + reshape(tau_067_snow(1:nx,1:ny,nzm:1:-1)*reffs(1:nx,1:ny,nzm:1:-1) &
                        , shape = (/ nx*ny, nzm /) )

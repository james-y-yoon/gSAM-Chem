subroutine box_smooth_3d_real8(A,TERR,nx,ny,nzm,nsx,nsy,M,N,comm,kblock)
...
!   works only for MPI
!
!  DOUBLE PRECISION VERSION for averaging large variables like temperature
!
!  ALGORITHM
!  ---------
...
!-----------------------------------------------------------------------
  ...
  ! SAT for a k-block
  real(8), allocatable :: pxX(:,:,:), pxW(:,:,:)
  real(8), allocatable :: SX(:,:,:),  SW(:,:,:)

  ! Exscan buffers
  real(8), allocatable :: send_west_X(:,:), recv_west_X(:,:)
  real(8), allocatable :: send_west_W(:,:), recv_west_W(:,:)
  real(8), allocatable :: send_south_X(:,:), recv_south_X(:,:)
  real(8), allocatable :: send_south_W(:,:), recv_south_W(:,:)

  ! Neighborhood tiles of SAT (concatenated into big local arrays)
  integer :: rxL, rxR, ryB, ryT
  integer :: nRx, nRy
  integer :: nxN, nyN
  real(8), allocatable :: SXN(:,:,:), SWN(:,:,:)   ! (nxN, nyN, kb)
...
contains
  ...
  subroutine gather_sat_tiles(Slocal, Sbig, nx,ny,nk, rx,ry, rxL,rxR,ryB,ryT, nsx,nsy, comm)
    ...
    real(8)   , intent(in)  :: Slocal(nx,ny,nk)
    real(8)   , intent(inout):: Sbig((rxR-rxL+1)*nx, (ryT-ryB+1)*ny, nk)
  ...
  subroutine pack_tile(S, nx,ny,nk, buf)
    ...
    real(8)   , intent(in) :: S(nx,ny,nk)
    real(8)   , intent(out):: buf(nx*ny*nk)
    integer :: i,j,k,idx
    idx = 0
    do k = 1, nk
      do j = 1, ny
  ...
  subroutine unpack_tile(Sbig, NXB,NYB,nk, bx,by, nx,ny, buf)
    ...
    real(8)   , intent(inout) :: Sbig(NXB,NYB,nk)
    real(8)   , intent(in)    :: buf(nx*ny*nk)
    integer :: i,j,k,idx
    idx = 0
    do k = 1, nk
      do j = 1, ny
  ...
  pure real function sat_from_neigh(Sbig, nx,ny,nRx,nRy,rxL,ryB,nxg,nyg, Ig,Jg, kk)
    ...
    real(8)   , intent(in) :: Sbig(nRx*nx, nRy*ny, *)
...
end subroutine box_smooth_3d_real8

...
use vars
use params
use microphysics, only: nmicro_fields, micro_field, mkname, mklongname, mkunits
use terrain
implicit none
...
real(8) thlfac, qwfac, wfac, divfac(nzm), divfacu(nzm), divfacv(nzm)
...
!    divfac=float(navgmom_x)*float(navgmom_y)
    ...
    do k=1,nzm
	  ...
	  micro(k,6)=micro(k,6)+evapb
         ...
         if(terrau(i,j,k).gt.0.) then
          divfacu(k) = divfacu(k)+1 
         ...
         if(terrav(i,j,k).gt.0.) then
          divfacv(k) = divfacv(k)+1 
...
!          write(*,*) 'print i and j ', i, j
    ...
    do k=1,nzm
      moment1(k,2,ii,jj)=thetal(k,1)/(divfac(k)+1.e-10)
      moment1(k,3,ii,jj)=totalqw(k,1)/(divfac(k)+1.e-10)
      moment1(k,1,ii,jj)=vertw(k,1)/(divfac(k)+1.e-10)
      moment1(k,4,ii,jj)=uavg(k,1)/(divfacu(k)+1.e-10)
      moment1(k,5,ii,jj)=vavg(k,1)/(divfacv(k)+1.e-10)

      moment2(k,2,ii,jj)=(thetal(k,2)/(divfac(k)+1.e-10))
      moment2(k,3,ii,jj)=(totalqw(k,2)/(divfac(k)+1.e-10))
      moment2(k,1,ii,jj)=(vertw(k,2)/(divfac(k)+1.e-10))
      moment2(k,4,ii,jj)=(uavg(k,2)/(divfacu(k)+1.e-10))
      moment2(k,5,ii,jj)=(vavg(k,2)/(divfacv(k)+1.e-10))

      moment3(k,2,ii,jj)=(thetal(k,3)/(divfac(k)+1.e-10))
      moment3(k,3,ii,jj)=(totalqw(k,3)/(divfac(k)+1.e-10))
      moment3(k,1,ii,jj)=(vertw(k,3)/(divfac(k)+1.e-10))
      moment3(k,4,ii,jj)=(uavg(k,3)/(divfacu(k)+1.e-10))
      moment3(k,5,ii,jj)=(vavg(k,3)/(divfacv(k)+1.e-10))
      moment3(k,6,ii,jj)=(qclavg(k,2)/(divfac(k)+1.e-10))
      moment3(k,7,ii,jj)=(qclavg(k,3)/(divfac(k)+1.e-10))
      moment3(k,8,ii,jj)=(qclavg(k,4)/(divfac(k)+1.e-10))
      moment3(k,9,ii,jj)=(qclavg(k,5)/(divfac(k)+1.e-10))

      moment4(k,2,ii,jj)=(thetal(k,4)/(divfac(k)+1.e-10))
      moment4(k,3,ii,jj)=(totalqw(k,4)/(divfac(k)+1.e-10))
      moment4(k,1,ii,jj)=(vertw(k,4)/(divfac(k)+1.e-10))

      moment5(k,1,ii,jj)=(vertw(k,5)/(divfac(k)+1.e-10))
      moment5(k,2,ii,jj)=(vertw(k,6)/(divfac(k)+1.e-10))
      moment5(k,3,ii,jj)=(vertw(k,7)/(divfac(k)+1.e-10))
      moment5(k,4,ii,jj)=(vertw(k,8)/(divfac(k)+1.e-10))
      moment5(k,5,ii,jj)=(vertw(k,9)/(divfac(k)+1.e-10))
      moment5(k,6,ii,jj)=(vertw(k,10)/(divfac(k)+1.e-10))
      moment5(k,7,ii,jj)=(vertw(k,11)/(divfac(k)+1.e-10))
      moment5(k,8,ii,jj)=(vertw(k,12)/(divfac(k)+1.e-10))      

      mom1cld(k,1,ii,jj)=(qclavg(k,1)/(divfac(k)+1.e-10))
      mom1cld(k,2,ii,jj)=(cldcnt(k)/(divfac(k)+1.e-10))
      mom1cld(k,3,ii,jj)=(micro(k,1)/(divfac(k)+1.e-10))
      mom1cld(k,4,ii,jj)=(micro(k,2)/(divfac(k)+1.e-10))
      mom1cld(k,5,ii,jj)=(micro(k,3)/(divfac(k)+1.e-10))
      mom1cld(k,6,ii,jj)=(micro(k,4)/(divfac(k)+1.e-10))
      mom1cld(k,7,ii,jj)=(micro(k,5)/(divfac(k)+1.e-10))
      mom1cld(k,8,ii,jj)=(micro(k,6)/(divfac(k)+1.e-10))
...
integer i,j,k,n,nfields,nfields1,ztype
...
if(masterproc) then
  ...
  if(savemomsep) then
    filename='./OUT_MOMENTS/'//trim(case)//'_moments_'//trim(caseid)//'_'// &
      timechar(1:10)//filetype
    open(46,file=filename,status='unknown',form='unformatted')	
  else
    filename='./OUT_MOMENTS/'//trim(case)//'_moments_'//trim(caseid)//filetype
  end if

  open(46,file=filename,status='unknown',form='unformatted')	


   if(.not.savemomsep) then
 ...
 222   continue
   ...
   write(46) nstep, time
   write(46) nxm,nym,nzm,nsubdomains,nsubdomains_x,nsubdomains_y,nfields,nfiles3D
   write(46) real(dx*navgmom_x,4),real(dy*navgmom_y,4)
   write(46) real(float(nstep)*dt/(3600.*24.)+day0,4),datechar
   write(46) dolatlon
   write(46) 'atm'
   write(46) real(z(1:nzm),4),real(pres(1:nzm),4), &
             real(lat_gl(::navgmom_y),4),real(lon_gl(::navgmom_x),4),&
             real(y_gl(::navgmom_y),4),real(dz*adz(1:nzm),4),real(rho(1:nzm),4),real(rhow(1:nzm),4)
...
units='K'
call compress3D(tmp,nxm,nym,nzm,name,long_name,units,savemombin,dompi,rank,nsubdomains,1,ztype)

...
CONTAINS
   ...
   !   y_cell_edge_coords    - REAL(8) array, y-edge coordinates (size y_fine_cells + 1)
   !   z_cell_edge_coords    - REAL(8) array, z-edge coordinates (size z_fine_cells + 1)
   !   p_cell_edge           - REAL(8) array, pressure at cell edges (size z_fine_cells + 1)
   ...
   !   rad_reduc_factor      - REAL(8), radial reduction factor
   !
   ! OUTPUTS:
   ...
   SUBROUTINE gmg_init(y_fine_cells, z_fine_cells, &
      ...
      REAL(8), INTENT(INOUT) :: y_cell_edge_coords(y_fine_cells + 1), &
         z_cell_edge_coords(z_fine_cells + 1)
      REAL(8), INTENT(INOUT) :: p_cell_edge(z_fine_cells + 1)
      CHARACTER(len=1), INTENT(IN) :: mu_discr, rho_discr
      LOGICAL, INTENT(IN) :: mu_is_const
      REAL(8), INTENT(IN) :: rad_reduc_factor

      ! Declare local variables
      INTEGER :: i, lvls, lvl, y_padside, z_padside
      REAL(8), ALLOCATABLE :: y_cell_widths(:), z_cell_widths(:)
      REAL(8), ALLOCATABLE :: y_cell_widths_work(:), z_cell_widths_work(:)
      REAL(8), ALLOCATABLE :: y_finer_cell_cent_coords(:), &
         z_finer_cell_cent_coords(:), y_coarser_cell_cent_coords(:), z_coarser_cell_cent_coords(:)
      REAL(8) :: min_diag_val, max_diag_val
...
! INPUTS:
!   solver      - TYPE(MultigridSolver), input/output
!   alpha_dble  - REAL(8), reaction coefficient
!   rhs_dble    - REAL(8) array, right-hand side vector
!   tol         - REAL(8), tolerance for convergence
!   max_v_cyc   - INTEGER, maximum number of V-cycles
!
! INPUTS/OUTPUTS:
!   sol_dble    - REAL(8) array, solution vector
!
! OUTPUTS:
!   res_norm    - REAL(8), final residual norm
...
!-----------------------------------------------------------------------
   SUBROUTINE gmg_solve(alpha_dble, rhs_dble, tol, max_v_cyc, &
      ...
      REAL(8), INTENT(IN) :: alpha_dble
      REAL(8), INTENT(IN) :: rhs_dble(solver%multigrid(1)%cells(3))
      REAL(8), INTENT(IN) :: tol
      INTEGER, INTENT(IN) :: max_v_cyc

      ! Declare input/output variables
      REAL(8), INTENT(INOUT) :: sol_dble(solver%multigrid(1)%cells(3))
      ...
      INTEGER :: i, lvls, lvl
      REAL :: alpha_sngl
      REAL(8) :: rhs_norm, trans_rhs_norm
      REAL(8) :: trans_res_norm
      REAL(8) :: res_norm_old, trans_res_norm_old

...
CONTAINS
   ...
   !   y_cell_edge_coords   - REAL(8) array, y-edge coordinates
   !   p_cell_edge          - REAL(8) array, pressure cell edges
   !   y_cell_widths        - REAL(8) array, cell widths in y
   !   z_cell_widths        - REAL(8) array, cell widths in z
   !   y_cell_cent_coords   - REAL(8) array, y-center coordinates
   ...
   !   grd%A_val_dble       - REAL(8) array, CRS values
   !   grd%A_diag_val_dble  - REAL(8) array, diagonal values (if mixed_precision)
   !-----------------------------------------------------------------------
   SUBROUTINE discretize(grd, opts, y_cell_edge_coords, p_cell_edge, &
      ...
      REAL(8), INTENT(IN) :: y_cell_edge_coords(grd%cells(1) + 1)
      REAL(8), INTENT(IN) :: p_cell_edge(grd%cells(2) + 1)
      REAL(8), INTENT(IN) :: y_cell_widths(grd%cells(1)), z_cell_widths(grd%cells(2))
      REAL(8), INTENT(IN) :: y_cell_cent_coords(grd%cells(1))
      ...
      ! Local variables
      INTEGER :: i, j
      REAL(8), ALLOCATABLE :: mu_cent(:), mu_edge(:)
      REAL(8), ALLOCATABLE :: rho_cent(:)
      INTEGER :: cent_ind
      INTEGER :: ind
      INTEGER :: total_nnz
      REAL(8) :: cent_val
...
!   rad_earth        - REAL(8), Earth's radius.
!   rad_reduc_factor - REAL(8), Scaling factor for radial reduction.
! Outputs:
!   mu               - REAL(8) array, Updated diffusion coefficients (size length).
!-----------------------------------------------------------------------
   SUBROUTINE diffusion_coeff(length, mu_is_const, rad_earth, rad_reduc_factor, mu)
      ...
      LOGICAL, INTENT(IN) :: mu_is_const
      REAL(8), INTENT(IN) :: rad_earth
      REAL(8), INTENT(IN) :: rad_reduc_factor
      REAL(8), INTENT(INOUT) :: mu(length)

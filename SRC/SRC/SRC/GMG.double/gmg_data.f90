...
MODULE gmg_data
   ...
   TYPE :: Grid
      ...
      DOUBLE PRECISION, ALLOCATABLE :: A_val_dble(:), A_diag_val_dble(:)
      REAL, ALLOCATABLE :: A_val_sngl(:), A_diag_val_sngl(:)

      ! Solution arrays
      DOUBLE PRECISION, ALLOCATABLE :: sol_dble(:)
      REAL, ALLOCATABLE :: sol_sngl(:)

      ! Right-hand side
      DOUBLE PRECISION, ALLOCATABLE :: rhs_dble(:)
      REAL, ALLOCATABLE :: rhs_sngl(:)

      ! Smoother buffer
      DOUBLE PRECISION, ALLOCATABLE :: GS_rhs_dble(:)
      REAL, ALLOCATABLE :: GS_rhs_sngl(:)

      ! Tridiagonal solver buffer
      DOUBLE PRECISION, ALLOCATABLE :: GS_up_diag_dble(:)
      REAL, ALLOCATABLE :: GS_up_diag_sngl(:)

      ! Residual
      DOUBLE PRECISION, ALLOCATABLE :: res_dble(:)
      REAL, ALLOCATABLE :: res_sngl(:)

      ! Prolongation operator
      INTEGER, ALLOCATABLE :: P_row_ptr(:), P_col_ind(:)
      DOUBLE PRECISION, ALLOCATABLE :: P_val_dble(:)
      REAL, ALLOCATABLE :: P_val_sngl(:)

      ! Restriction operator
      INTEGER, ALLOCATABLE :: R_row_ptr(:), R_col_ind(:)
      DOUBLE PRECISION, ALLOCATABLE :: R_val_dble(:)
   ...
   TYPE :: Options
      ...
      DOUBLE PRECISION :: rad_earth = 6.371229D+6
...
CONTAINS
   ...
   !   alppha   - DOUBLE PRECISION, the reaction coefficient
   !   rhs      - DOUBLE PRECISION array, the right-hand side vector
   ...
   SUBROUTINE dump_linsys(grd, alpha, rhs, filename)
      ...
      DOUBLE PRECISION, INTENT(IN) :: rhs(grd%cells(3))

...
CONTAINS
   ...
   !   y_finer_cell_cent_coords  - DOUBLE PRECISION array, finer y-center coordinates
   !   z_finer_cell_cent_coords  - DOUBLE PRECISION array, finer z-center coordinates
   !   y_padside                 - INTEGER, padding side in y-direction
   !   z_padside                 - INTEGER, padding side in z-direction
   !   y_cell_edge_coords        - DOUBLE PRECISION array, y-edge coordinates, input/output
   !   z_cell_edge_coords        - DOUBLE PRECISION array, z-edge coordinates, input/output
   !   y_cell_widths             - DOUBLE PRECISION array, cell widths in y, input/output
   !   z_cell_widths             - DOUBLE PRECISION array, cell widths in z, input/output
   !   p_cell_edge               - DOUBLE PRECISION array, pressure cell edges in z-direction, input/output
   !
   ! OUTPUTS:
   !   y_coarser_cell_cent_coords - DOUBLE PRECISION array, coarser y-center coordinates
   !   z_coarser_cell_cent_coords - DOUBLE PRECISION array, coarser z-center coordinates
   !   y_cell_widths_work         - DOUBLE PRECISION array, working array for cell widths in y
   !   z_cell_widths_work         - DOUBLE PRECISION array, working array for cell widths in z
   !-----------------------------------------------------------------------
   SUBROUTINE coarsen(solver, finer_lvl, y_finer_cell_cent_coords, z_finer_cell_cent_coords, &
      ...
      DOUBLE PRECISION, INTENT(IN) :: y_finer_cell_cent_coords(:)
      DOUBLE PRECISION, INTENT(IN) :: z_finer_cell_cent_coords(:)
      INTEGER, INTENT(INOUT) :: y_padside, z_padside
      DOUBLE PRECISION, INTENT(INOUT) :: y_cell_edge_coords(:)
      DOUBLE PRECISION, INTENT(INOUT) :: z_cell_edge_coords(:)
      DOUBLE PRECISION, INTENT(INOUT) :: y_cell_widths(:)
      DOUBLE PRECISION, INTENT(INOUT) :: z_cell_widths(:)
      DOUBLE PRECISION, INTENT(INOUT) :: p_cell_edge(:)
      DOUBLE PRECISION, INTENT(OUT) :: y_coarser_cell_cent_coords(:)
      DOUBLE PRECISION, INTENT(OUT) :: z_coarser_cell_cent_coords(:)
      DOUBLE PRECISION, INTENT(OUT) :: y_cell_widths_work(:)
      DOUBLE PRECISION, INTENT(OUT) :: z_cell_widths_work(:)
   ...
   !   sol_dble   - DOUBLE PRECISION array, solution vector, input/output
   !   alpha      - DOUBLE PRECISION, reaction coefficient
   ...
   SUBROUTINE apply_cycle(solver, sol_dble, alpha, fmg, mixed_precision)
      ...
      DOUBLE PRECISION, INTENT(INOUT) :: sol_dble(:)
...
!   y_padside                  - INTEGER, padding side in y-direction
!   z_padside                  - INTEGER, padding side in z-direction
!   y_coarser_cell_widths      - DOUBLE PRECISION array, cell widths of coarser grid in y
!   z_coarser_cell_widths      - DOUBLE PRECISION array, cell widths of coarser grid in z
!   y_coarser_cell_cent_coords - DOUBLE PRECISION array, coarser y-center coordinates
!   z_coarser_cell_cent_coords - DOUBLE PRECISION array, coarser z-center coordinates
!   y_finer_cell_cent_coords   - DOUBLE PRECISION array, finer y-center coordinates
!   z_finer_cell_cent_coords   - DOUBLE PRECISION array, finer z-center coordinates
...
!   solver%multigrid(coarser_lvl)%P_val_dble - DOUBLE PRECISION array, prolongation values
...
!-----------------------------------------------------------------------
   SUBROUTINE build_prolongator(solver, coarser_lvl, y_padside, z_padside, &
      ...
      DOUBLE PRECISION, INTENT(IN) :: y_coarser_cell_widths(:), z_coarser_cell_widths(:)
      DOUBLE PRECISION, INTENT(IN) :: y_finer_cell_cent_coords(:), z_finer_cell_cent_coords(:)
      DOUBLE PRECISION, INTENT(IN) :: y_coarser_cell_cent_coords(:), z_coarser_cell_cent_coords(:)
      ...
      DOUBLE PRECISION :: factor, y_factor, z_factor
...
!   solver%multigrid(coarser_lvl)%R_val_dble  - DOUBLE PRECISION array, restriction values
...
!-----------------------------------------------------------------------
   SUBROUTINE build_restrictor_fvm(solver, finer_lvl, y_padside, z_padside, &
      ...
      DOUBLE PRECISION, INTENT(IN) :: y_finer_cell_widths(:), z_finer_cell_widths(:)
      DOUBLE PRECISION, INTENT(IN) :: y_coarser_cell_widths(:), z_coarser_cell_widths(:)
      DOUBLE PRECISION, INTENT(IN) :: y_finer_cell_cent_coords(:), z_finer_cell_cent_coords(:)
      DOUBLE PRECISION, INTENT(IN) :: y_coarser_cell_cent_coords(:), z_coarser_cell_cent_coords(:)
      ...
      DOUBLE PRECISION :: y_factor, z_factor, y_overlap, z_overlap
      DOUBLE PRECISION :: area_total, area_coarse, area_fine
      INTEGER :: nnz, num_entries
      INTEGER :: y_side, z_side
      INTEGER :: idx, m, n
      INTEGER :: y_fine, z_fine
      INTEGER, DIMENSION(2) :: y_f_inds, z_f_inds
      DOUBLE PRECISION :: y_diff, z_diff, y_range, z_range
      DOUBLE PRECISION :: weight, total_weight

...
!       Works only with Cartesian grids
...
use vars
use params, only: dowallx, dowally, docolumn, dolatlon
implicit none
...
if(dolatlon) then
   if(masterproc) print*,'pressure_orig cannot be used for latlon grid! exit...'
   call task_abort()
end if
...
do m = 0,nsubdomains-1
  ...
  if(m.lt.npressureslabs.and.m.ne.rank) then
    ...
    n = m*nzslab + 1
    call task_bsend_float(m,p(0,1-YES3D,n,na),nzslab*nxp1*nyp1, 33)
  endif
	...
	call task_test(reqs_in(m), flag(m), rf, tag)
	   ...
	   count=count-1
           ...
           do k = 1,nzslab
            do j=1-YES3D,ny
             do i=0,nx
               p(i,j,k+n,na) = bufp_slabs(i,j,k,m)
             end do
            end do
           end do
        endif   

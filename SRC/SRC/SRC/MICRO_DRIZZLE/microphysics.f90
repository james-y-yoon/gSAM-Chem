...
integer, parameter :: flag_advect(nmicro_fields) = (/1,1,1/) ! dummy parameter in SAM1MOM (don't change -MK)

! SAM1MOM 3D microphysical fields are output by default.
integer :: flag_micro3Dout(nmicro_fields) = (/0,0,0/) ! redefined in micro_init
...
real, allocatable, dimension(:,:,:) :: reffs, &
...
subroutine micro_setparm()
  ...
  !------------
  if(masterproc) print*,'Microphysics: DRIZZLE'
  ...
  close(55)
   ...
   if(masterproc) then
      ...
      write (unit=55,nml=MICRO_DRIZZLE,IOSTAT=ios)
      write(55,*) ' '
      close(unit=55)
   end if
...
subroutine micro_write_fields3D(nfields1)
  ...
  if(doprecip) then
    ...
    do k=1,nzm
     do j=1,ny
      do i=1,nx
        tmp(i,j,k)=conp(i,j,k)*rho(k)*1.e-6
      end do
     end do
    end do
    name='CONP'
    long_name='Drizzle Number Concenration'
    units='#/cm3'
    call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains,nfiles3D,1)
   end if

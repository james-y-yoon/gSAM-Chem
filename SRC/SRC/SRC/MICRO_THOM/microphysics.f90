...
use params, only: cp, rgas, rv, lcond, lsub, fac_cond, fac_sub, ggr, &
     doprecip, docloud, salt_factor, tabs_s, dosubgridcloudfraction
...
use vars, only: rho, w, t, tabs, qv, qcl, qci, qpl, qpi, qpg, cld, &
     gamaz, prectot, precsfc, precsfcsnow, precflux, qpfall, tlat, prec_xy, preca_xy, &
...
subroutine micro_setparm()
  ...
  integer :: n
   ...
   NAMELIST /MICRO_THOMPSON/ & ! DEFAULT VALUES in micro_params.f90
      ...
      doeiscld, & ! option for fractional cloudiness of low cloud in radiation
      BrownEtAl2017_pnr_sml_fix, & !option to use backported fix for rain number from melting snow (default=true)
      snow_fall_a, & ! option to change default fall speed relationship
      snow_fall_b, & !   Changing snow_fall_* will require rebuiling lookup tables,
      snow_fall_fv, &!   adding ~1 minute to first simulation when optimization used
      clice_fall_a, &! option to change cloud ice fall speed relationship
      clice_fall_b, &!   Suggested options for snow/clice in MICRO_THOM/micro_params.f90
      snow_fall_a_fudge, clice_fall_a_fudge, & ! scaling factors for these fallspeed relationships
      do_preserve_cloud_ice, & ! Changes to weaken/remove processes that deplete cloud ice completely
      D0i_iau, & ! option to increase cloud ice autoconversion threshold from 20.e-6 m
   ...
   !------------
   if(masterproc) print*,'Microphysics: THOM'
   ...
   if(masterproc) write (*,nml=MICRO_THOMPSON,IOSTAT=ios)
   close(55)
  ...
  if(.not.isallocatedMICRO) then
     ...
     allocate(micro_field(dimx1_s:dimx2_s,dimy1_s:dimy2_s,nzm,nmicro_fields), &
          ...
          precice_xy(nx,ny), preciceflux(nz), &
  ...
  if(douse_reffi) then
     if(TRIM(rad_scheme_name()).eq."cam") then
        if(masterproc) write(*,*) 'Using RAD_CAM with MICRO_THOM'
        if(doThompsonEtAl2016SnowEffRad) then
           if(masterproc) write(*,*) 'Snow/Ice EffRad from Thompson et al (2016)'
        else
           if(masterproc) write(*,*) 'Snow/Ice EffRad based on Fu (1996, JClim)'
        end if
     else
        if(masterproc) write(*,*) 'Using RAD_RRTM with MICRO_THOM'
        if(masterproc) write(*,*) 'Snow/Ice Generalize effective size (Dge) based on Fu (1996, JClim)'
...
subroutine micro_init()
  ...
  if(.NOT.doprecip) then
    ! do not advect/diffuse precipitation variables
    flag_advect(iqr) = 0
    flag_advect(inr) = 0
    flag_advect(iqs) = 0
    flag_advect(iqg) = 0
  ...
  total_micro_3Dout = sum(flag_micro3Dout)
  ...
  if(nrestart.eq.0) then
    ...
    call satadj_liquid(nzm,tabs0,qv0,qc0,pres*100.)
     ...
     do k = 1,nzm
       if(readinit) then
           micro_field(1:nx,1:ny,k,iqv) = qv(1:nx,1:ny,k)
           micro_field(1:nx,1:ny,k,iqcl) = qcl(1:nx,1:ny,k)
       else
      ...
      if(MAXVAL(qc0).gt.0.) then
        ...
        do k = 1,nzm
          ...
          CloudLiquidMassMixingRatio(1:nx,1:ny,k) = micro_field(1:nx,1:ny,k,iqcl)
          if(qc0(k).gt.0.) reffc(1:nx,1:ny,k) = MAX( 2.51, MIN( 50., 1.e6*re_qc0(k) ) )
        end do
      end if
...
!!$       end do
  ...
  if(doicemicro) then
    ...
    mkname(inci) = 'NI'
    mklongname(inci) = 'CLOUD ICE CONCENTRATION'
    mkunits(inci) = '#/kg'
  ...
  if(doaerosols) then
    mkname(incl) = 'NC'
    mklongname(incl) = 'CLOUD WATER CONCENTRATION'
    mkunits(incl) = '#/kg'
...
integer :: i1, i2, j1, j2, i, j, k, m, n, nn, kk
...
!      tmpw = 0.5*(w(i,j,1:nzm) + w(i,j,2:nz))  ! MK: changed for stretched grids 
      ...
      if(doaerosols) then
          ...
          call mp_thompson (qv1d(kk:), qcl1d(kk:), qci1d(kk:), qr1d(kk:), qs1d(kk:), qg1d(kk:), nci1d(kk:), &
               nr1d(kk:), t1d(kk:), p1d(kk:), rho1d(kk:), dzq(kk:), &
               ppt_rain, ppt_snow, ppt_grau, ppt_clice, &
               mtendq1d(kk:,:), mtendn1d(kk:,:), stendq1d(kk:,:), stendn1d(kk:,:), &
               proc_ratesq1d(kk:,:), proc_ratesn1d(kk:,:), &
               1,nzm-kk+1,dtn,1,1,doprecip, &
               do_output_process_rates)
...
!        total_water_prec = total_water_prec + ppt_rain
        ...
        prectot = prectot + wgty(j)*pptncv
        if(dostatis) then
          precsfc(i,j) = precsfc(i,j) + pptncv
          precsfcsnow(i,j) = precsfcsnow(i,j) + ppticencv
        end if
        prec_xy(i,j) = prec_xy(i,j) + pptncv
        preca_xy(i,j) = preca_xy(i,j) + pptncv

        precice_xy(i,j) = precice_xy(i,j) + ppticencv
      else
        ! add rain to cloud
      ...
      if(MAXVAL(qcl1d(:)+qci1d(:)+qs1d(:)).gt.TINY(1.)) then
        ...
        if(reff_ice_holds_Dge) then
          ...
          if(doThompsonEtAl2016SnowEffRad) then
             ! do nothing.  re_qci and re_qs are computed following Thompson et al (2016)
          else
             ! Re-scale of Dge into effective radius following eqn 3.12 in Fu (1996, JClim)
             re_qci1d(:) = (3.*sqrt(3.)/8.)*dge_qci1d(:)
             re_qs1d(:) = (3.*sqrt(3.)/8.)*dge_qs1d(:)
          end if
...
integer :: n, k, j, i
real :: tmp(nzm,nmicro_fields), omc, cld_min
...
if(doicemicro) then
   ...
   qpg(1:nx,1:ny,1:nzm) = micro_field(1:nx,1:ny,1:nzm,iqg) ! graupel
else
  qci(:,:,:) = 0.
  qpi(:,:,:) = 0.
  qpg(:,:,:) = 0.
...
else ! RRTM -- Snow is radiatively active
   where(qcl(1:nx,1:ny,1:nzm) &
        + CloudIceMassMixingRatio(1:nx,1:ny,1:nzm) &

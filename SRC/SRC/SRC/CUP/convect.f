...
C
        SUBROUTINE CONVECT
     &    (T,   Q,    QS,     U,    V,      TRA,    P,    PH,   LV0, 
     &     IPBL, MINORIG, ND,  NL,   NTRA,   DELT, IFLAG,  FT,     FQ,   FU,
     &     FV,  FTRA, PRECIP, WD,   TPRIME, QPRIME, CBMF, QCONDC,
     &     MUP, MDOWN,MP,     RAINFLUX)
C
C-----------------------------------------------------------------------------
C    *** On input:      ***
C
...
C   
C     LV0  Array of specific heat of condensation/deposition - added by MK 2023
C
...
C
C   Extended output to the original scheme for diagnostics and to work together
C   with Sandrine Bony's cloud fraction scheme..
C   Updated by Kerry Emanuel 06/2023
C
C	  QCONDC: The in-cloud mixing ratio of condensed water (kg/kg) ! cld
C				Diagnostic for use in the Bony-Emanuel cloud       ! cld
C				parameterization.                                  ! cld
C
C         RAINFLUX profile of precipitation flux (mm/d)
C
C         MUP in-cloud updraft mass flux ((kg/m**2)/s)
C
C         MDOWN in-cloud downdraft mass flux ((kg/m**2)/s)
C
C         MP precipitation-driven mass downdraft mass
C------------------------------------------------------------------------------
C
...
C
      INTEGER NENT(NA)
      REAL T(ND),Q(ND),QS(ND),U(ND),V(ND),TRA(ND,NTRA),P(ND),PH(ND+1)
      REAL FT(ND),FQ(ND),FU(ND),FV(ND),FTRA(ND,NTRA), LV0(ND)
      ...
      REAL MUP(NA),MDOWN(NA),RAINFLUX(NA)
C
      REAL QCONDC(ND)                                         ! cld
      REAL QCOND(NA),NQCOND(NA),WA(NA),MA(NA),SIGA(NA),AX(NA) ! cld
      real om
C
C -----------------------------------------------------------------------
C
...
C
   ...
   25    CONTINUE
     ...
     1    (TH(JN)*(1.+Q(JN)*EPSI-Q(JN))))THEN
          JN = JN+1
          if(JN.EQ.NL-1) goto 30 ! fix for a problem that JN may become ND sometimes... MK 06/2023
         END IF
         IF(I.EQ.1)JC=JN 
   30   CONTINUE
C
...
C
        CALL TLIFT(P,T,Q,QS,GZ,ICB,NK,TVP,TP,CLW,LV0,ND,NL,1)
        DO 54 I=NK,ICB
...
C
        CALL TLIFT(P,T,Q,QS,GZ,ICB,NK,TVP,TP,CLW,LV0,ND,NL,2)
C
...
C
     ...
     1        CL-CPD))+CPD*(T(I+1)-T(I)))/(LV(I)+T(I)*(CL-CPD))
        ...
        QP(I)=MAX(QP(I),0.0)
C
        RAINFLUX(I)=WT(I)*SIGD*WATER(I)*3600.*24000./(ROWL*G)   ! Downward rainwater flux
C        
  400   CONTINUE
C
...
C
  ...
  410    AM=AM+M(K)
   ...
   ! prevent rare cases when IFLAG=4 by modifying AM - MK 2023
        IF((2.*G*DPINV*AM).GE.DELTI) then
          AM = 0.99*DELTI/(2.*G*DPINV)
        end if
...
C                                                           ! cld
       ...
       DO I=ICB,INB-1                                       ! cld
        AX(I)=0.                                            ! cld
        DO J=ICB,I                                          ! cld
         AX(I)=AX(I)+RD*(TVP(J)-TV(J))*(PH(J)-PH(J+1))/P(J) ! cld
        ENDDO                                               ! cld
        IF (AX(I).GT.0.0) THEN                              ! cld
         WA(I)=SQRT(2.*AX(I))                               ! cld
        ENDIF                                               ! cld
       ENDDO                                                ! cld
       DO I=1,NL                                            ! cld
        IF (WA(I).GT.0.0)                                   ! cld
     1    SIGA(I)=MA(I)/WA(I)*RD*TVP(I)/P(I)/100./DELTA     ! cld
...
C ---------------------------------------------------------------------------
C
        SUBROUTINE TLIFT(P,T,Q,QS,GZ,ICB,NK,TVP,TPK,CLW,LV0,ND,NL,KK)
        REAL GZ(ND),TPK(ND),CLW(ND),P(ND)
        REAL T(ND),Q(ND),QS(ND),TVP(ND),LV0(ND)
...
C
        AH0=(CPD*(1.-Q(NK))+CL*Q(NK))*T(NK)+Q(NK)*(LV0(NK)-CPVMCL*(
     1   T(NK)-273.15))+GZ(NK)
...
C
        ...
        !   ES=6.112*EXP(17.67*TC/DENOM)
           ES = esatw(TG)   ! replace with SAM function - MK
          ELSE  
        !   ES=EXP(23.33086-6111.72784/TG+0.15215*LOG(TG))
           ES = esati(TG)   ! replace with SAM function - MK
          END IF  

...
! (autor is multiplied later by qcc)
           ...
           if(do_scale_dependence_of_autoconv) then
              autor = autor*min(1.,100./(dx*mu(j)))**2
           else
            ! fudge factor similar to one used in IFS Cy47r3 doc)
              autor = autor * max(1.,(0.001*dx*mu(j))**2)
           end if
         else
...
! Evaporation of hydrometeors outside clouds:
    ...
    if(q(i,j,k)-qn(i,j,k).lt.0.) then
      print*,'precip_proc.f90: q(i,j,k)-qn(i,j,k).lt.0.! iflag,rank,i,j,k,landmask,q,qn,qsat,dq,tabs,qp,pres:',&
           iflag,rank,i,j,k,landmask(i,j),q(i,j,k),qn(i,j,k),qsatt(i,j,k),dq,tabs(i,j,k),qp(i,j,k),pres(k)
      call task_abort()
    end if

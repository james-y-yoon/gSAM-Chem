...
!bloss integer, parameter :: tabsize      = 12  ! number of quantities used from lookup table
 integer, parameter :: tabsize      = 13  ! number of quantities used from lookup table
...
!   print*, '               -- ABORTING -- '
 ...
 do jj = 1,densize
    do ii = 1,rimsize
       do i = 1,isize
         !bloss: Read lambda_ice in from lookup table.  Place in itab(:,:,:,13).
         !bloss: Corresponding write statement is on line 1401 of create_p3_lookupTable_1.f90
          read(10,*) dum,dum,dum,dum,itab(jj,ii,i,1),itab(jj,ii,i,2),           &
               itab(jj,ii,i,3),itab(jj,ii,i,4),itab(jj,ii,i,5),                 &
               itab(jj,ii,i,6),itab(jj,ii,i,7),itab(jj,ii,i,8), itab(jj,ii,i,13), & !bloss: lambda --> itab(:,:,:,13)
...
!==========================================================================================!
 ...
 SUBROUTINE p3_main(qc,nc,qr,nr,th_old,th,qv_old,qv,dt,qitot,qirim,nitot,birim,ssat,uzpl, &
             ...
             !       rho, pgam_out, lamc_out, lami_out, mtend, stend, & !bloss/SAM: input rho for consistency with SAM
...
! real, intent(out), dimension(its:ite,kts:kte,nCat) :: lami_out    ! slope parameter for ice
 real, dimension(its:ite,kts:kte,nCat) :: lami_out    ! slope parameter for ice

 !Guangxing Lin add SEDIMENTATION TENDENCIES FOR MIXING RATIO
 real, intent(inout), dimension(its:ite,kts:kte,8+4*nCat) :: mtend    ! microphysical tendencies for all microphysical fields (e.g., kg/kg/s)
 real, intent(inout), dimension(its:ite,kts:kte,8+4*nCat) :: stend    ! sedimentation tendencies for all microphysical fields (e.g., kg/kg/s)
...
!  (all N process rates in # kg-1)
 ...
 real, dimension(nCat) :: qcrfrz    ! freezing of cloud droplets (qinuc + qchetc + qcheti) and rain (qrhti+qrhetc) !BG
 real, dimension(nCat) :: ncrfrz    ! number freezing of cloud droplets (qinuc + qchetc + qcheti) and rain (qrhti+qrhetc) !BG
 !SAM additions end
...
! to be added as namelist parameters (future)
 ...
 !SAM additions begin
 integer  :: idx !BG: index for process rate array
 !SAM additions end
...
!------------------------------------------------------------------------------------------!
 ...
 mtend(:,:,iqr)  = qr(:,:)
 mtend(:,:,inr)  = nr(:,:)
 !bloss: Only works for 1 category
 mtend(:,:,iqit) = qitot(:,:,1) 
 mtend(:,:,iqir) = qirim(:,:,1)
 mtend(:,:,iqib) = birim(:,:,1) !bloss(2020-12-10): fix typo (qitot-->birim)
 mtend(:,:,inci) = nitot(:,:,1)
     ...
     ! initialize inverse supersaturation relaxation timescale for combined ice categories
       ...
       iice_loop1: do iice = 1,nCat
          ...
          ! call to lookup table interpolation subroutines to get process rates
             ...
             !bloss: Get lambda_ice out of lookup table
             call access_lookup_table(dumjj,dumii,dumi,13,dum1,dum4,dum5,lami_out(i,k,iice))
...
! this is not applied at the first time step, since saturation adjustment is applied at the first step
       ...
       if (.not.(log_predictNc).and.sup_cld.gt.1.e-6.and.it.gt.1) then
          ...
          qcnuc = max(0., dum*odt*SCF(k) ) !bloss(2021-01): Bug fix to prevent qcnuc<0
       endif
...
qcrfrz = qinuc+qchetc+qcheti+qrhetc+qrheti +qchomi +qrhomi!BG sum of all freezing of cloud drops and rain [mass]
ncrfrz = ninuc+nchetc+ncheti+nrhetc+nrheti +nchomi +nrhomi!BG sum of all freezing of cloud drops and rain [number]
...
!BG add accumulation of microphysical process rates here [in main loop as rates are 1dim or in iice function]
 if (do_accumulate_micro_proc_rates) then
    ...
    !idx = idx + 1
    !micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(qccol(1:nCat)) ! collection of cloud water by ice [kg/kg/s]
    !idx = idx + 1
    !micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(qwgrth(1:nCat)) ! wet growth rate [kg/kg/s]
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(qidep(1:nCat)) ! vapor deposition on ice [kg/kg/s]
    !idx = idx + 1
    !micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(qrcol(1:nCat)) ! collection rain mass by ice [kg/kg/s]
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(qisub(1:nCat)) ! sublimation of ice [kg/kg/s]
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(qimlt(1:nCat)) ! melting of ice [kg/kg/s]
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(qcrfrz(1:nCat)) ! freezing of cloud droplets (qinuc + qchetc + qcheti) and rain (qrhti+qrhetc) !BG
    !number rates
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(nimlt(1:nCat)) ! melting of ice [#/kg]
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(nisub(1:nCat)) ! sublimation of ice [#/kg]
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(nislf(1:nCat)) ! change in ice number from collection within a category [#/kg]
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(ncrfrz(1:nCat)) ! number freezing of cloud droplets (qinuc + qchetc + qcheti) and rain (qrhti+qrhetc) !BG
endif
!SAM additions end
...
!bloss             qitsten(i,k) = qitsten(i,k) + fluxdiv_qit*inv_rho(i,k) !kg/kg/s
             stend(i,k,iqit) = stend(i,k,iqit) + fluxdiv_qit*inv_rho(i,k)*dt_sub/dt !kg/kg/s
             stend(i,k,iqir) = stend(i,k,iqir) + fluxdiv_qir*inv_rho(i,k)*dt_sub/dt !kg/kg/s
             stend(i,k,iqib) = stend(i,k,iqib) + fluxdiv_bir*inv_rho(i,k)*dt_sub/dt !m3/kg/s
             stend(i,k,inci) = stend(i,k,inci) + fluxdiv_nit*inv_rho(i,k)*dt_sub/dt !#/kg/s
...
! homogeneous freezing of cloud and rain
    ...
    ! compute mean-mass ice diameters (estimated; rigorous approach to be implemented later)
       ...
       if (qc(i,k).ge.qsmall .and. t(i,k).lt.233.15) then
         ...
         !homfrz term? 
          qchomi(iice_dest) = Q_nuc
          nchomi(iice_dest) = N_nuc
...
!.....................................................
  ...
  !bloss: Only works for nCat==1
  mtend(:,:,iqit) = ( qitot(:,:,1) - mtend(:,:,iqit) )*odt ! assume nCat==1
  mtend(:,:,iqir) = ( qirim(:,:,1) - mtend(:,:,iqir) )*odt
  mtend(:,:,iqib) = ( birim(:,:,1) - mtend(:,:,iqib) )*odt
  mtend(:,:,inci) = ( nitot(:,:,1) - mtend(:,:,inci) )*odt

...
use grid, only: nx, ny, nz, nzm, & ! grid dimensions; nzm=nz-1 - # of levels for all scalars
                ...
                masterproc, dostatis, &  !BG a la Bloss
...
!BG a la' Bloss
   NAMELIST /MICRO_P3/ &
      ...
      do_output_micro_process_rates, doeiscld, do_scale_dependence_of_autoconv, &
      doallice_radiatively_active, icefall_fudge
...
!bloss  compute_reffl = .false.
  ...
  flag_micro3Dout(inci) = 1
  flag_micro3Dout(iqcl) = 1
  flag_micro3Dout(iqir) = 1
  flag_micro3Dout(iqib) = 1
...
!BG end
   ...
   do j = 1, ny
     ...
     tmpqv_old(1:nx, 1:nzm) = micro_field(1:nx,j,1:nzm,iqv_old)  !water vapor from last time step
    ...
    do k = 1,nzm
      do i = 1, nx
        ...
        tmpth_old(i, k) = micro_field(i,j,k,itabs_old)*(1.e+5/(pres(k)*100.0))**0.286 !note pres is mb
     ...
     ! note: code for prediction of ssat not currently avaiable, set 2D array to 0
      ...
      do i=1,nx
        ...
        call p3_main( qc=tmpqc(i:i,kk:), nc=tmpnc(i:i,kk:), qr=tmpqr(i:i,kk:), &
              ...
              diag_ze=diag_zdbz(i:i,kk:), diag_effc=diag_effc(i:i,kk:), diag_effi=diag_effi(i:i,kk:), &
...
!bloss     cloudliq(1:nx,j,1:nzm) = tmpqc(1:nx,1:nzm)
     ...
     qci(1:nx,j,1:nzm) = qi_type(1:nx,1:nzm,1) 
     qpi(1:nx,j,1:nzm) = qi_type(1:nx,1:nzm,2)+qi_type(1:nx,1:nzm,3)+qi_type(1:nx,1:nzm,4)+ &
                         qi_type(1:nx,1:nzm,5)+qi_type(1:nx,1:nzm,6) 
     qpg(1:nx,j,1:nzm) = qi_type(1:nx,1:nzm,4)+qi_type(1:nx,1:nzm,5)+qi_type(1:nx,1:nzm,6)
     qhail(1:nx,j,1:nzm) = qi_type(1:nx,1:nzm,5)
     qpel(1:nx,j,1:nzm) = qi_type(1:nx,1:nzm,6)
     ...
     if(reff_ice_holds_Dge) then
       ...
       reffi(1:nx,j,1:nzm) = reffi(1:nx,j,1:nzm)*1.5396  ! correct based on Qiang Fu (1996)
...
!bloss     CloudLiquidLambda(1:nx,j,1:nzm) = tmp_cl_lambda(1:nx,1:nzm)
     ...
     precinst(1:nx, j) = precinst(1:nx,j) + pcprt_liq(1:nx)*1000.0 &
                                +  pcprt_sol(1:nx)*1000.0 
     prectot = prectot + wgty(j)*sum(pcprt_liq(1:nx)+pcprt_sol(1:nx))*1000.0*dtn !m/s-->mm/s->mm
     if(dostatis) then
      precsfc(1:nx, j) = precsfc(1:nx,j) + pcprt_liq(1:nx)*1000.0*dtn & !m/s-->mm/s->mm
                                +  pcprt_sol(1:nx)*1000.0*dtn !m/s-->mm/s->mm
      precsfcsnow(1:nx, j) = precsfcsnow(1:nx,j) + pcprt_sol(1:nx)*1000.0*dtn !m/s-->mm/s->mm
     end if
     prec_xy(1:nx, j) = prec_xy(1:nx,j) + pcprt_liq(1:nx)*1000.0*dtn & !m/s-->mm/s->mm
                                +  pcprt_sol(1:nx)*1000.0*dtn !m/s-->mm/s->mm
     preca_xy(1:nx, j) = preca_xy(1:nx,j) + pcprt_liq(1:nx)*1000.0*dtn/dz & !m/s-->mm/s->mm/dz
                                +  pcprt_sol(1:nx)*1000.0*dtn !m/s-->mm/s->mm
     ...
     if(dostatis) then
       ...
       do i=1,nx
         ...
         do k = 1,nzm    !BG diag_effc/effi is in meters!
          ...
          qxoeffr(k,iqcl) = qxoeffr(k,iqcl) + tmpqc(i,k)/(1.e-20+ diag_effc(i,k))
          ...
          !if(doicemicro) then
             if((tmpqit(i,k,1).gt.1.e-6).AND.(diag_effi(i,k).gt.1.e-6)) then
               ...
               qxoeffr(k,iqit) = qxoeffr(k,iqit) + tmpqit(i,k,1)/(1.e-20+ diag_effi(i,k))
             end if

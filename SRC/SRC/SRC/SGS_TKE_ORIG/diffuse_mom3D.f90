...
real rdx2u,rdx2v,rdx2w,rdy2u,rdy2v,rdy2w

!-----------------------------------------

call t_startf('diffuse_mom3D1')
...
do k=1,nzm
  ...
  do j=1,ny
   jb=j-1
   rdx2u=rdx2*imu(j)**2 *grdf_x(j,k)
   rdx2v=rdx2*imuv(j)**2 *grdfv_x(j,k)
   rdx2w=rdx2*imu(j)**2 *grdfw_x(j,kcu)
   do i=0,nx
    ic=i+1
   ...
   do i=1,nx
    ib=i-1
    dudt(i,j,k,na)=dudt(i,j,k,na)-(fux(i)-fux(ib))
    dvdt(i,j,k,na)=dvdt(i,j,k,na)-(fvx(i)-fvx(ib))
    dwdt(i,j,kc,na)=dwdt(i,j,kc,na)-(fwx(i)-fwx(ib))
   end do  
  end do 
end do
...
do k=1,nzm
  ...
  do j=0,ny
   jc=j+1
   rdy2u=rdy2/adyv(jc)*muv(jc) *grdfu_y(j,k)
   rdy2v=rdy2/ady(j)*mu(j) *grdfv_y(j,k)
   rdy2w=rdy2/adyv(jc)*muv(jc) *grdfw_y(j,kcu)
   do i=1,nx
    ib=i-1
  ...
  do j=1,ny
    ...
    rdy2w=1./(ady(j)*mu(j))
    do i=1,nx    
     dudt(i,j,k,na)=dudt(i,j,k,na)-rdy2u*(fuy(i,j)-fuy(i,jb))
     dvdt(i,j,k,na)=dvdt(i,j,k,na)-rdy2v*(fvy(i,j)-fvy(i,jb))
     dwdt(i,j,kc,na)=dwdt(i,j,kc,na)-rdy2w*(fwy(i,j)-fwy(i,jb))
   end do 
  end do 
end do 

call t_stopf('diffuse_mom3D1')

!-------------------------

if(doimplicitdiff) goto 333

call t_startf('diffuse_mom3D2')
...
if (dostatis) then
 ...
 do k=2,nzm
  ...
  do j=1,ny
   do i=1,nx 
    dwdt(i,j,k,na)=dwdt(i,j,k,na)-(fwz(i,j,k+1)-fwz(i,j,k))*rhoi
   end do
  end do
 end do ! k
...
if(collect_coars) then
  ...
  do k=1,nzm
   ...
   do j=1,ny
    do i=1,nx
      misc1(i,j,kc)=min(tkmax(j,k),tk(i,j,k))
    end do
   end do
  end do

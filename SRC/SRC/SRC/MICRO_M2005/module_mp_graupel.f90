...
! FALLSPEED PARAMETERS (V=AD^B)
         AI = clice_fall_a*clice_fall_a_fudge !700.
         AC = 3.E7
         AS = snow_fall_a*snow_fall_a_fudge !11.72
         AR = 841.99667
         BI =  clice_fall_b ! 0.865 ! 1. MK
         BC = 2.
         BS = snow_fall_b ! 0.41
         BR = 0.8
! V1.3
...
! V1.3
	 ...
	 RHOG = 400.
         ...
         ECR = 1.
     !    DCS = 125.E-6
         DCS = clice_snow_Dauto ! 250.E-6  ! increase threshold - MK
...
!v1.4
         IF (QNI3D(K).GE.0.1E-20) THEN
         IF (QC3D(K).GE.0.5E-20.OR.QR3D(K).GE.0.1E-20) THEN
...
!         IF (QI3D(K).GE.1.E-8 .AND.QVQVSI(K).GE.1.) THEN
         IF (QI3D(K).GE.1.E-9 .AND.QVQVSI(K).GE.1.) THEN  ! MK
...
! fix for correction by AA 4/6/11
        ...
        UMR=MIN(UMR,9.1*dum)
        UNR=MIN(UNR,9.1*dum)
!  Change the maximum velocity of graupel from more like-hail value - MK
        if(IGRAUP.eq.1) then
         UMG=MIN(UMG,10.*dum)
         UNG=MIN(UNG,10.*dum)
        else

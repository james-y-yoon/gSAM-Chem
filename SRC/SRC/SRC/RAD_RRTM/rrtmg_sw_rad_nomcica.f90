...
!------------------------------------------------------------------
      ...
      subroutine rrtmg_sw &
            (ncol    ,nlay    ,icld    , &
             ...
             dirdnuv ,difdnuv ,dirdnir ,difdnir )
...
! ----- Output -----
      ...
      real(kind=rb), intent(out) :: swhrc(:,:)        ! Clear sky shortwave radiative heating rate (K/d)
                                                      !    Dimensions: (ncol,nlay)
! added for SLM model (MK 2023)
      real(kind=rb), intent(out) :: dirdnuv(:,:)      ! Direct downward shortwave flux, UV/vis
      real(kind=rb), intent(out) :: difdnuv(:,:)      ! Diffuse downward shortwave flux, UV/vis
      real(kind=rb), intent(out) :: dirdnir(:,:)      ! Direct downward shortwave flux, near-IR
      real(kind=rb), intent(out) :: difdnir(:,:)      ! Diffuse downward shortwave flux, near-IR
                                                      ! Dimensions: (ncol,nlay)
...
! Modify to loop over all columns (nlon) or over daylight columns
      ...
      do iplon = 1, ncol
         ...
         if (coszen(iplon) .lt. zepzen) then  ! skip columns with no sun - MK
           swuflx(iplon,:) = 0.
           swdflx(iplon,:) = 0. 
           swuflxc(iplon,:) = 0.
           swdflxc(iplon,:) = 0.

...
!
   a = adzw(nzm)/adzw(nzm-1)
   b = adz(nzm-1)*rho(nzm-1)/(adz(nzm-1)*rho(nzm-1))
   c = 1./(1.+ b + a)
   do j=1,ny
    do i=1,nx
      t(i,j,nzm-1)=(t(i,j,nzm)+b*t(i,j,nzm-1)+a*t(i,j,nzm-2))*c
      t(i,j,nzm)=(1.+a)*t(i,j,nzm-1)-a*t(i,j,nzm-2)
    end do    
   end do 

...
! end if
 ...
 do j=1,ny
  ...
  do i=0,nx
    ...
    tkx=min(tkmax(j,k),tkx)
    flx(i,j,k)=-rdx5*tkx*(field(ic,j,k)-field(i,j,k))
  end do 
  do i=1,nx
    ib=i-1
 ...
 do j=0,ny
  jc=j+1
  rdy5 = rdy2/adyv(jc)*muv(jc)         
  do i=1,nx
   tky=max(tk0,0.5*(tkh(i,j,k)+tkh(i,jc,k)))
   tky=min(tkmax(j,k),tky)
   flx(i,j,k)=-rdy5*tky*(field(i,jc,k)-field(i,j,k))
  end do 
 end do
 do j=1,ny
  jb=j-1
...
do k=1,nzm-1
 ...
 do j=1,ny
  do i=1,nx
    tkz=min(tkmax(j,k),0.5*(tkh(i,j,k)+tkh(i,j,kc)))  
    flx(i,j,k)=-rdz5*tkz*(field(i,j,kc)-field(i,j,k))  
...
do j=1,ny
 do i=1,nx	
   ...
   flux(1) = flux(1) + flx(i,j,0)*wgtw(j,1)
 ...
 do j=1,ny
  do i=1,nx		 
   field(i,j,k)=field(i,j,k)+ &
          dtn*(dfdt(i,j,k)-(flx(i,j,k)-flx(i,j,kb))*rhoi)
  end do 
 end do 	 
end do 

...
!bloss/SAM begin
use micro_params !BG
use params, only: cp_SAM => cp, &
     rd_SAM => rgas, &
     rv_SAM => rv, &
     g_SAM => ggr, &
     lcond_SAM => lcond, &
     lfus_SAM => lfus
use grid, only: masterproc, dompi,nstep,rank
!bloss/SAM end
 implicit none
 ...
 integer :: itmp(1) !bloss: for clean MPI broadcast
...
!bloss integer, parameter :: tabsize      = 12  ! number of quantities used from lookup table
 integer:: tabsize      = 12  ! number of quantities used from lookup table !bloss(SplitIceOptics)
 ...
 !bloss real, dimension(densize,rimsize,isize,tabsize) :: itab   !ice lookup table values
 real, allocatable, dimension(:,:,:,:) :: itab   !ice lookup table values !bloss(SplitIceOptics)
...
! Local variables and parameters:
 ...
 character(len=16) :: version_intended_table_1 = '4.1'   !lookupTable_1 version intended for this P3 version
...
!lookup_file_2 = 'MY_LOCAL_PATH/p3_lookup_table_2.dat-v'//trim(version_intended_table_2)
 ...
 if(doSplitIceOptics) then
    lookup_file_1 = trim(lookup_file_1)//'-SplitIceOpt'
    version_intended_table_1 = '4.1-SplitIceOpt'
    tabsize=tabsize+6
    write(*,*) 'Option for Split Ice Optics enabled in MICRO_P3BLAZ/module_mp_p3.f90'
    write(*,*) 'Reading lookup file ', TRIM(lookup_file_1)
 end if
 ALLOCATE(itab(densize,rimsize,isize,tabsize), STAT=istat)
 if(istat.ne.0) call task_abort_msg('Could not allocate itab in p3_init in MICRO_P3BLAZ/module_mp_p3.f90')
...
!bloss/SAM: Note that some parameters will be set from variables in the
!  MICRO_P3 namelist.  The default values are the same as those used
!  here.

! maximum total ice concentration (sum of all categories)
 max_total_Ni = MaxTotalIceNumber !bloss 2000.e+3 !about 2/cm3  !(m)
 ndust        = NumCirrusINP  !BG apr 2020
 nsulf        = NumCirrusSulf !BG apr 2020   

! switch for warm-rain parameterization
! = 1 Seifert and Beheng 2001
! = 2 Beheng 1994
! = 3 Khairoutdinov and Kogan 2000
 iparam = iWarmRainScheme !bloss: 3 (Default = KK2000)

! droplet concentration (m-3)
!bloss nccnst = 200.e+6
 nccnst = 1.e6*Nc0 !bloss: Nc0 is an option in the MICRO_P3 namelist
...
! physical constants
 cp     = cp_SAM !bloss 1005.
 inv_cp = 1./cp
 g      = g_SAM !bloss 9.816
 rd     = rd_SAM !bloss 287.15
 rv     = rv_SAM !bloss 461.51
 ep_2   = 0.622
...
! Barklie and Gokhale (1959)
 ...
 aimm   = 0.65
 rin    = 0.1e-6
 mi0    = 4.*piov3*900.*(IceNucleiRadius)**3 !bloss 1.e-18
...
! aerosol/droplet activation parameters
 ...
 vi     = 3.
 epsm   = 0.9
 rhoa   = 1777. !bloss: This seems to be sulfate aerosol
 map    = 0.132
 ma     = 0.0284
 rr     = 8.3187
...
! mode 1
 inv_rm1 = 1./aerosol_mode1_radius !bloss 2.e+7           ! inverse aerosol mean size (m-1)
 sig1    = aerosol_mode1_sigmag !bloss 2.0             ! aerosol standard deviation
 nanew1  = aerosol_mode1_number !bloss 300.e6          ! aerosol number mixing ratio (kg-1)
...
! mode 2
 inv_rm2 = 1./aerosol_mode2_radius !bloss 7.6923076e+5    ! inverse aerosol mean size (m-1)
 sig2    = aerosol_mode2_sigmag !bloss 2.5             ! aerosol standard deviation
 nanew2  = aerosol_mode2_number !bloss 0.             ! aerosol number mixing ratio (kg-1)
...
! read in ice microphysics table
 ...
 procnum = -1
 if(masterproc) procnum = 0 !bloss/SAM
!SAM#ifdef ECCCGEM
!SAM call rpn_comm_rank(RPN_COMM_GRID,procnum,istat)
 itab = 0.
 itabcoll = 0.D0
 itabcolli1 = 0.D0
 itabcolli2 = 0.D0
    ...
    do ii = 1,rimsize
       ...
       do i = 1,isize
          read(10,*) dum,dum,dum,dum,itab(jj,ii,i,1),itab(jj,ii,i,2),           &
               ...
               itab(jj,ii,i,15),itab(jj,ii,i,16),itab(jj,ii,i,17),               &
               itab(jj,ii,i,18)
        enddo
     else
               do i = 1,isize
...
!SAM#ifdef ECCCGEM
!SAM call rpn_comm_bcast(global_status,1,RPN_COMM_INTEGER,0,RPN_COMM_GRID,istat)
!SAM#endif

if(dompi) then
  if(masterproc) itmp(1) = global_status
...
!SAM#ifdef ECCCGEM
!SAM call rpn_comm_bcast(itab,size(itab),RPN_COMM_REAL,0,RPN_COMM_GRID,istat)
!SAM call rpn_comm_bcast(itabcoll,size(itabcoll),RPN_COMM_DOUBLE_PRECISION,0,RPN_COMM_GRID,istat)
!SAM#endif

if(dompi) then
   call task_bcast_fourdim_array_real(0,itab,densize,rimsize,isize,tabsize)
   call task_bcast_fivedim_array_real8(0,itabcoll,densize,rimsize,isize,rcollsize,colltabsize)
end if
...
!SAM#ifdef ECCCGEM
!SAM   call rpn_comm_bcast(itabcolli1,size(itabcolli1),RPN_COMM_DOUBLE_PRECISION,0,RPN_COMM_GRID,istat)
!SAM   call rpn_comm_bcast(itabcolli2,size(itabcolli2),RPN_COMM_DOUBLE_PRECISION,0,RPN_COMM_GRID,istat)
!SAM#endif

   if(dompi) then
     call task_bcast_sixdim_array_real8(0,itabcolli1,iisize,rimsize,densize,iisize,rimsize,densize)
     call task_bcast_sixdim_array_real8(0,itabcolli2,iisize,rimsize,densize,iisize,rimsize,densize)
   end if
...
! variables for cloud fraction (currently not used with WRF)
   ...
   do j = jts,jte      ! j loop (north-south)
     ...
     ! note: code for prediction of ssat not currently avaiable, set 2D array to 0
      ...
      STOP 'Call to P3_main is disabled in SAM interface'

!bloss/SAM       call P3_MAIN(qc_3d(its:ite,kts:kte,j),nc(its:ite,kts:kte),                                       &
!bloss/SAM               qr_3d(its:ite,kts:kte,j),qnr_3d(its:ite,kts:kte,j),                                      &
!bloss/SAM               th_old_3d(its:ite,kts:kte,j),th_3d(its:ite,kts:kte,j),qv_old_3d(its:ite,kts:kte,j),      &
!bloss/SAM               qv_3d(its:ite,kts:kte,j),dt,qi1_3d(its:ite,kts:kte,j),                                   &
!bloss/SAM               qir1_3d(its:ite,kts:kte,j),qni1_3d(its:ite,kts:kte,j),                                   &
!bloss/SAM               qib1_3d(its:ite,kts:kte,j),ssat(its:ite,kts:kte),                                        &
!bloss/SAM               W(its:ite,kts:kte,j),P(its:ite,kts:kte,j),                                               &
!bloss/SAM               DZ(its:ite,kts:kte,j),itimestep,pcprt_liq,pcprt_sol,its,ite,kts,kte,n_iceCat,            &
!bloss/SAM               diag_zdbz_3d(its:ite,kts:kte,j),diag_effc_3d(its:ite,kts:kte,j),                         &
!bloss/SAM               diag_effi_3d(its:ite,kts:kte,j),diag_vmi_3d(its:ite,kts:kte,j),                          &
!bloss/SAM               diag_di_3d(its:ite,kts:kte,j),diag_rhopo_3d(its:ite,kts:kte,j),                          &
!bloss/SAM               n_diag_2d,diag_2d(its:ite,1:n_diag_2d),                                                  &
!bloss/SAM               n_diag_3d,diag_3d(its:ite,kts:kte,1:n_diag_3d),                                          &
!bloss/SAM               log_predictNc,typeDiags_ON,trim(model),clbfact_dep,clbfact_sub,debug_on,                 &
!bloss/SAM               scpf_on,scpf_pfrac,scpf_resfact,cldfrac)
...
!bloss/SAM               qr_3d(its:ite,kts:kte,j),qnr_3d(its:ite,kts:kte,j),                                  &
!bloss/SAM               th_old_3d(its:ite,kts:kte,j),th_3d(its:ite,kts:kte,j),qv_old_3d(its:ite,kts:kte,j),  &
!bloss/SAM               qv_3d(its:ite,kts:kte,j),dt,qitot(its:ite,kts:kte,1:n_iceCat),                       &
!bloss/SAM               qirim(its:ite,kts:kte,1:n_iceCat),nitot(its:ite,kts:kte,1:n_iceCat),                 &
!bloss/SAM               birim(its:ite,kts:kte,1:n_iceCat),ssat(its:ite,kts:kte),                             &
!bloss/SAM               W(its:ite,kts:kte,j),P(its:ite,kts:kte,j),                                           &
!bloss/SAM               DZ(its:ite,kts:kte,j),itimestep,pcprt_liq,pcprt_sol,its,ite,kts,kte,n_iceCat,        &
!bloss/SAM               diag_zdbz_3d(its:ite,kts:kte,j),diag_effc_3d(its:ite,kts:kte,j),                     &
!bloss/SAM               diag_effi(its:ite,kts:kte,1:n_iceCat),diag_vmi(its:ite,kts:kte,1:n_iceCat),          &
!bloss/SAM               diag_di(its:ite,kts:kte,1:n_iceCat),diag_rhopo(its:ite,kts:kte,1:n_iceCat),          &
!bloss/SAM               n_diag_2d,diag_2d(its:ite,1:n_diag_2d),                                              &
!bloss/SAM               n_diag_3d,diag_3d(its:ite,kts:kte,1:n_diag_3d),                                      &
!bloss/SAM               log_predictNc,typeDiags_ON,trim(model),clbfact_dep,clbfact_sub,debug_on,             &
!bloss/SAM               scpf_on,scpf_pfrac,scpf_resfact,cldfrac)
  ...
  !--- substepping microphysics
   ...
   do i_substep = 1, n_substep
     ...
     !convert to potential temperature:
      ...
      !bloss/SAM: define these variables to get rid of warnings
      diag_vis = 0.
      diag_vis1 = 0.
      diag_vis2 = 0.
      diag_vis3 = 0.
...
!bloss/SAM         call p3_main(qc,nc,qr,nr,theta_m,theta,qvap_m,qvap,dt_mp,qitot_1(:,:),qirim_1(:,:),  &
!bloss/SAM                   nitot_1(:,:),birim_1(:,:),ssat,ww,pres,DZ,kount,prt_liq,prt_sol,i_strt,ni, &
!bloss/SAM                   k_strt,nk,n_iceCat,diag_Zet,diag_effc,diag_effi_1(:,:),diag_vmi,diag_di,   &
!bloss/SAM                   diag_rhoi,n_diag_2d,diag_2d,n_diag_3d,diag_3d,log_predictNc,typeDiags_ON,  &
!bloss/SAM                   trim(model),clbfact_dep,clbfact_sub,debug_on,scpf_on,scpf_pfrac,           &
!bloss/SAM                   scpf_resfact,cldfrac,prt_drzl,prt_rain,prt_crys,prt_snow,prt_grpl,         &
!bloss/SAM                   prt_pell,prt_hail,prt_sndp,qi_type,diag_vis,diag_vis1,diag_vis2,diag_vis3)

      else
        !general (nCat >= 1):
!bloss/SAM         call p3_main(qc,nc,qr,nr,theta_m,theta,qvap_m,qvap,dt_mp,qitot,qirim,nitot,birim,    &
!bloss/SAM                   ssat,ww,pres,DZ,kount,prt_liq,prt_sol,i_strt,ni,k_strt,nk,n_iceCat,        &
!bloss/SAM                   diag_Zet,diag_effc,diag_effi,diag_vmi,diag_di,diag_rhoi,n_diag_2d,diag_2d, &
!bloss/SAM                   n_diag_3d,diag_3d,log_predictNc,typeDiags_ON,trim(model),clbfact_dep,      &
!bloss/SAM                   clbfact_sub,debug_on,scpf_on,scpf_pfrac,scpf_resfact,cldfrac,prt_drzl,     &
!bloss/SAM                   prt_rain,prt_crys,prt_snow,prt_grpl,prt_pell,prt_hail,prt_sndp,qi_type,    &
!bloss/SAM                   diag_vis,diag_vis1,diag_vis2,diag_vis3)
      endif
...
!==========================================================================================!
 ...
 SUBROUTINE p3_main(qc,nc,qr,nr,th_old,th,qv_old,qv,dt,qitot,qirim,nitot,birim,ssat,uzpl, &
                    ...
                    diag_qice_clice, diag_Dge_clice, & !bloss(SplitIceOptics)
                    diag_qice_snow,  diag_Dge_snow,  & !bloss(SplitIceOptics)
                    diag_qice_graup, diag_Dge_graup, & !bloss(SplitIceOptics)
                    ...
                    rho, pgam_out, lamc_out, mtend, stend, & !bloss/SAM: input rho for consistency with SAM
		    micro_proc_rates,do_accumulate_micro_proc_rates) !BG/bloss/SAM: Add tendency output
...
! note: Nc may be specified or predicted (set by log_predictNc)
 ...
 real, intent(in),    dimension(its:ite,kts:kte)      :: rho        ! density                          kg m-3 !bloss/SAM: Make this an input
 ...
 !bloss(SplitIceOptics): cloud ice = all ice w/D<150um,
 !                       snow = all ice with 150um < D < 500um AND rime fraction < 0.5
 !                       graup = everything else
 !  Unlike the diagnostic types below, most grid points with ice will contain all three
 !    of these "types" of ice.  There are two goals of this approach:
 !      - Simplify the radiation/microphysics coupling when using nCat>1 by combining
 !         all small ice, aggregates and graupel together and return the qice and Dge of each.
 !      - Allow for tuning of ice radiative effects by including/excluding different types of ice
 !         and by changing the definition of the categories (though the latter requires rebuilding the lookup table).
 real, intent(out),   dimension(its:ite,kts:kte)      :: diag_qice_clice  ! ice water mass mixing ratio of "cloud ice", kg kg-1
 real, intent(out),   dimension(its:ite,kts:kte)      :: diag_qice_snow  ! ice water mass mixing ratio of "snow", kg kg-1
 real, intent(out),   dimension(its:ite,kts:kte)      :: diag_qice_graup  ! ice water mass mixing ratio of "graup", kg kg-1
 real, intent(out),   dimension(its:ite,kts:kte)      :: diag_Dge_clice  ! ice water mass mixing ratio of "cloud ice", m
 real, intent(out),   dimension(its:ite,kts:kte)      :: diag_Dge_snow  ! ice water mass mixing ratio of "snow", m
 real, intent(out),   dimension(its:ite,kts:kte)      :: diag_Dge_graup  ! ice water mass mixing ratio of "graup",m 
 !bloss(SplitIceOptics)
 ...
 !SAM additions begin
 !Guangxing Lin add parmeter output for radiation calculations
 real, intent(out), dimension(its:ite,kts:kte)      :: pgam_out    ! spectral shape parameter for droplets
 real, intent(out), dimension(its:ite,kts:kte)      :: lamc_out    ! slope parameter for droplets

 !Guangxing Lin add SEDIMENTATION TENDENCIES FOR MIXING RATIO
 real, intent(inout), dimension(its:ite,kts:kte,7+4*nCat) :: mtend    ! microphysical tendencies for all microphysical fields (e.g., kg/kg/s)
 real, intent(inout), dimension(its:ite,kts:kte,7+4*nCat) :: stend    ! sedimentation tendencies for all microphysical fields (e.g., kg/kg/s)

 !BG process rates for output
 real, intent(inout), dimension(its:ite,kts:kte, nmicro_proc) :: micro_proc_rates !BG
 logical, intent(in)  :: do_accumulate_micro_proc_rates
 !SAM additions end
...
!----- Local variables and parameters:  -------------------------------------------------!
 ...
 real, dimension(its:ite,kts:kte) :: tc    ! temperature in celsius !BG, for not transforming a few times
...
!  (all N process rates in # kg-1)
 ...
 real, dimension(nCat) :: qinuc2    !BG cirrus deposition freezing of dust nuc
 real, dimension(nCat) :: qinuc3    !BG hom freezing of sulfate solution droplets nuc
 ...
 real, dimension(nCat) :: ninuc2    !BG cirrus deposition freezing of dust nuc, change in ice num
 real, dimension(nCat) :: ninuc3    !BG hom freezing of sulfate solution droplets, change in ice num
 ...
 !SAM additions begin
 real, dimension(nCat) :: qchomi    ! homogeneous freezing (cloud droplets) !BG added as separate tendency
 real, dimension(nCat) :: nchomi    ! homogeneous freezing (cloud droplets) !BG added as separate tendency
 real, dimension(nCat) :: qrhomi    ! homogeneous freezing (rain) !BG added as separate tendency
 real, dimension(nCat) :: nrhomi    ! homogeneous freezing (rain) !BG added as separate tendency
 real, dimension(nCat) :: qcrfrz_het !renamed by adding _het !freezing of cloud droplets (qinuc + qchetc + qcheti) and rain (qrhti+qrhetc) !BG
 real, dimension(nCat) :: ncrfrz_het !renamed by adding _het !number freezing of cloud droplets (qinuc + qchetc + qcheti) and rain (qrhti+qrhetc) !BG
 real, dimension(nCat) :: qcrfrz_hom   !BG homogeneous freezing of rain and cloud droplets
 real, dimension(nCat) :: ncrfrz_hom   !BG homogeneous freezing of rain and cloud droplets
 real, dimension(nCat) :: qcrfrz   !BG  freezing of rain and cloud droplets
 real, dimension(nCat) :: ncrfrz   !BG  freezing of rain and cloud droplets
 !SAM additions end
 ...
 real, dimension(its:ite,kts:kte)      :: inv_dzq,inv_rho,ze_ice,ze_rain,prec,           & !bloss: rho,       &
            rhofacr,rhofaci,acn,xxls,xxlv,xlf,qvs,qvi,sup,supi,ss,vtrmi1,vtrnitot,       &
            tmparr1,mflux_r,mflux_i,invexn,                                              &
            relhum     !BG added relhum
 real, dimension(its:ite,kts:kte)      :: qvi_fix,qvs_fix !BG added for depofix
 ...
 real    :: lammax,lammin,mu,dv,sc,dqsdt,ab,kap,epsr,epsc,xx,aaa,epsilon,sigvl,epsi_tot, &
            ...
            drhop,timeScaleFactor,dt_left,tmp4,tmp5,tmp6,tmp7,tmp8,tmp9,scrit,het_frac  !BG added scrit
real    :: aaa_fix,ssat_cld_fix,dqsdt_fix,dqsidt_fix,dum_fix,abi_fix,oabi_fix !BG added all in this line for depofix
...
!BG start
!new freezing after Liu and Penner, 2005 with pre existing ice formulation from Karcher et al. 2006
!at STP
   real  :: ad, bd
   real  ::  wbar1, wbar2
   ...
   real :: nimey                     ! nucleated number from deposition nucleation (meyers) !BG not used
   real :: n1, ni                    ! nucleated number
   real :: A, B, regm            ! work variable
   real :: esl, esi, deles           ! work variable

   !real(r8), intent(out) :: fhom       ! how much fraction of cloud can reach Shom
   !BG i keep this local
   real :: fhom       ! how much fraction of cloud can reach Shom

   ! used in SUBROUTINE Vpreice
   real :: Ni_preice        ! cloud ice number conc (1/m3)
   real :: lami,Ri_preice   ! mean cloud ice radius (m)
   real :: Shom             ! initial ice saturation ratio; if <1, use hom threshold Si
   real :: detaT,RHimean    ! temperature standard deviation, mean cloudy RHi
   real :: wpicehet   ! diagnosed Vertical velocity Reduction caused by preexisting ice (m/s), at shet

   real :: weffhet    ! effective Vertical velocity for ice nucleation (m/s)  weff=wbar-wpicehet
   real :: wpice !intent out?      ! diagnosed Vertical velocity Reduction caused by preexisting ice (m/s), at Shom
   real :: weff  !intent out?     ! effective Vertical velocity for ice nucleation (m/s); weff=wbar-wpice
!BG end

!BG continuation
real, parameter :: Shet   = 1.2     ! het freezing threshold
real, parameter :: rhoice = 0.5e3   ! kg/m3, Wpice is not sensitive to rhoice
real, parameter :: minweff= 0.001   ! m/s
real, parameter :: gamma4=6.0       !used in preice
...
! quantities related to process rates/parameters, interpolated from lookup tables:
 ...
 !bloss(SplitIceOptics)
 real    :: f1pr31   ! "cloud ice" as a mass fraction of all ice
 real    :: f1pr32   ! Generalized effective size (Dge) of "cloud ice" (D<150um)
 real    :: f1pr33   ! "snow" as a mass fraction of all ice
 real    :: f1pr34   ! Dge of "snow" (150um<D<500um and rime fraction<0.5)
 real    :: f1pr35   ! "graupel" as a mass fraction of all ice
 real    :: f1pr36   ! Dge of "graupel" (everything else)

 real :: Dge_numer, Dge_denom
 real :: Dge_clice(nCat), Dge_snow(nCat), Dge_graup(nCat)
 real :: qfrac_clice(nCat), qfrac_snow(nCat), qfrac_graup(nCat)

 real :: IceMassRadius, tmp_xx, icefall_scale
...
! to be added as namelist parameters (future)
 ...
 !SAM additions begin
 integer  :: idx, fidx !BG: index for process rate array
 !SAM additions end
...
! note -- this code could be moved to 'p3_init'
 ...
 !end select
  ...
  deltaD_init = minDiam !BG, defined in the P3 namelist
...
!diag_effr = 25.e-6 ! default value
 ...
 diag_qice_snow = 0.
 diag_qice_graup = 0.
 diag_Dge_clice = 25.e-6 ! default value
 diag_Dge_snow  = 25.e-6 ! default value
 diag_Dge_graup = 25.e-6 ! default value
 diag_vmi  = 0.
 diag_di   = 0.
 diag_rhoi = 0.
 diag_2d   = 0.
...
!SAM additions begin
 !Guangxing Lin -- bloss: change to mtend, stend.
 mtend = 0.
 stend = 0.
...
!------------------------------------------------------------------------------------------!
 !bloss: set up for computation of microphysical source terms
 mtend(:,:,iqv)  = qv(:,:)+qc(:,:) ! total water = qv + qcl
 mtend(:,:,incl) = nc(:,:)
 mtend(:,:,iqr)  = qr(:,:)
 mtend(:,:,inr)  = nr(:,:)
 do iice = 1,nCat
   mtend(:,:,iqit(iice)) = qitot(:,:,iice) !bloss: Relax to allow nCat>1
   mtend(:,:,iqir(iice)) = qirim(:,:,iice)
   mtend(:,:,iqib(iice)) = birim(:,:,iice) !bloss(2020-12-10): fix typo (qitot-->birim)
   mtend(:,:,inci(iice)) = nitot(:,:,iice)
 end do
...
!bloss/SAM: rho is now an input
!bloss/SAM       rho(i,k)     = pres(i,k)/(rd*t(i,k))
!bloss/SAM: for consistency, use SAM values for latent heats
       inv_rho(i,k) = 1./rho(i,k)
       xxlv(i,k)    = lcond_SAM !bloss 3.1484e6-2370.*t(i,k)
       xxls(i,k)    = xxlv(i,k)+lfus_SAM !bloss 0.3337e6
      ...
      ! if supersaturation is predicted then diagnose sup and supi from ssat
       ...
       !BG relative humidty with respect to water in %
       relhum(i,k)  = (sup(i,k)+1.)*100.0
       !BG end

       rhofacr(i,k) = (rhosur*inv_rho(i,k))**0.54
       rhofaci(i,k) = icefall_fudge*(rhosui*inv_rho(i,k))**0.54
...
!   main k-loop (for processes):
    ...
    ! initialize ice-phase  process rates
       ...
       !SAM additions begin
      qcrfrz  = 0.;     ncrfrz  = 0. !BG added, sum of all freezing rates
      qchomi  = 0.;     nchomi  = 0.;      qrhomi=0.;     nrhomi=0. !BG added, for hom frz
      ninuc2  = 0.;     ninuc3  = 0.; !BG
      qinuc2  = 0.;     qinuc3  = 0.; !BG
      qcrfrz_het=0.;ncrfrz_het=0.;!in the other loop !qcrfrz_hom=0.;ncrfrz_hom=0. !BG added, sum of all freezing rates
      !in the other loop !!! qchomi  = 0.;  nchomi  = 0.; qrhomi=0.;  nrhomi=0. !BG added, for hom frz
       !SAM additions end
...
!         N_nuc = cons5*exp(log(cdistr(i,k))+log(gamma(mu_r(i,k)+4.))-3.*log(lamr(i,k)))*exp(aimm*(273.15-t(i,k)))*SPF(k)
          ...
          if (nCat>1) then
             if (frzmodes) then !BG freezing separate category separate source
                iice_dest=2   !imm frz of rain = cat 2
             else
               !determine destination ice-phase category:
               dum1  = 900.     !density of new ice
               D_new = ((Q_nuc*6.)/(pi*dum1*N_nuc))**thrd
...
! rime splintering (Hallet-Mossop 1974)
       ...
       rimesplintering_on:  if (log_hmossopOn) then
          ...
          if (nCat>1) then
             if (frzmodes) then !BG freezing separate categories
                iice_dest=3 !Hallet Mossop = cat 3 (if allowed)
             else
               !determine destination ice-phase category
...
!................................................................
   ...
   if (iIceNucleationScheme.eq.1) then !BG
  !................................................................
  ...
  ! allow ice nucleation if < -15 C and > 5% ice supersaturation
      if (t(i,k).lt.258.15 .and. supi_cld .ge.0.05) then
        if (do_meyers) then !BG
          dum = exp(-0.639+0.1296*100.*supi_cld)*1000.*inv_rho(i,k)  !Meyers et al. (1992)
        else
          dum = 0.005*dexp(dble(0.304*(273.15-t(i,k))))*1000.*inv_rho(i,k)   !Cooper (1986) in units # kg-1
        endif !BG Meyers or Cooper
         dum = min(dum,100.e3*inv_rho(i,k)*SCF(k)) !set max to 100 per L
         ...
         if (N_nuc.ge.1.e-20) then
            Q_nuc = max(0.,(dum-sum(nitot(i,k,:)))*mi0*odt)
             if (nCat>1) then
                if (frzmodes) then !BG freezing separate category separate source
                   iice_dest=4  !depo/condensation freezing nucleation STANDARD = Cat4
                else
                  !determine destination ice-phase category:
                  dum1  = 900.     !density of new ice
                  D_new = ((Q_nuc*6.)/(pi*dum1*N_nuc))**thrd
         ...
         endif ! if(N_nuc>1e-20)

      endif !(t(i,k).lt.258.15 .and. supi(i,k).ge.0.05)


   elseif (iIceNucleationScheme.eq.2) then !BG

      if ( (t(i,k).lt.258.15) .and. (t(i,k) .ge. 236.15) .and. (qc(i,k) .gt. qsmall) ) then
      ! liquid cloud between -15 and -37 C
      if ( (supi(i,k).ge.0.05) .or. ( (t(i,k).lt.241.15) .and. (supi(i,k).ge.0.005) ) ) then
         ! 1.) deposition/condensation-freezing nucleation for MIXED PHASE
         !Notes:
         ! - BG added second condition (T<-31 C, Si>1.005) to prevent too much "hom frz at -37"
         ! - BG added a qc>qsmall condition above: based on recent findings that deposition freezing
         !      is negligible in mixed phase (e.g. Ansmann et al., 2018).  Therefore, in mixed phase,
         !      freeze occurs only by immersion, contact and possibly condensation freezing 
         !      (e.g. Hoose and Mohler, 2012, Lohmann et al., 2016)
         ...
         if (N_nuc.ge.1.e-20) then
            ...
            if (nCat>1) then
               if (frzmodes) then !BG freezing separate category separate source
                  iice_dest=4   !depo/cond frz BG modified in Cat4 
               else
                  !determine destination ice-phase category:
                  dum1  = 900.     !density of new ice
                  D_new = ((Q_nuc*6.)/(pi*dum1*N_nuc))**thrd
      ...
      end if ! Si>1.05 or (Si>1.005 & T<241.15K)

   endif ! 236.15 K < T < 258.15 .and. qc > qcsmall

   !BG aug 2023 test
   if (dust_ramp) then
      ! ndust = NumCirrusINP above -40C and = ramp_min*NumCirrusINP below -70C
      !   varies linearly by temperature in between
      ndust = NumCirrusINP*MAX(ramp_min, MIN(1., &
                               ramp_min + (1. - ramp_min) * (t(i,k) - 203.15) / 30.0 ) )
      !in units cm-3
   end if !dust ramp BG 
   ...
   if (do_new_lp_frz) then
    ...
    if (use_preexisting_ice) then
       !This formulation, which accounts for the presence of pre-existing ice on nucleation,
       !  is based on Karcher et al., 2006, with the implementation taken from E3SMv1 and 
       ...
       if (do_mesoscale_variab) then
          ! Option to allow mesoscale variability of supersaturation, which
          !   restricts homogeneous nucleation to a fraction fo the grid cell
          !   (fhom).  Seems to be intended for larger (GCM) grid boxes, probably
          !   at least 10km.  Might make sense in finer resolution simulations
          !   with subgrid cloudiness.  Described in Shi et al., 2015, ACP section 2.3
   ...
   ni = 0.
    ...
    if((tc(i,k).le.-35.0) .and. (supi_cld.ge. 0.2) .and. (wbar1 .ge. 1.e-6)  ) then
       ...
       regm = 0.8 * (A * log(wbar1) ) + B !BGBGBGGB

       if( (tc(i,k).lt.-40. .and. wbar1.gt.1.) &
            .OR. (tc(i,k).lt.regm-5.) ) then
       ...
       else ! either heterogeneous-only nucleation or het-hom competition

          if (tc(i,k) .gt. regm) then
             ! heterogeneous nucleation only, following Liu et al., 2007, eq 10.
             !   Note: it depends on INP number and W!
          ...
          end if ! if heterogeneous-only nucleation

       end if ! if homogeneous-only nucleation
       ni = n1 !number of newly-nucleated ice crystals [cm-3]

    end if !((tc(i,k).le.-35.0) .and. (supi(i,k).ge. 0.2) .and. (wbar1 .ge. 1.e-6)  )

   dum = ni*1.e+6*inv_rho(i,k) !from cm-3 to m-3 to kg-1 air
   dum = min(dum,80.e+6*inv_rho(i,k)*SCF(k)) !limit ni_max to 80000 per L or 80/cc
   ...
   if(use_preexisting_ice) then
      ...
      !BG changed therefore to:  #odt = inverse model timestep 
      !=> we want to get a rate of nucleation /second here
      N_nuc = dum*odt
   end if
   ...
   if (N_nuc.ge.1.e-20) then
     ...
     het_frac = (niimm+nidep)/(niimm+nidep+nihf)
     qinuc2(iice_dest) = het_frac*Q_nuc !BG no multiplication with inverse model timestep odt, done above already
     ninuc2(iice_dest) = het_frac*N_nuc
     qinuc3(iice_dest) = (1.-het_frac)*Q_nuc !BG no multiplication with inverse model timestep odt, done above already
     ninuc3(iice_dest) = (1.-het_frac)*N_nuc
  ...
  !  The Mohler et al and homogeneous-freezing-only Liu & Penner
  !    options are included for sensitivity studies.
  if (enable_cirrus_mohler_ice_nucleation) then
     ...
     if ( (t(i,k).lt.236.15) .and. (supi(i,k).ge. scrit) ) then
        ...
        if (N_nuc.ge.1.e-20) then
           ...
           if (nCat>1) then
             if (frzmodes) then !BG freezing separate category separate source
                iice_dest=7   !dust het frz in cirrus = Cat 7
             else
               !determine destination ice-phase category:
               dum1  = 900.     !density of new ice
               D_new = ((Q_nuc*6.)/(pi*dum1*N_nuc))**thrd
     ...
     endif !(t(i,k).lt.236.15 .and. supi(i,k).ge.0.05)
  end if ! (enable_cirrus_mohler_ice_nucleation)

  if (enable_lphom_ice_nucleation) then
     ...
     if ( (t(i,k).lt.236.15)  .and. (supi_cld.ge. 0.42)) then !added some very conservative supi condition not to always go in that loop
        ...
        if (N_nuc.ge.1.e-20) then
           ...
           if (nCat>1) then
             if (frzmodes) then !BG freezing separate category separate source
                iice_dest=5  !LP scheme Hom freezing ONLY in cirrus= cat 5
             else
               !determine destination ice-phase category:
               dum1  = 900.     !density of new ice
               D_new = ((Q_nuc*6.)/(pi*dum1*N_nuc))**thrd
  ...
  endif ! (enable_lphom_ice_nucleation)

  ! ===== END SENSITIVITY STUDIES ==========

endif ! iIceNucleationScheme options

! For sensitivity studies switching off flavors of ice nucleation
if (no_ice_nucleation .or. no_het_ice_nucleation) then
   qinuc(:) = 0.
   ninuc(:) = 0.
   qinuc2(:) = 0.
...
! this is not applied at the first time step, since saturation adjustment is applied at the first step
       ...
       if (.not.(log_predictNc).and.sup_cld.gt.1.e-6.and.it.gt.1) then
          ...
          ab    = 1. + dqsdt*xxlv(i,k)*inv_cp
          !dum   = min(dum,(Qv_cld(k)-dumqvs)/ab)  ! limit overdepletion of supersaturation !ABS commenting out at Peters suggestion
          dum = max(0., min(dum,(Qv_cld(k)-dumqvs)/ab)) !ABS added at Peters suggestion
          qcnuc = dum*odt*SCF(k)
       endif
          ...
          elseif (iparam.eq.3) then
           ...
           !Khroutdinov and Kogan (2000)
            ...
            ! note: ncautr is change in Nr; ncautc is change in Nc
             ! make dependency of rate on grid spacing - MK
             if(do_scale_dependence_of_autoconv) then
              qcaut = qcaut*min(1.,100./deltax)**2
             end if
...
! be greater than the sum of the sources, thereby resulting in overdepletion.
   ...
   !   amongst categories.
       ...
       tmp1 = sum(qidep)+sum(qinuc)+sum(qinuc2)+sum(qinuc3) !BG added qinuc2 and 3
        ...
        ! but accounts for rapid evaporation of small melting ice particles)
         ...
         ! add sink terms, assume density stays constant for sink terms
          ...
          nitot(i,k,iice) = nitot(i,k,iice) + (ninuc(iice)+ninuc2(iice)+ninuc3(iice)-nimlt(iice)-nisub(iice)-      &
                            ...
                            ncheti(iice)+nimul(iice))*dt !BG added ninuc2/3
...
!SAM additions begin
       ...
       !bloss: REMOVE homogeneous freezing from this since it is done separately
qcrfrz = qinuc+qchetc+qcheti+qrhetc+qrheti  !+ qinuc2+qinuc3 !BG sum of all freezing of cloud drops and rain [mass]
ncrfrz = ninuc+nchetc+ncheti+nrhetc+nrheti  !+ ninuc2+ninuc3 !BG sum of all freezing of cloud drops and rain [number]

!BG add accumulation of microphysical process rates here [in main loop as rates are 1dim or in iice function]
 if (do_accumulate_micro_proc_rates) then
    ...
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(qinuc(:)) ! deposition/condensation freezing nuc [kg/kg/s] ABS added but not sure what it is -- Myers/Cooper mixed-phase freezing
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(qinuc2(:)) ! contact freezing before (turned off) so now it's in-situ freezing in cirrus
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(qinuc3(:))! het/homogeneous nucleation competition as in Liu & Penner
    idx = idx + 1
      ...
      !find bottom
       ...
       two_moment: if (log_predictNc) then  !2-moment cloud:
          ...
          substep_sedi_c2: do while (dt_left.gt.1.e-4)
             ...
             !Guangxing Lin/bloss/SAM: output both qc/nc using stend
!bloss             qcsten(i,k) = qcsten(i,k) + fluxdiv_qx*inv_rho(i,k) !kg/kg/s
             stend(i,k,iqv) = stend(i,k,iqv) + fluxdiv_qx*inv_rho(i,k)*dt_sub/dt !kg/kg/s
             stend(i,k,incl) = stend(i,k,incl) + fluxdiv_nx*inv_rho(i,k)*dt_sub/dt !#/kg/s
...
!bloss                qcsten(i,k) = qcsten(i,k) + fluxdiv_qx*inv_rho(i,k) !kg/kg/s
          ...
          substep_sedi_c1: do while (dt_left.gt.1.e-4)
             ...
             !Guangxing Lin/bloss/SAM: output both qc using stend
!bloss             qcsten(i,k) = qcsten(i,k) + fluxdiv_qx*inv_rho(i,k) !kg/kg/s
             stend(i,k,iqv) = stend(i,k,iqv) + fluxdiv_qx*inv_rho(i,k)*dt_sub/dt !kg/kg/s
...
!            Co_max = max(Co_max, max(V_nr(k),V_qr(k))*dt_left*inv_dzq(i,k))
          ...
          nr(i,k) = nr(i,k) + fluxdiv_nx*dt_sub*inv_rho(i,k)
           ...
           ! Guangxing Lin/bloss/SAM: output both qr/nr using stend
!bloss          qrsten(i,k) = qrsten(i,k) + fluxdiv_qx*inv_rho(i,k) !kg/kg/s
          stend(i,k,iqr) = stend(i,k,iqr) + fluxdiv_qx*inv_rho(i,k)*dt_sub/dt !kg/kg/s
          stend(i,k,inr) = stend(i,k,inr) + fluxdiv_nx*inv_rho(i,k)*dt_sub/dt !#/kg/s
          do k = k_qxtop-kdir,k_temp,-kdir
             !-- compute flux divergence
             fluxdiv_qx = (flux_qx(k+kdir) - flux_qx(k))*inv_dzq(i,k)
             fluxdiv_nx = (flux_nx(k+kdir) - flux_nx(k))*inv_dzq(i,k)
             ...
             kloop_sedi_i1: do k = k_qxtop,k_qxbot,-kdir
                ...
                qi_notsmall_i1: if (qitot(i,k,iice)>qsmall) then
                 ...
                 !==
                   ...
                   doSizeDependentIceFallFudge: if(doSizeDependentIceFallFudge) then
                      IceMassRadius = (qitot(i,k,iice) / (4. * piov3 * 916.7 * nitot(i,k,iice) ) ) **(1./3.)

                      icefall_scale = 1.
                      if(IceMassRadius.lt.60.e-6) then
                         tmp_xx = MAX(0., MIN(1., (IceMassRadius - 25.e-6) / (75.e-6 - 25.e-6) ) )
                         icefall_scale = icefall_fudge_small + tmp_xx*(icefall_fudge_medium - icefall_fudge_small)
                      else
                         tmp_xx = MAX(0., MIN(1., (IceMassRadius - 75.e-6) / (150.e-6 - 75.e-6) ) )
                         icefall_scale = icefall_fudge_medium + tmp_xx*(icefall_fudge_large - icefall_fudge_medium)
                      end if
!bloss                      write(*,995) qitot(i,k,iice), nitot(i,k,iice), 1.e6*IceMassRadius, tmp_xx, icefall_scale
995                   format('qitot/nitot/Rice(um)/tmp_xx/icefall_scale ',2e12.4,3F10.3)

                      V_qit(k) = V_qit(k)*icefall_scale
                      V_nit(k) = V_nit(k)*icefall_scale
                   end if doSizeDependentIceFallFudge
            ...
            !zitot(i,k,iice) = zitot(i,k,iice) + fluxdiv_nit*dt_sub*inv_rho(i,k)
             ...
             !Guangxing Lin/bloss/SAM: output all ice sedimentation tendencies using stend
!bloss             qitsten(i,k) = qitsten(i,k) + fluxdiv_qit*inv_rho(i,k) !kg/kg/s
             stend(i,k,iqit(iice)) = stend(i,k,iqit(iice)) + fluxdiv_qit*inv_rho(i,k)*dt_sub/dt !kg/kg/s
             stend(i,k,iqir(iice)) = stend(i,k,iqir(iice)) + fluxdiv_qir*inv_rho(i,k)*dt_sub/dt !kg/kg/s
             stend(i,k,iqib(iice)) = stend(i,k,iqib(iice)) + fluxdiv_bir*inv_rho(i,k)*dt_sub/dt !m3/kg/s
             stend(i,k,inci(iice)) = stend(i,k,inci(iice)) + fluxdiv_nit*inv_rho(i,k)*dt_sub/dt !#/kg/s
...
! and compute diagnostic fields for output
    ...
    ! cloud:
       if (qc(i,k)*iSCF(k).ge.qsmall) then
          ...
          lamc_out(i,k) = lamc(i,k) !Guangxing Lin
          pgam_out(i,k) = mu_c(i,k) !Guangxing Lin
       else
          qv(i,k) = qv(i,k)+qc(i,k)
          th(i,k) = th(i,k)-invexn(i,k)*qc(i,k)*xxlv(i,k)*inv_cp
          qc(i,k) = 0.
       ...
       iice_loop_final_diagnostics:  do iice = 1,nCat
          ...
          qi_not_small:  if (qitot(i,k,iice).ge.qsmall) then
           ...
           ! if (.not. tripleMoment_on) zitot(i,k,iice) = diag_mom6(qitot(i,k,iice),nitot(i,k,iice),rho(i,k))
             ...
             if(doSplitIceOptics) then !bloss
                call access_lookup_table(dumjj,dumii,dumi,13,dum1,dum4,dum5,f1pr31) !qfrac cloud ice
                call access_lookup_table(dumjj,dumii,dumi,14,dum1,dum4,dum5,f1pr32) !Dge cloud ice
                call access_lookup_table(dumjj,dumii,dumi,15,dum1,dum4,dum5,f1pr33) !qfrac snow
                call access_lookup_table(dumjj,dumii,dumi,16,dum1,dum4,dum5,f1pr34) !Dge snow
                call access_lookup_table(dumjj,dumii,dumi,17,dum1,dum4,dum5,f1pr35) !qfrac graupel
                call access_lookup_table(dumjj,dumii,dumi,18,dum1,dum4,dum5,f1pr36) !Dge graupel
             end if !bloss
  ...
  ! note that reflectivity from lookup table is normalized, so we need to multiply by N
          ...
          ! note factor of air density below is to convert from m^6/kg to m^6/m^3
             ...
             if(doSplitIceOptics) then
                ! accumulate IWC and projected area for all iice=1,nCat
!bloss                write(*,991) f1pr31,f1pr32,f1pr33,f1pr34,f1pr35,f1pr36
                991 format('qfrac/Dge = ',6e12.4)
                qfrac_clice(iice) = f1pr31
                Dge_clice(iice)   = f1pr32
                qfrac_snow(iice)  = f1pr33
                Dge_snow(iice)    = f1pr34
                qfrac_graup(iice) = f1pr35
                Dge_graup(iice)   = f1pr36
             end if
       ...
       if(doSplitIceOptics.AND.(SUM(qitot(i,k,1:nCat)).gt.qsmall)) then
          ! Combine cloud ice (i.e., D<150 um) across each category
          Dge_numer = 0.
          Dge_denom = 0.
          do iice = 1,nCat
             Dge_numer = Dge_numer + qfrac_clice(iice)*qitot(i,k,iice)
             Dge_denom = Dge_denom + qfrac_clice(iice)*qitot(i,k,iice)/MAX(1.e-6,Dge_clice(iice))
          end do
          if(Dge_denom.gt.0.) then
             diag_qice_clice(i,k) = Dge_numer
             diag_Dge_clice(i,k) = Dge_numer/Dge_denom
          else
             diag_qice_clice(i,k) = 0.
             diag_Dge_clice(i,k) = 25.e-6
          end if
          
          ! Combine snow (i.e., 150 < D < 500 um, rime frac < 0.5) across each category
          Dge_numer = 0.
          Dge_denom = 0.
          do iice = 1,nCat
             Dge_numer = Dge_numer + qfrac_snow(iice)*qitot(i,k,iice)
             Dge_denom = Dge_denom + qfrac_snow(iice)*qitot(i,k,iice)/MAX(1.e-6,Dge_snow(iice))
          end do
          if(Dge_denom.gt.0.) then
             diag_qice_snow(i,k) = Dge_numer
             diag_Dge_snow(i,k) = Dge_numer/Dge_denom
          else
             diag_qice_snow(i,k) = 0.
             diag_Dge_snow(i,k) = 25.e-6
          end if
          
          ! Combine graupel (i.e., everything else) across each category
          Dge_numer = 0.
          Dge_denom = 0.
          do iice = 1,nCat
             Dge_numer = Dge_numer + qfrac_graup(iice)*qitot(i,k,iice)
             Dge_denom = Dge_denom + qfrac_graup(iice)*qitot(i,k,iice)/MAX(1.e-6,Dge_graup(iice))
          end do
          if(Dge_denom.gt.0.) then
             diag_qice_graup(i,k) = Dge_numer
             diag_Dge_graup(i,k) = Dge_numer/Dge_denom
          else
             diag_qice_graup(i,k) = 0.
             diag_Dge_graup(i,k) = 25.e-6
          end if
          
!bloss             write(*,997) diag_qice_clice(i,k), diag_qice_snow(i,k), diag_qice_graup(i,k)
997          format('qice (clice/snow/graup) = ', 3e12.4)
!bloss             write(*,998) 1.e6*diag_Dge_clice(i,k), 1.e6*diag_Dge_snow(i,k), 1.e6*diag_Dge_graup(i,k)
998          format('Dge  (clice/snow/graup) = ', 3f12.4)

             
       end if !bloss(SplitIceOptics)
       ...
       do k = kbot,ktop,kdir
          do iice = nCat,2,-1
           ...
           ! simility condition (similar mean sizes)
             ...
             !BG changed this for testing
             if ( (diag_di(i,k,iice) .gt. (1./deltaD_init)*diag_di(i,k,iice-1) ) .and. &
                  (diag_di(i,k,iice) .lt. deltaD_init*diag_di(i,k,iice-1) ) ) then
...
!SAM additions begin
    !bloss: compute of microphysical source terms for each species
    !print*,'maxval number ice in module p3',maxval(nitot(:,:,1)) !BG
    ! check to see that mtend holds values from beginning of time step.
!bloss    if(MINVAL(mtend(:,:,iqv)).gt.1.e-6) then
      !bloss: compute of microphysical source terms (mtend holds values at beginning of time step)
      !  subtract off final values and remove sedimentation tendencies (that have been computed explicitly).
 mtend(:,:,iqv)  = ( qv(:,:) + qc(:,:) - mtend(:,:,iqv) )*odt ! total water = qv + qcl
 mtend(:,:,incl) = ( nc(:,:) - mtend(:,:,incl) )*odt
 mtend(:,:,iqr)  = ( qr(:,:) - mtend(:,:,iqr)  )*odt
 mtend(:,:,inr)  = ( nr(:,:) - mtend(:,:,inr)  )*odt
 do iice = 1,nCat
   mtend(:,:,iqit(iice)) = ( qitot(:,:,iice) - mtend(:,:,iqit(iice)) )*odt ! assume nCat==1
   mtend(:,:,iqir(iice)) = ( qirim(:,:,iice) - mtend(:,:,iqir(iice)) )*odt
   mtend(:,:,iqib(iice)) = ( birim(:,:,iice) - mtend(:,:,iqib(iice)) )*odt
   mtend(:,:,inci(iice)) = ( nitot(:,:,iice) - mtend(:,:,inci(iice)) )*odt
 end do

 ! mtend now holds the microphysical+sedimentation tendency over the time step.
 !   Remove the sedimentation tendency to leave the microphysical tendency
 do n = 1,size(mtend,dim=3)
   do k = 1,size(mtend,dim=2)
     do i = 1,size(mtend,dim=1)
       mtend(i,k,n) =  mtend(i,k,n) - stend(i,k,n) 
     end do
   end do
 end do

!!$ mtend(:,:,iqv) =  mtend(:,:,iqv) - stend(:,:,iqv) 
!!$ mtend(:,:,incl) = mtend(:,:,incl) - stend(:,:,incl)
!!$ mtend(:,:,iqr) = mtend(:,:,iqr) - stend(:,:,iqr)
!!$ mtend(:,:,inr) = mtend(:,:,inr) - stend(:,:,inr) 
!!$ mtend(:,:,iqit) = mtend(:,:,iqit) - stend(:,:,iqit)
!!$ mtend(:,:,iqir) = mtend(:,:,iqir) - stend(:,:,iqir)
!!$ mtend(:,:,iqib) = mtend(:,:,iqib) - stend(:,:,iqib)
!!$ mtend(:,:,inci) = mtend(:,:,inci) - stend(:,:,inci)
!!$
!!$      do iice = 1,nCat
!!$        mtend(:,:,iqit(iice)) = (qitot(:,:,iice)-mtend(:,:,iqit(iice)))/dt  - stend(:,:,iqit(iice)) ! relax to allow nCat>1
!!$        mtend(:,:,iqir(iice)) = (qirim(:,:,iice)-mtend(:,:,iqir(iice)))/dt - stend(:,:,iqir(iice))
!!$        mtend(:,:,iqib(iice)) = (qitot(:,:,iice)-mtend(:,:,iqib(iice)))/dt - stend(:,:,iqib(iice))
!!$        mtend(:,:,inci(iice)) = (nitot(:,:,iice)-mtend(:,:,inci(iice)))/dt - stend(:,:,inci(iice))
!!$      end do
!!$    end if
!bloss    end if
!SAM additions end
...
! Always diagnose hydrometer types as they are used by the rest of SAM (added by Marat)
    ktop_typeDiag = ktop
...
! V1.7
      ...
      !bloss: Use SAM sat.f90 saturation vapor pressures
      !      Based on an exponential formulation, they are valid for T<-80C
      !      and require less computation than Murphy-Koop.      
...
!-------------------------------------------
      ...
      IF (doSAMsvp) THEN
         ...
         IF (i_type.EQ.1.AND. T.lt.273.15) THEN
            ! ice saturation vapor pressure in Pa
            polysvp1 = 100.*esati(t) 
         ELSE
            ! liquid saturation vapor pressure in Pa
            polysvp1 = 100.*esatw(t) 
         END IF

      ELSE ! Default P3 computation
         if (i_type.EQ.1 .and. T.lt.273.15) then
            ! ICE
...
!--------------------------------------------------------------------------
	  ...
	  !bloss/SAM: add option for fixed gamma exponent for cloud
          if(dofix_pgam) then 
            !bloss: Fix gamma exponent for cloud
            mu_c = pgam_fixed

            if(iparam.eq.1) then
              ! limit mu_c if using Seifert-Behend
              mu_c = max(mu_c,2.)
              mu_c = min(mu_c,15.)
            end if

          else
            ! use Morrison formula for mu_c
...
subroutine hetero(T,ww,        Ns,Nis,Nid)
          !hetero(regm,wbar2,ndust,niimm,nidep)
...
SUBROUTINE Vpreice(P_in, T_in, R_in, C_in, S_in, V_out)
   ...
   REAL, INTENT(in)  :: P_in       ! [Pa],INITIAL AIR pressure
   REAL, INTENT(in)  :: T_in       ! [K] ,INITIAL AIR temperature
   REAL, INTENT(in)  :: R_in       ! [m],INITIAL MEAN  ICE CRYSTAL NUMBER RADIUS
   REAL, INTENT(in)  :: C_in       ! [m-3],INITIAL TOTAL ICE CRYSTAL NUMBER DENSITY, [1/cm3]
   REAL, INTENT(in)  :: S_in       ! [-],INITIAL ICE SATURATION RATIO;; if <1, use hom threshold Si
   REAL, INTENT(out) :: V_out      ! [m/s], VERTICAL VELOCITY REDUCTION (caused by preexisting ice)

   ! SUBROUTINE parameters
   REAL, PARAMETER :: ALPHAc  = 0.5 ! density of ice (g/cm3), !!!V is not related to ALPHAc
   REAL, PARAMETER :: FA1c    = 0.601272523
   REAL, PARAMETER :: FA2c    = 0.000342181855
   REAL, PARAMETER :: FA3c    = 1.49236645E-12
   REAL, PARAMETER :: WVP1c   = 3.6E+10
   REAL, PARAMETER :: WVP2c   = 6145.0
   REAL, PARAMETER :: FVTHc   = 11713803.0
   REAL, PARAMETER :: THOUBKc = 7.24637701E+18
   REAL, PARAMETER :: SVOLc   = 3.23E-23    ! SVOL=XMW/RHOICE
   REAL, PARAMETER :: FDc     = 249.239822
   REAL, PARAMETER :: FPIVOLc = 3.89051704E+23
   REAL :: T,P,S,R,C
   REAL :: A1,A2,A3,B1,B2
   REAL :: T_1,PICE,FLUX,ALP4,CISAT,DLOSS,VICE

   T = T_in          ! K  , K
   P = P_in*1e-2  ! Pa , hpa

   IF (S_in.LT.1.0) THEN
      S = 2.349 - (T/259.0) ! homogeneous freezing threshold, according to Ren & McKenzie, 2005
   ELSE
      S = S_in                    ! INPUT ICE SATURATION RATIO, -,  >1
   ENDIF

   R     = R_in*1e2   ! m  => cm
   C     = C_in*1e-6  ! m-3 => cm-3
   T_1   = 1.0/ T
   ...
   DLOSS = FPIVOLc * C * B1 * R**2 / ( 1.0+ B2 * R )
   VICE  = ( A2 + A3 * S ) * DLOSS / ( A1 * S )  ! 2006,(19)
   V_out = VICE*1e-2  ! cm/s => m/s
...
subroutine frachom(Tmean,RHimean,detaT,fhom)
   ...
   ! form section - 2.2. Mesoscale temperature fluctuations
   ! relevant for length scales between 1-12 km (Gaussian distributions) and even larger
   ! where you have a Gaussian too, but with a flatter tail
   !BG comment
   !I think this is useful for GCMs or when using coarser resolutions, on the order of magnitude of 10 km
   !

   real, intent(in)  :: Tmean, RHimean, detaT
   real, intent(out) :: fhom

   real, parameter :: seta = 6132.9  ! K
   integer,  parameter :: Nbin=200          ! (Tmean - 3*detaT, Tmean + 3*detaT)

   real :: PDF_T(Nbin)    ! temperature PDF;  ! PDF_T=0  outside (Tmean-3*detaT, Tmean+3*detaT)
   real :: Sbin(Nbin)     ! the fluctuations of Si that are driven by the T variations
   real :: Sihom, deta
   integer  :: i

   Sihom = 2.349-Tmean/259.0   ! homogeneous freezing threshold, according to Ren & McKenzie, 2005
   fhom  = 0.0

   do i = Nbin, 1, -1

      deta     = (i - 0.5 - Nbin/2)*6.0/Nbin   ! PDF_T=0  outside (Tmean-3*detaT, Tmean+3*detaT)
      Sbin(i)  = RHimean*exp(deta*detaT*seta/Tmean**2.0)
      PDF_T(i) = exp(-deta**2.0/2.0)*6.0/(sqrt(2.0*Pi)*Nbin)
   ...
   fhom=fhom/0.997   ! accounting for the finite limits (-3 , 3)

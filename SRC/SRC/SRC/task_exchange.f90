...
subroutine task_exchange(f,dimx1,dimx2,dimy1,dimy2,dimz,i_1, i_2, j_1, j_2)
...
integer i1, i2, j1, j2, mask
	...
	mask=100000+i1*10000+i2*1000+j1*100+j2*10
	ids(1) = mask+7
	ids(2) = mask+3
	ids(7) = mask+5
	ids(8) = mask+6
...
! Fill the data from the buffers that have the same rank (were not sent):
	...
	do m = 1,mend
	 if(flag(m)) then
           count = count+1
           tag = ids(m)
           call task_assign_bnd(f,dimx1,dimx2,dimy1,dimy2,dimz,buff_recv(1,m),tag)
	 end if 
	end do

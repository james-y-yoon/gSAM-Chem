...
do k=2,nzm-1
  ...
  do j=1,ny
    do i=1,nx
      ...
      t_tend(i,j,k) =  - rdz * (t(i,j,k1)-t(i,j,k2))*terra(i,j,k)
      q_tend(i,j,k) =  &
       - rdz * (micro_field(i,j,k1,index_water_vapor)-micro_field(i,j,k2,index_water_vapor)) &
                             *terra(i,j,k)
      ulsvadv(k) = ulsvadv(k) - rdz*(u(i,j,k1)-u(i,j,k2))*terrau(i,j,k) 
      vlsvadv(k) = vlsvadv(k) - rdz*(v(i,j,k1)-v(i,j,k2))*terrav(i,j,k) 
    end do
  end do

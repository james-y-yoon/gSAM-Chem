...
SUBROUTINE oceflx (rbot, ubot  ,vbot,  tbot, qbot, thbot ,   & 
           &       zbot, ts, qs, wd, shf, lhf,   &
           &       taux  ,tauy  ,ra, raq, tref  ,qref  ,uref, vref )

   use grid, only: rank, dx, dy, mu, ady
   use params
...
!  Tropical cyclon case (dotc=.true.) 
!  Marat 2025
!-------------------------------------------------------------------------------
        ...
        if(.not.dotc) then
          ...
          ! these settings are based on Komori et al (J. Phys. Oce, 2018)  - MK
          rdn    = sqrt(cdn(min(33.,vmag)))
          factor = max(0.,vmag-33.)*6.51e-5/rdn
       !   factor = 0.
          rhn = (1.0-stable)*(0.0327+factor) + stable * 0.018
          ren = 0.0346+factor 
        end if
        ...
        if(.not.dotc) then
          ...
          ! these settings are based on Komori et al (J. Phys. Oce, 2018)  - MK
          rdn    = sqrt(cdn(min(33.,u10n)))
          factor = max(0.,u10n-33.)*6.51e-5/rdn
       !   factor = 0.
          rhn = (1.0-stable)*(0.0327+factor) + stable * 0.018
          ren = 0.0346+factor                
        end if
        ...
        !--- momentum flux ---
        taux = -tau * (ubot-us) / vmag * tau_fudge
        tauy = -tau * (vbot-vs) / vmag * tau_fudge
        
        !--- heat flux ---
        shf  = -tau * tstar / ustar / rbot * shf_fudge
        lhf  = -tau * qstar / ustar / rbot * lhf_fudge

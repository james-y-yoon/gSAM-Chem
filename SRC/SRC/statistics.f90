...
use terrain
use slm_vars, only: snow_mass, rho_snow, snowt, soilt, t_canop, &
                    lhf_soil, shf_soil, lhf_canop, shf_canop 
use buildings, only: buildings_stat
implicit none	
	...
	real tvirt(nx,ny,nzm), tmp3D(nx,ny,nzm)
	...
	real cwpmax,cwpt(nx,ny),cwpl(nx,ny),cwpm(nx,ny),cwph(nx,ny)
	logical condition, condition_cl
	real zero(nzm)

	integer topind(nx,ny),z_inv_ind(nx,ny),z_base_ind(nx,ny),z_top_ind(nx,ny),ncloud	
        real zzz,grad_max(nx,ny),grad,www
        real acld, acldl, acldm, acldh

        real(8), external :: water_sum
!========================================================================
! UW ADDITIONS
...
!-----------------------------------------------	
	 ...
	 do j=1,ny
	  do i=1,nx
	   ...
	   qss=qpi(i,j,k)
           ...
           qcz(k)=qcz(k)+qcc*www
           qiz(k)=qiz(k)+qii*www
	   prof1(k)=prof1(k)+(qcc+qii)*www
	   prof2(k)=prof2(k)+(qrr+qss)*www
	   prof3(k)=prof3(k)+(qcc+qii+qrr+qss)*www
	   tmp(1)=tabs(i,j,k)*prespot(k)
           tpz(k)=tpz(k)+tmp(1)*www
	   tlz(k)=tlz(k)+tmp(1)*(1.-fac_cond*(qcl(i,j,k)+qci(i,j,k))/tabs(i,j,k))*www
           tvirt(i,j,k)=tmp(1)*(1.+epsv*qv(i,j,k)-(qcl(i,j,k)+qci(i,j,k))-(qpl(i,j,k)+qpi(i,j,k)))*www
	   tvz(k)=tvz(k)+tvirt(i,j,k)*www
           tez(k)=tez(k)+(tabs(i,j,k)+gamaz(k)+fac_cond*qv(i,j,k)-fac_fus*(qii+qss))*www
	   qvz(k) =qvz(k)+qv(i,j,k)*www
     	   dse(k)=dse(k)+(tabs(i,j,k)+gamaz(k))*www
	   mse(k)=mse(k)+(tabs(i,j,k)+gamaz(k)+fac_cond*qv(i,j,k))*www
	   sse(k)=sse(k)+(tabs(i,j,k)+gamaz(k)+fac_cond*qsatw(tabs(i,j,k),pres(k)))*www
	   qsatwz(k) = qsatwz(k)+qsatw(tabs(i,j,k),pres(k))*www
	   relhz(k)=relhz(k)+(qv(i,j,k)/qsatw(tabs(i,j,k),pres(k)))*www
	   relhiz(k)=relhiz(k)+(qv(i,j,k)/qsati(tabs(i,j,k),pres(k)))*www
	  end do
	 end do
	end do	
	
	call hbuf_put('TL',t0,1.)
	call hbuf_put('TABS',tabs0,1.)
	call hbuf_put('U',u0+ug,1.)
	call hbuf_put('V',v0+vg,1.)
	call hbuf_put('QT',q0,1.e3)
	...
	call hbuf_put('RELHI',relhiz,100.*factor_xy)
...
!-------------------------------------------------------------
	...
	call hbuf_put('PRECIP',precflux,factor_xy*dz/dts*86400.)
	
	do j=1,ny
	 do i=1,nx
	  precsfc(i,j)=precsfc(i,j)/dtst*86400.
	  precsfcsnow(i,j)=precsfcsnow(i,j)/dtst*86400.
          if(precsfc(i,j).gt.0.1) s_ar = s_ar + wgty(j)
          if(precsfc(i,j)/86400./rhow(1).gt.3.65e-5) s_arthr = s_arthr + wgty(j)
          precmean = precmean+precsfc(i,j)*wgty(j)
          snowmean = snowmean+precsfcsnow(i,j)*wgty(j)
          prec2 = prec2+(precsfc(i,j)**2)*wgty(j)
	 end do
	end do
	...
	do k=1,nzm
	 ...
	 qsatwz(k) = qsatwz(k)*factor_xy
        ...
        wvp = wvp + water_sum(qv)
        cwp = cwp + water_sum(qcl)
        iwp = iwp + water_sum(qci)
        rwp = rwp + water_sum(qpl)
        gwp = gwp + water_sum(qpg)
        swp = swp + water_sum(qpi-qpg)
        wvpobs = wvpobs + water_sum(tmp3D)
	...
	do k=2,nzm
	 ...
	 do j=1,ny
	  do i=1,nx
		...
		(tvirt(i,j,k-1)-tvz(k-1)+tvirt(i,j,k)-tvz(k))*www
	    qcwle(k) = qcwle(k) + 0.5*w(i,j,k)* &
		(qcl(i,j,k-1)-qcz(k-1)+ qcl(i,j,k)-qcz(k))*www
	    qiwle(k) = qiwle(k) + 0.5*w(i,j,k)* &
                (qci(i,j,k-1)-qiz(k-1)+qci(i,j,k)-qiz(k))*www
	    prof1(k)=prof1(k)+rho(k)*0.5* &
                (w(i,j,k)**2+w(i,j,k+1)**2)*(t(i,j,k)-t0(k))*www
          end do
	 end do
...
!-----------------------------------------------------------
	...
	do k=1,nzm
	 ...
	 do j=1,ny
	  do i=1,nx
            www=wgt(j,k)*terra(i,j,k)
	    u2z(k) = u2z(k)+(u(i,j,k)-u0(k))**2*wgtu(j,k)*terrau(i,j,k)	  
	    v2z(k) = v2z(k)+(v(i,j,k)-v0(k))**2*wgtv(j,k)*terrav(i,j,k)
	    w2z(k) = w2z(k)+0.5*(w(i,j,k+1)**2+w(i,j,k)**2)*wgtw(j,k)*terraw(i,j,k)
	    w22(k) = w22(k)+w(i,j,k)**2*wgtw(j,k)*terraw(i,j,k)
	    w3z(k) = w3z(k)+0.5*(w(i,j,k+1)**3+w(i,j,k)**3)*wgtw(j,k)*terraw(i,j,k)
	    t2z(k) = t2z(k)+(t(i,j,k)-t0(k))**2*wgt(j,k)*terra(i,j,k)	  
	    tqz(k) = tqz(k)+(t(i,j,k)-t0(k))*(qv(i,j,k)+qcl(i,j,k)+qci(i,j,k)-q0(k))*wgt(j,k)*terra(i,j,k)
	    q2z(k) = q2z(k)+(qv(i,j,k)+qcl(i,j,k)+qci(i,j,k)-q0(k))**2*wgt(j,k)*terra(i,j,k)
	    if(w(i,j,k)+w(i,j,k+1).gt.0) aup(k) = aup(k) + wgt(j,k)*terra(i,j,k)
	  end do
	 end do
	 ...
	 do j=1,ny
	  do i=1,nx
	    qc2z(k) = qc2z(k)+(qcl(i,j,k)-qcz(k))**2*wgt(j,k)*terra(i,j,k)
	    qi2z(k) = qi2z(k)+(qci(i,j,k)-qiz(k))**2*wgt(j,k)*terra(i,j,k)
	    qs2z(k) = qs2z(k)+(qsatw(tabs(i,j,k),pres(k))-qsatwz(k))**2*wgt(j,k)*terra(i,j,k)
	  end do
	 end do
	 ...
	 do j=1,ny
	  do i=1,nx
	    fadv(k)=fadv(k)+w(i,j,k)**2*rhow(k)*0.5* &
                         ( u(i,j,k-1)-u0(k-1)+u(i,j,k)-u0(k))*wgtw(j,k)*terraw(i,j,k)
	  end do
	 end do
...
!  VW advection d(w'w'v')/dz:
	...
	do k=2,nzm
	 ...
	 do j=1,ny
	  do i=1,nx
	    fadv(k)=fadv(k)+w(i,j,k)**2*rhow(k)*0.5* &	
                         ( v(i,j,k-1)-v0(k-1)+v(i,j,k)-v0(k))*wgtw(j,k)*terraw(i,j,k)
	  end do
	 end do
	 fadv(k)=fadv(k)*factor_xy
	end do
...
!  UW advection d(p'u')/dz:
	...
	do k=1,nz
	 ...
	 if(k.eq.1) then
	   do j=1,ny
	    do i=1,nx
	      fadv(k)=fadv(k)+(1.5*(u(i,j,k)-u0(k))*(pp(i,j,k)-pres(k))*100.- &
                     0.5*(u(i,j,k+1)-u0(k+1))*(pp(i,j,k+1)-pres(k+1))*100.)*wgtw(j,k)*terraw(i,j,k)
	    end do
	   end do
	 else if(k.eq.nz) then
	   do j=1,ny
	    do i=1,nx
	     fadv(k)=fadv(k)+(1.5*(u(i,j,k-1)-u0(k-1))*(pp(i,j,k-1)-pres(k-1))*100.- &
                  0.5*(u(i,j,k-2)-u0(k-2))*(pp(i,j,k-2)-pres(k-2))*100.)*wgtw(j,k-1)*terraw(i,j,k-1)
	    end do
	   end do
	 else
	   do j=1,ny
	    do i=1,nx
	      fadv(k)=fadv(k)+0.5*((u(i,j,k)-u0(k))*(pp(i,j,k)-pres(k))*100.+ &
                       (u(i,j,k-1)-u0(k-1))*(pp(i,j,k-1)-pres(k-1))*100.)*wgtw(j,k)*terraw(i,j,k)
	    end do
	   end do
	 end if
	 fadv(k)=fadv(k)*factor_xy
...
!  VW advection d(p'v')/dz:
	...
	do k=1,nz
	 ...
	 if(k.eq.1) then
	   do j=1,ny
	    do i=1,nx
	      fadv(k)=fadv(k)+(1.5*(v(i,j,k)-v0(k))*(pp(i,j,k)-pres(k))*100.- & 
                    0.5*(v(i,j,k+1)-v0(k+1))*(pp(i,j,k+1)-pres(k+1))*100.)*wgtw(j,k+1)*terraw(i,j,k+1)
	    end do
	   end do
	 else if(k.eq.nz) then
	   do j=1,ny
	    do i=1,nx
             fadv(k)=fadv(k)+(1.5*(v(i,j,k-1)-v0(k-1))*(pp(i,j,k-1)-pres(k-1))*100.- &
                     0.5*(v(i,j,k-2)-v0(k-2))*(pp(i,j,k-2)-pres(k-2))*100.)*wgtw(j,k-1)*terraw(i,j,k-1)
	    end do
	   end do
	 else
	   do j=1,ny
	    do i=1,nx
	      fadv(k)=fadv(k)+0.5*((v(i,j,k)-v0(k))*(pp(i,j,k)-pres(k))*100.+ &
                       (v(i,j,k-1)-v0(k-1))*(pp(i,j,k-1)-pres(k-1))*100.)*wgtw(j,k)*terraw(i,j,k)
	    end do
	   end do
	 end if
	 fadv(k)=fadv(k)*factor_xy
...
!       explicit dissipation of scalar variances is not computed yet
!       assume that most of contrinution of diffusion is actually dissipation - MK 
...
!  HW advection d(w'w'h')/dz:
	...
	do k=2,nzm
	 ...
	 do j=1,ny
	  do i=1,nx
	    fadv(k)=fadv(k)+w(i,j,k)**2*rhow(k)*0.5* &
                 	      ( t(i,j,k-1)-t0(k-1)+t(i,j,k)-t0(k))*wgtw(j,k)*terraw(i,j,k)
	  end do
	 end do
	end do
	do k=1,nzm
...
!  QW advection d(w'w'q')/dz:
	...
	do k=2,nzm
	 ...
	 do j=1,ny
	  do i=1,nx
	    fadv(k)=fadv(k)+w(i,j,k)**2*rhow(k)*0.5*wgtw(j,k)*terraw(i,j,k)* &
...
!bloss:  Major modification of conditional statistics to allow 
            ...
            do j = 1,ny
               do i = 1,nx
                  ...
                  if((icondavg_cld.gt.0).and.(qcl(i,j,k)+qci(i,j,k).gt.coef)) then
                     condavg_mask(i,j,k,icondavg_cld) = cld(i,j,k) ! cloud
                  end if
                  ...
                  if(icondavg_cor.gt.0) then
                     ...
                     if(LES_S) then
                       condition=condition_cl.and.condition
                     else
                       condition=condition.and.w(i,j,k)+w(i,j,k+1).gt.2.
                     end if
                  ...
                  if(icondavg_cordn.gt.0) then
                     ...
                     ! in LES_S, buoyant, saturated or rainy statistics
                     condition_cl = qcl(i,j,k)+qci(i,j,k).gt.coef &
                     ...
                     if(LES_S) then
                       ...
                       condition=condition.and.w(i,j,k)+w(i,j,k+1).lt.-2.
                     end if
         ...
         do ncond = 1,ncondavg
            ...
            do k=1,nzm
               ...
               do j=1,ny
                  ...
                  do i=1,nx
                     ...
                     if(condavg_mask(i,j,k,ncond).gt.0) then
                        ...
                        www = wgt(j,k)*terra(i,j,k)
                        clds(k)=clds(k) + cld(i,j,k)*www
                        tmp(1)=0.5*(w(i,j,k+1)*wgtw(j,k+1)*terraw(i,j,k+1)+w(i,j,k)*wgtw(j,k)*terraw(i,j,k))
                        wcl(k) = wcl(k) + tmp(1)
                        ucl(k) = ucl(k) + (u(i,j,k) + ug)*wgtu(j,k)*terrau(i,j,k) !bloss: include ground speed
                        vcl(k) = vcl(k) + (v(i,j,k) + vg)*wgtv(j,k)*terrav(i,j,k)
                        ucla(k) = ucla(k) + (u(i,j,k) - u0(k))*wgtu(j,k)*terrau(i,j,k) !bloss: u,v anomalies
                        vcla(k) = vcla(k) + (v(i,j,k) - v0(k))*wgtv(j,k)*terrav(i,j,k)
                        qcc=qcl(i,j,k)
                        qii=qci(i,j,k)
                        dse(k)=dse(k)+(tabs(i,j,k)+gamaz(k))*www
                        mse(k)=mse(k)+(tabs(i,j,k)+gamaz(k)+fac_cond*qv(i,j,k))*www
                        tcl(k) = tcl(k) + t(i,j,k)*www
                        qcll(k) = qcll(k) + (qv(i,j,k)+qcl(i,j,k)+qci(i,j,k))*www
                        qccl(k) = qccl(k) + qcc*www
                        qicl(k) = qicl(k) + qii*www
                        qpcl(k) = qpcl(k) + (qpl(i,j,k) + qpi(i,j,k))*www
                        tvcl(k) = tvcl(k) + tvirt(i,j,k)*www 
                        tvcla(k) = tvcla(k) + (tvirt(i,j,k) - tvz(k))*www
                        tacl(k) = tacl(k) + tabs(i,j,k)*www 
                        twcl(k) = twcl(k) + t(i,j,k)*0.5*(w(i,j,k+1)+w(i,j,k))*www
                        qwcl(k) = qwcl(k) + (qv(i,j,k)+qcl(i,j,k)+qci(i,j,k))*0.5*(w(i,j,k+1)+w(i,j,k))*www
                        tvwcl(k) = tvwcl(k)+tvirt(i,j,k)*0.5*(w(i,j,k+1)+w(i,j,k))*www
                        qcwcl(k) = qcwcl(k) + qcc*0.5*(w(i,j,k+1)+w(i,j,k))*www
                        qiwcl(k) = qiwcl(k) + qii*0.5*(w(i,j,k+1)+w(i,j,k))*www

                        !bloss: frozen MSE and anomaly
                        fmse(k)=fmse(k)+(t(i,j,k)+fac_cond*(qv(i,j,k)+qcl(i,j,k) &
                                         +qci(i,j,k)+qpl(i,j,k)+qpi(i,j,k)))*www
                        fmsecla(k)=fmsecla(k)+(t(i,j,k)-t0(k) &
                             +fac_cond*(qv(i,j,k)+qcl(i,j,k)+qci(i,j,k)+qpl(i,j,k) &
                                         +qpi(i,j,k)-q0(k)-qp0(k)))*www

                        !bloss: pressure gradient forces
                        dpdxcl(k) = dpdxcl(k) - (pp(i,j,k)-pp(i-1,j,k))*100./dx*wgtu(j,k)*terrau(i,j,k)
                        dpdycl(k) = dpdycl(k) - (pp(i,j,k)-pp(i,jb,k))*100./(dy*adyv(j))*wgtv(j,k)*terrav(i,j,k)
                        dpdzcl(k) = dpdzcl(k) - & 
                             ( 0.5*(pp(i,j,kc)-pres(kc)-pp(i,j,k)+pres(k))*100./(dz*adzw(kc)) &
                             + 0.5*(pp(i,j,k)-pres(k)-pp(i,j,kb)+pres(kb))*100./(dz*adzw(k)))*wgt(j,k)*terra(i,j,k)
               ...
               !                   + (w(i,j,k)-w(i,jb,k))*dz/(dy*ady(j))

                            uwsubgrid = 0.
                            vwsubgrid = 0.
                           ! resolved
                           uwresolved = 0.25*(w(i,j,k)+w(i-1,j,k)) &
                                *(u(i,j,k)+u(i,j,k-1)-u0(k)-u0(k-1))*wgtu(j,k)*terrau(i,j,k)
                           vwresolved = 0.25*(w(i,j,k)+w(i,jb,k)) &
                                *(v(i,j,k)+v(i,j,k-1)-v0(k)-v0(k-1))*wgtv(j,k)*terrav(i,j,k)
                        end if
                        ...
                        rhowmsecla(k) = rhowmsecla(k) &
                             + tmprhow*(tmpmse - t0(k) - fac_cond*(q0(k) + qp0(k)))*www
                        rhowqtcl(k) = rhowqtcl(k) + tmprhow*tmpqt*www
                        rhowqtcla(k) = rhowqtcla(k) + tmprhow*(tmpqt-q0(k))*www
                        rhowtlcl(k) = rhowtlcl(k) + tmprhow*(t(i,j,k))*www
                        rhowtlcla(k) = rhowtlcla(k) + tmprhow*(t(i,j,k)-t0(k))*www
                        rhowtvcl(k) = rhowtvcl(k) + tmprhow*tvirt(i,j,k)*www
                        rhowtvcla(k) = rhowtvcla(k) + tmprhow*(tvirt(i,j,k)-tvz(k))*www

                        rhouwcl(k) = rhouwcl(k) + tmprhow*(u(i,j,k) - u0(k))*www
                        rhovwcl(k) = rhovwcl(k) + tmprhow*(v(i,j,k) - v0(k))*www
                        rhowwcl(k) = rhowwcl(k) + rho(k)*0.5*(w(i,j,k)**2 + w(i,j,k+1)**2)*www

                     endif
                  end do
               end do
               condavg_factor(k,ncond) = condavg_factor(k,ncond)+clds(k)
               wacl(k) = wcl(k)

            end do

            call hbuf_put(TRIM(condavgname(ncond)),clds,factor_xy)
	 ...
	 do j=1,ny
	  do i=1,nx
	    if(qcl(i,j,k)+qci(i,j,k).gt.coef) then
	      hydro(k) = hydro(k) + wgt(j,k)*terra(i,j,k)
	      tmp(1)=0.5*(w(i,j,k+1)+w(i,j,k))
	      if(tmp(1).gt.0.) then
		prof1(k)=prof1(k)+rho(k)*tmp(1)*wgt(j,k)*terra(i,j,k)
	      else
	        prof2(k)=prof2(k)+rho(k)*tmp(1)*wgt(j,k)*terra(i,j,k)
	      endif	
	    elseif(qpl(i,j,k)+qpi(i,j,k).gt.1.e-4) then
	      hydro(k) = hydro(k) + wgt(j,k)*terra(i,j,k)
	      if(w(i,j,k)+w(i,j,k+1).lt.0.) &
 	         prof3(k)=prof3(k)+rho(k)*0.5*(w(i,j,k+1)+w(i,j,k))*wgt(j,k)*terra(i,j,k)      
	    endif
	  end do
	 end do
	...
	do k=1,nzm
	 ...
	 do j=1,ny
	  do i=1,nx
	    ...
	    if(condition) then
	      prof1(k)=prof1(k)+rho(k)*0.5*(w(i,j,k+1)+w(i,j,k))*wgt(j,k)*terra(i,j,k)      
	      if(condition_cl) then
	        cldd(k)=cldd(k)+cld(i,j,k)*wgt(j,k)*terra(i,j,k)
	      end if
	    endif
	  end do
	 end do
	...
	do k=1,nzm
	 ...
	 do j=1,ny
	  do i=1,nx
	    ...
	    if(condition) then
	      if(condition_cl) then
	        prof2(k)=prof2(k)+rho(k)*0.5*(w(i,j,k+1)+w(i,j,k))*wgt(j,k)*terra(i,j,k)      
	        cldd(k)=cldd(k) + cld(i,j,k)*wgt(j,k)*terra(i,j,k)
	      else
	        prof3(k)=prof3(k)+rho(k)*0.5*(w(i,j,k+1)+w(i,j,k))*wgt(j,k)*terra(i,j,k)      
	      end if
...
!  Radiation and other stuff
	...
	do k=nzm,1,-1
	 ...
	 do j=1,ny
	  do i=1,nx
	    ...
	    cwpt(i,j)=cwpt(i,j)+tmp(1)*(qcl(i,j,k)+qci(i,j,k))
            if(docup) then
	      acld= max(acld,cld(i,j,k)) 
              if(pres(k).ge.700.) then
	        acldl= max(acldl,cld(i,j,k))*wgt(j,k)*terra(i,j,k)
              else if(pres(k).le.400.) then
	        acldh= max(acldh,cld(i,j,k))*wgt(j,k)*terra(i,j,k)
              else
	        acldm= max(acldm,cld(i,j,k))*wgt(j,k)*terra(i,j,k) 
	      end if
            end if
            if(pres(k).ge.700.) then
	      cwpl(i,j)=cwpl(i,j)+tmp(1)*(qcl(i,j,k)+qci(i,j,k))*wgt(j,k)*terra(i,j,k)
            else if(pres(k).le.400.) then
	      cwph(i,j)=cwph(i,j)+tmp(1)*(qcl(i,j,k)+qci(i,j,k))*wgt(j,k)*terra(i,j,k)
            else
	      cwpm(i,j)=cwpm(i,j)+tmp(1)*(qcl(i,j,k)+qci(i,j,k))*wgt(j,k)*terra(i,j,k)
	    end if
	    if(cwpt(i,j).gt.cwpmax.and.topind(i,j).eq.1)topind(i,j)=k
	  end do
	 end do
	end do
        if(docup) then
         s_acld = s_acld + acld
         s_acldl = s_acldl + acldl
         s_acldm = s_acldm + acldm
         s_acldh = s_acldh + acldh
        end if
	...
	do j=1,ny
	 do i=1,nx
           if(.not.docup) then 
	     if(cwpt(i,j).gt.cwpmax) s_acld=s_acld+wgty(j)
	     if(cwpl(i,j).gt.cwpmax) s_acldl=s_acldl+wgty(j)
	     if(cwpm(i,j).gt.cwpmax) s_acldm=s_acldm+wgty(j)
	     if(cwph(i,j).gt.cwpmax) s_acldh=s_acldh+wgty(j)
	     if(tabs(i,j,topind(i,j)).lt.245.) s_acldcold=s_acldcold+wgty(j)
           end if
           s_sst = s_sst + sstxy(i,j)*wgty(j)
           zzz = z(z_inv_ind(i,j))*0.001
           z_inv = z_inv + zzz*wgty(j)
           z2_inv = z2_inv + zzz**2*wgty(j)
           cwpmean = cwpmean + cwpt(i,j)*wgty(j)
           cwp2 = cwp2 + cwpt(i,j)**2*wgty(j)
           if(z_base_ind(i,j).gt.0) then
             z_cbmn = z_cbmn + z(z_base_ind(i,j))*0.001*coef*wgty(j)
             z2_cb = z2_cb + (z(z_base_ind(i,j))*0.001)**2*coef*wgty(j)
             z_cb = min(z_cb,z(z_base_ind(i,j))*0.001)
           end if
           if(z_top_ind(i,j).gt.0) then
             z_ctmn = z_ctmn + z(z_top_ind(i,j))*0.001*coef*wgty(j)
             z2_ct = z2_ct + (z(z_top_ind(i,j))*0.001)**2*coef*wgty(j)
             z_ct = max(z_ct,z(z_top_ind(i,j))*0.001)
           end if
           s_t2m = s_t2m +t2m(i,j)*wgty(j)
           s_q2m = s_q2m +q2m(i,j)*wgty(j)
           s_u10m = s_u10m +u10m(i,j)*wgty(j)
           s_v10m = s_v10m +v10m(i,j)*wgty(j)
           if(SLM) then
            s_snowd = s_snowd +snow_mass(i,j)/rho_snow*wgty(j)
            s_snowt = s_snowt +snowt(i,j)*wgty(j)
            s_lhfc = s_lhfc +lhf_canop(i,j)*wgty(j)
            s_shfc = s_shfc +shf_canop(i,j)*wgty(j)
            s_lhfs = s_lhfs +lhf_soil(i,j)*wgty(j)
            s_shfs = s_shfs +shf_soil(i,j)*wgty(j)
            s_tsoil = s_tsoil +soilt(1,i,j)*wgty(j)
            s_tcanopy = s_tcanopy +t_canop(i,j)*wgty(j)
            s_soilflux = s_soilflux +soilflux(i,j)*wgty(j)
           end if
           k = k_terra(i,j)
           s_shf = s_shf + rhow(k)*cp*fluxbt(i,j)*wgty(j)
           s_lhf = s_lhf + rhow(k)*lcond*fluxbq(i,j)*wgty(j)
	 end do
	end do
	...
	call hbuf_put('HLLAT',tlat+tlatqi+tlatqc,factor_xy*86400./dtn)
	call hbuf_put('HLDISS',tdiss,factor_xy*86400./dtn)
	call hbuf_put('HLPDISS',tpdiss,factor_xy*86400./dtn)
	call hbuf_put('HLBUOY',tbuoy,factor_xy*86400./dtn)
...
!  Apparent heat/moisture sources/sinks
	...
	call hbuf_put('Q2',prof3,factor_xy*86400.) 	
        ...
        if(docloud) call compute_instr_diags()
...
!===================================================
        ...
        if(dobuildings) call buildings_stat()
...
! function to aid computation of various water paths statistics

real(8) function water_sum(q)

  use grid, only: wgty,nx,ny,nzm,nx_gl,ny_gl
  use vars, only : adz,dz,rho
  use terrain, only: terra
  implicit none
  real, intent(in) :: q(nx,ny,nzm)
  real(8) tmp
  integer i,j,k
  ...
  do k=1,nzm
    ...
    do j=1,ny
      do i=1,nx
        tmp = tmp + q(i,j,k)*wgty(j)*terra(i,j,k)
      end do
    end do
    water_sum = water_sum + tmp*adz(k)*dz*rho(k)
  end do

...
subroutine advect_scalar3D_tracer (f, u, v, w, rho, rhow, flux)
 	
!     positively definite monotonic advection with non-oscillatory option
!     modified to mitigate the terrain effects on scalars.
!     WARNING!!! Don't use with temperature fields !!
...
real mx1(0:nxp1,0:nyp1,nzm)
real mn1(0:nxp1,0:nyp1,nzm)
...
if(nonos) then
 ...
 do k=1,nzm
  do j=-1,nyp2
   do i=-1,nxp3
    uuu(i,j,k)=terrau(i,j,k)*(max(0.,u(i,j,k))*f(i-1,j,k)+min(0.,u(i,j,k))*f(i,j,k))
   end do
  end do
 end do
 do k=1,nzm
  do j=-1,nyp3
   do i=-1,nxp2
    vvv(i,j,k)=terrav(i,j,k)*(max(0.,v(i,j,k))*f(i,j-1,k)+min(0.,v(i,j,k))*f(i,j,k))
   end do
  end do
 end do
 do k=1,nzm
  ...
  do j=-1,nyp2
   do i=-1,nxp2
    www(i,j,k)=terraw(i,j,k)*(max(0.,w(i,j,k))*f(i,j,kb)+min(0.,w(i,j,k))*f(i,j,k))
   end do
  end do
  flux(k) = 0.
  do j=1,ny
  ...
  do j=-1,nyp2
   do i=-1,nxp2
      f(i,j,k)=max(0.,f(i,j,k) -(uuu(i+1,j,k)-uuu(i,j,k)+ &
                    vvv(i,j+1,k)-vvv(i,j,k)+www(i,j,k+1)-www(i,j,k))*ig(j,k)*terra(i,j,k)) 
   end do
  end do
 end do 
 ...
 do k=1,nzm
  ...
  do j=0,nyp1
   ...
   do i=0,nxp2
    ib=i-1
    uuu(i,j,k)=terrau(i,j,k)*(andiff(f(ib,j,k),f(i,j,k),u(i,j,k),g(j,k)+g(j,k)) &
 ...
 do k=1,nzm
  ...
  do j=0,nyp2
   ...
   do i=0,nxp1
    ib=i-1
    ic=i+1
    vvv(i,j,k)=terrav(i,j,k)*(andiff(f(i,jb,k),f(i,j,k),v(i,j,k),g(j,k)+g(jb,k)) &
 ...
 do k=1,nzm
  ...
  do j=0,nyp1
   ...
   do i=0,nxp1
    ib=i-1
    ic=i+1
    www(i,j,k)=terraw(i,j,k)*(andiff(f(i,j,kb),f(i,j,k),w(i,j,k),g(j,k)+g(j,kb)) &
...
do k=1,nzm
 ...
 !     most likely truncation error.
   ...
   f(i,j,k)=max(0.,f(i,j,k) -(uuu(i+1,j,k)-uuu(i,j,k)+ &
                    vvv(i,j+1,k)-vvv(i,j,k)+www(i,j,k+1)-www(i,j,k))*ig(j,k)*terra(i,j,k))
!   f(i,j,k)=max(mn1(i,j,k),min(mx1(i,j,k),f(i,j,k) -(uuu(i+1,j,k)-uuu(i,j,k)+ &
!                    vvv(i,j+1,k)-vvv(i,j,k)+www(i,j,k+1)-www(i,j,k))*ig(j,k)))
  end do
 end do
end do 

end subroutine advect_scalar3D_tracer

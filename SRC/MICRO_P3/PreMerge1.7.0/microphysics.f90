...
!bloss     tmp_micro_proc_rates(1:nzm,1:nmicro_proc) = micro_proc_rates(1:nx,j,1:nzm,1:nmicro_proc) !BG initialize
...
!bloss(2024-12: iloop)(2024-12)      do i=1,nx
        ...
        call p3_main( qc=tmpqc(1,kk:), nc=tmpnc(1,kk:), qr=tmpqr(1,kk:), &
              ...
              diag_ze=diag_zdbz(1,kk:), diag_effc=diag_effc(1,kk:), diag_effi=diag_effi(1,kk:,:), &
              diag_vmi=diag_vmi(1,kk:,:), diag_di=diag_di(1,kk:,:), diag_rhoi=diag_rhopo(1,kk:,:), &
...
!bloss(2024-12: iloop)(2024-12)      end do ! i
    ...
    ! Note that only tmp*(1,kk:nzm) has been affected by microphysics.
     ...
     if(reff_ice_holds_Dge) then
       ...
       reffi(i,j,1:nzm) = reffi(i,j,1:nzm)*4./3./1.1547
...
!bloss     CloudLiquidLambda(1:nx,j,1:nzm) = tmp_cl_lambda(1:nx,1:nzm)
     ...
     prec_xy(i, j) = prec_xy(i,j) + pcprt_liq(1)*1000.0*dtn & !m/s-->mm/s->mm
                                +  pcprt_sol(1)*1000.0*dtn !m/s-->mm/s->mm
     preca_xy(i, j) = preca_xy(i,j) + pcprt_liq(1)*1000.0*dtn/dz & !m/s-->mm/s->mm/dz
                                +  pcprt_sol(1)*1000.0*dtn !m/s-->mm/s->mm
...
!bloss(2024-12: iloop)       do i = 1, nx
         ...
         stend(k,:) = stend(k,:) + tmp_stend(1,k,:) !bloss(stend)
         mtend(k,:) = mtend(k,:) + tmp_mtend(1,k,:) !bloss(mtend)
!bloss(2024-12: iloop)       end do !i=1,nx
  ...
  !   end do !i=1,nx
     ...
     if(dostatis) then
       ...
       do k = 1,nzm
           tlat(k) = tlat(k) &
                - dtn*fac_cond*( tmp_stend(1,k,iqv) + tmp_stend(1,k,iqr) )
           qpfall(k) = qpfall(k) + dtn*tmp_stend(1,k,iqv) &
                + dtn*tmp_stend(1,k,iqr)
           do iice = 1,nCat
              tlat(k) = tlat(k) - dtn*fac_sub*tmp_stend(1,k,iqit(iice))
              qpfall(k) = qpfall(k) + dtn*tmp_stend(1,k,iqit(iice))
           end do
       end do
       ...
       !  integrated up to level at which output
         ...
         do k = 1,nzm    !BG diag_effc/effi is in meters!
          ...
          qxoeffr(k,iqcl) = qxoeffr(k,iqcl) + tmpqc(1,k)/(1.e-20+ diag_effc(1,k))
         ...
         do iice = 1,nCat
            do k = 1,nzm
               ...
               if((tmpqit(1,k,iice).gt.1.e-6).AND.(diag_effi(1,k,iice).gt.1.e-6)) then
                  ...
                  qxoeffr(k,iqit(iice)) = qxoeffr(k,iqit(iice)) + tmpqit(1,k,iice)/(1.e-20+ diag_effi(1,k,iice))
                  trtau(k,iqit(iice)) = trtau(k,iqit(iice)) + tmpi
               end if
            end do ! do k = 1,nzm
...
subroutine micro_diagnose()
   ...
   if(doallice_radiatively_active) then
     where(qcl(1:nx,1:ny,1:nzm)+qci(1:nx,1:ny,1:nzm)+qpi(1:nx,1:ny,1:nzm).gt.0)
...
subroutine micro_statistics()
  ...
  call hbuf_put('QINI',tr0,factor_xy*mkoutputscale(inci(1))*mkoutputscale(iqit(1)))

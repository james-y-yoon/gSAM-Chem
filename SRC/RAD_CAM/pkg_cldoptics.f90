...
!
    ...
    real(r4), intent(in) :: landfrac(pcols)      ! Land fraction
    real(r4), intent(in) :: icefrac(pcols)       ! Ice fraction
    real(r4), intent(in) :: t(pcols,pver)        ! Temperature
    real(r4), intent(in) :: ps(pcols)            ! Surface pressure
    real(r4), intent(in) :: pmid(pcols,pver)     ! Midpoint pressures
    real(r4), intent(in) :: landm(pcols)
    real(r4), intent(in) :: snowh(pcols)         ! Snow depth over land, water equivalent (m)
!
! Output arguments
!
    real(r4), intent(out) :: rel(pcols,pver)      ! Liquid effective drop size (microns)
    real(r4), intent(out) :: rei(pcols,pver)      ! Ice effective drop size (microns)
!
...
!------------------------------Parameters-------------------------------
!
    real(r4) kabsl                  ! longwave liquid absorption coeff (m**2/g)
...
!
    ...
    real(r4), intent(in) :: clwp(pcols,pver)       ! cloud liquid water path (g/m**2)
    real(r4), intent(in) :: rei(pcols,pver)        ! ice effective drop size (microns)
    real(r4), intent(in) :: fice(pcols,pver)       ! fractional ice content within cloud
!
! Output arguments
!
    real(r4), intent(out) :: emis(pcols,pver)       ! cloud emissivity (fraction)
!
!---------------------------Local workspace-----------------------------
!
    integer i,k                 ! longitude, level indices
    real(r4) kabs                   ! longwave absorption coeff (m**2/g)
    real(r4) kabsi                  ! ice absorption coefficient
!
!-----------------------------------------------------------------------
!
    do k=1,pver
       do i=1,ncol
          ...
          emis(i,k) = min(0.999,1. - exp(-1.66*kabs*clwp(i,k)))
       end do
    end do
!
    return
...
!
    ...
    real(r4), intent(in) :: pint(pcols,pverp)   ! Interface pressure
    real(r4), intent(in) :: cld(pcols,pver)     ! Fractional cloud cover
!
! Output arguments
!
    real(r4), intent(out) :: pmxrgn(pcols,pverp)! Maximum values of pressure for each
...
!
    ...
    real(r4) pnm(pcols,pverp)    ! Interface pressure
...
!
    ...
    real(r4), intent(in) :: zi(pcols,pverp)      ! height at layer interfaces(m)
    real(r4), intent(in) :: tpw(pcols)           ! total precipitable water (mm)
!
! Output arguments
!
    real(r4) clwp(pcols,pver)     ! cloud liquid water path (g/m**2)
    real(r4) hl(pcols)            ! liquid water scale height
    real(r4) rhl(pcols)           ! 1/hl

!
!---------------------------Local workspace-----------------------------
!
    integer i,k               ! longitude, level indices
    real(r4) clwc0                ! reference liquid water concentration (g/m**3)
    real(r4) emziohl(pcols,pverp) ! exp(-zi/hl)
!
!-----------------------------------------------------------------------
!
...
!
    do i=1,ncol
       hl(i)  = 700.0*log(max(tpw(i)+1.0_r4,1.0_r4))
       rhl(i) = 1.0/hl(i)
    end do
!
...
! 
!-----------------------------------------------------------------------
!bloss    use physconst,          only: tmelt
    implicit none
!------------------------------Arguments--------------------------------
!
! Input arguments
!
    ...
    real(r4), intent(in) :: icefrac(pcols)       ! Ice fraction
    real(r4), intent(in) :: snowh(pcols)         ! Snow depth over land, water equivalent (m)
    real(r4), intent(in) :: landm(pcols)         ! Land fraction ramping to zero over ocean
    real(r4), intent(in) :: t(pcols,pver)        ! Temperature
...
!
    real(r4), intent(out) :: rel(pcols,pver)      ! Liquid effective drop size (microns)
!
!bloss: hard-wire melting temperature, so that physconst module is not required.
    real(r4), parameter :: tmelt = 273.16_r4  ! specify melting temperature
!
!---------------------------Local workspace-----------------------------
!
    integer i,k               ! Lon, lev indices
    real(r4) rliqland         ! liquid drop size if over land
    real(r4) rliqocean        ! liquid drop size if over ocean
    real(r4) rliqice          ! liquid drop size if over sea ice
!
!-----------------------------------------------------------------------
!
    rliqocean = 14.0
...
! jrm Reworked effective radius algorithm
          ...
          rel(i,k) = rel(i,k) + (rliqice-rel(i,k)) * min(1.0_r4,max(0.0_r4,icefrac(i)))
! end jrm
       end do
    end do
...
!===============================================================================
  subroutine reitab(ncol, t, re)
    ...
    integer, intent(in) :: ncol
    real(r4), intent(out) :: re(pcols,pver)
    real(r4), intent(in) :: t(pcols,pver)
    real(r4) retab(95)
    real(r4) corr
    integer i
    integer k

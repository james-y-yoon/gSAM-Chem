...
subroutine radcswmx(i0,j0,irank,lchnk   ,ncol    ,                            &
...
! Upward diffuse solar flux at surface - Marat K
   real(r4), intent(out) :: solsud(pcols)     ! Up Diffuse solar rad on surface (< 0.7)
   real(r4), intent(out) :: sollud(pcols)     ! Up Diffuse solar rad on surface (>= 0.7)
...
! 
   ...
   real(r4) fluxdup(nspint,pcols)  ! Up diffuse  flux at surface (Marat K)
!
...
! 
   ...
   solld(1:ncol)    = 0.0_r4
! Marat K
   solsud(1:ncol)    = 0.0_r4
   sollud(1:ncol)    = 0.0_r4
...
! Compute upward diffuse flux at surface: (Marat K)
               k = pverp
               do ns = 1, nspint
                xexpt = exptdn(ns,k,iconfig,i)
                xtdnt = tdntot(ns,k,iconfig,i)
                ...
                xrups = rupdir(ns,k,iconfig,i)
                rdenom = 1._r4/(1._r4 - xrdnd * xrupd)
                fluxdup(ns,i) = fluxdup(ns,i) + xwgt *  &
                               ((xtdnt - xexpt) * xrupd * rdenom)
               end do ! do ns = 1, nspint
...
! For upward diffuse flux (Marat K):
            do i = 1, Nday
            do ns = 1, nspint
               fluxdup(ns,i)=fluxdup(ns,i) / totwgt(i)
            end do ! do i = 1, nday
            end do ! do i = 1, nday
...
! 
            if (wavmid(ns) < 0.7_r4) then
               ...
               solsud(i) = solsud(i)+fluxdup(ns,i)*solflx(i)*0.001_r4 ! Marat K
            else
               soll(i)  = soll(i) + wexptdn(ns,i)*solflx(i)*0.001_r4
               solld(i) = solld(i)+(fluxdn(ns,pverp,i)-wexptdn(ns,i))*solflx(i)*0.001_r4 
               sollud(i) = sollud(i)+fluxdup(ns,i)*solflx(i)*0.001_r4 ! Marat K
